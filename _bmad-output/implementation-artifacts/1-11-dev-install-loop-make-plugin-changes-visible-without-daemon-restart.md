...
As **an engineer iterating on the flow plugin on a feature branch in a worktree**,
...
PR #90 (Story 3.2) was the first user-surface story that **could not produce real-Claude-Code evidence** for its `/flow:scan` AC. The reason is structural, and is the same wall every subsequent user-surface story (3.5, 3.6, 4.x slash commands, …) will hit:

1. **The crew marketplace is registered at `/Users/<user>/projects/crew/` (main).** `~/.claude/plugins/known_marketplaces.json` resolves `flow@flow` to a `directory` source at the main checkout. A worktree at `.worktrees/story/3-2-…/` containing the new skill is invisible to `/plugin install flow@flow`.
...
3. **`/plugin uninstall flow@flow && /plugin install flow@flow` makes it worse.** The reinstall wipes the overlay and re-copies from main (which doesn't have the new skill).
4. **Spike 8739bbf (PR #85) discovered that for a *current-branch* dev loop the live-source path already works** — `tsc --watch` + `/reload-plugins` rebuilds and rebinds in ~5–8 s. That spike is the basis for `plugins/flow/docs/worktree-smoke.md`'s "Daily dev loop" section. **But the worktree-branch case is unsolved** — the marketplace source is wrong, the daemon's skill-index cache is sticky across `/reload-plugins`, and the cache overlay gets wiped on reinstall.
...
Add a `pnpm dev:install` (or equivalently-named) script under `plugins/flow/scripts/` that, when run from inside a git worktree, makes the worktree's `plugins/flow/` tree (skills + `mcp-server/dist/` + catalogue) authoritative for the next Claude Code session — so a user-surface AC on any branch can be smoke-verified the same way `/flow:status` was on main — and document the script's place in the dev loop alongside the existing watch-build flow.

### This story does NOT

- (a) Touch `_bmad-output/implementation-artifacts/sprint-status.yaml` — orchestrator-owned.
- (b) Change the production install path (`/plugin marketplace add ./` → `/plugin install flow@flow` → restart). End-users on a clean checkout follow `plugins/flow/docs/README-install.md` unchanged. The dev-install path is a **separate, engineer-only** code path with its own command and its own doc.
- (c) Replace the existing "Daily dev loop" (watch-build + `/reload-plugins`) documented in `plugins/flow/docs/worktree-smoke.md`. That loop is still correct for editing on the **current branch**. This story covers the **switching-to-a-worktree-branch** case the spike explicitly left unsolved (`symlink-dev-install.md` § "Recipe" still applies; this story replaces *that* uninstall+install ritual with the new command).
...
- (e) Modify `plugins/flow/.claude-plugin/plugin.json`, `marketplace.json`, or `~/.claude/plugins/known_marketplaces.json`. The chosen mechanism (see Task 2) operates on the install cache directory; it does not rewrite Claude Code's own registries.
...
> **Verbatim from epic (with the trailing `---` phantom-AC stripped).** ACs 1–5 are the prose ACs from the epic; AC6 is the deterministic content-structure anchor; AC7 is the integration scenario. Tagging follows `plugins/flow/docs/user-surface-acs.md`.
...
**Then** the slash-command picker lists every skill present under `plugins/flow/skills/` (including any new ones added since the last "real" `/plugin install`) — i.e. the daemon's skill-index cache no longer masks the new state. _(The mechanism — daemon kill, cache-invalidation file, symlink trick, or whatever the spec chooses — is an implementation detail.)_
...
**Then** the production install path (`/plugin install flow@flow`) and the new dev-install path are clearly distinguished, with one short paragraph explaining when to use which.
...
- MUST resolve the **target** as `~/.claude/plugins/cache/crew/crew/<version>/` where `<version>` is read from `plugins/flow/.claude-plugin/plugin.json`.
...
- MUST be invocable as `pnpm --dir plugins/flow dev:install` (since the repo root has no `package.json`). The dev-loop doc (Task 4) names this exact invocation as the canonical operator literal; deviating from it requires updating the doc in the same change.
...
- NEVER spawn a `kill`/`pkill` against an arbitrary PID. If the chosen mechanism includes a daemon-restart step, it MUST target only processes whose command line includes `plugins/flow/mcp-server/dist/index.js` (verified via `pgrep -f`), and MUST document the kill in stdout before issuing it. If no programmatic daemon-restart is reliable, the script MUST instead print "now restart Claude Code" as its final line and exit 0.
- NEVER write to the cache without first verifying the source tree contains `plugins/flow/.claude-plugin/plugin.json` and `plugins/flow/mcp-server/dist/index.js` (or running the build to produce the latter). Empty / broken caches must not be createable by this script.
...
- [x] **Task 1 — Pick the mechanism (research subtask + decision record) (AC: 1, 2, 4)**
  ...
  - [x] 1.2 **Research subtask (the daemon re-scan trigger).** Finding: no programmatic invalidation API is available. `/reload-plugins` restarts the MCP server but does NOT re-scan the skill index across sessions. A full Claude Code restart is the only reliable mechanism. Documented in `plugins/flow/docs/spikes/dev-install-decision.md`.
  - [x] 1.3 Record the chosen mechanism in `plugins/flow/docs/spikes/dev-install-decision.md` (new file under the existing `spikes/` dir — mirrors the spike doc convention from 8739bbf). One page: chosen mechanism, ruled-out alternatives, the Task 1.2 result.
  - [x] 1.4 `--kill-daemon` flag implemented in `dev-install.sh` (gated, off by default). Uses targeted `pgrep -f plugins/flow/mcp-server/dist/index.js` — documented in decision doc and script header.

- [x] **Task 2 — Implement `plugins/flow/scripts/dev-install.sh` (AC: 1, 3, 4)**
  - [x] 2.1 Created `plugins/flow/scripts/dev-install.sh` (POSIX shell, `set -e`, exit-code contract in header).
...
- [x] **Task 3 — Wire `pnpm dev:install` (AC: 1, 6)**
  - [x] 3.1 Added `"dev:install": "./scripts/dev-install.sh"` to `plugins/flow/package.json`.
  - [x] 3.2 Repo root has no `package.json` (confirmed). Engineer uses `pnpm --dir plugins/flow dev:install`.
  - [x] 3.3 Dev-loop doc uses `pnpm --dir plugins/flow dev:install` as the canonical literal.

- [x] **Task 4 — Write the engineer-facing dev-loop doc (AC: 5)**
  - [x] 4.1 Created `plugins/flow/docs/dev-loop.md`.
  - [x] 4.2 Contents implemented: two-path table, when-to-use paragraph, fenced bash block with `pnpm --dir plugins/flow dev:install`, three-bullet mechanism summary, after-running guidance (restart Claude Code), relationship to daily loop, troubleshooting table (all four exit codes).
  - [x] 4.3 Updated `plugins/flow/docs/README-install.md` with one paragraph at the very top distinguishing engineer vs end-user audience and pointing at `dev-loop.md`.
  - [x] 4.4 Updated `plugins/flow/docs/worktree-smoke.md` under "Recipe" with a one-command alternative callout.

- [x] **Task 5 — Add the vitest harness (AC: 6, 7)**
  - [x] 5.1 Added `plugins/flow/mcp-server/tests/dev-install.test.ts` (mirrors existing pattern, uses `import.meta.url`).
  - [x] 5.2 AC6 content-anchor: executable bit assertion; `$HOME/.claude/plugins/cache/crew/crew` substring in script; `pnpm --dir plugins/flow dev:install` in dev-loop.md.
  ...
  - [x] 5.4 File header cites Story 1.11 AC6/AC7 and links `plugins/flow/docs/dev-loop.md`.
...
**NEW files:**
- `plugins/flow/scripts/dev-install.sh` — the script itself (executable).
- `plugins/flow/docs/dev-loop.md` — the engineer-facing dev-loop doc.
- `plugins/flow/docs/spikes/dev-install-decision.md` — one-page decision record from Task 1.
- `plugins/flow/mcp-server/tests/dev-install.test.ts` — vitest harness.

**UPDATE files:**
- `plugins/flow/package.json` — add `"dev:install"` script entry.
- `plugins/flow/docs/README-install.md` — add one paragraph at the very top distinguishing prod vs dev install paths and pointing at `dev-loop.md`.
- `plugins/flow/docs/worktree-smoke.md` — add one sentence under "Recipe" pointing at `dev-loop.md` / `pnpm dev:install` as the supported one-command alternative.
...
- `plugins/flow/.claude-plugin/plugin.json`, `plugins/flow/.claude-plugin/marketplace.json` (if/when present), or any other plugin manifest.
- `~/.claude/plugins/known_marketplaces.json`, `~/.claude/settings.json`, `~/.claude/plugins/cache/<other-plugin>/**`.
- `plugins/flow/scripts/worktree-smoke.sh` — preserved as the explicit three-line recipe printer.
...
**`plugins/flow/package.json`** (5 lines as of authoring):
...
**`plugins/flow/docs/README-install.md`** (127 lines):
...
**`plugins/flow/docs/worktree-smoke.md`** (103 lines):
...
**`plugins/flow/scripts/worktree-smoke.sh`** (47 lines):
...
- The production install path (`/plugin marketplace add ./` → `/plugin install flow@flow` → `/reload-plugins`) documented in `README-install.md`. End-users on a clean checkout follow that path unchanged.
- The daily dev loop (`pnpm --dir plugins/flow/mcp-server build:watch` + `/reload-plugins`) from `worktree-smoke.md`. Still the fastest loop for current-branch edits.
...
- Every existing vitest suite under `plugins/flow/mcp-server/tests/`. Zero skips, zero new warnings.
...
{
  "ac_refs": [6],
  "test_path": "plugins/flow/mcp-server/tests/dev-install.test.ts",
  "test_command": "pnpm --dir plugins/flow test dev-install"
}
```
...
// user_surface_verified — covers AC1, AC2, AC5 (minimum); over-cover with [1,2,5,6] also legal
{
  ...
  "observations": [
    ...
    {"ac_ref": 2, "pasted_output": "<Claude Code slash-command picker output showing the sentinel skill in the /flow: namespace>"},
...
Story 1.10's vitest harness (`readme-install.test.ts`) asserts a slash-command literal allowlist (`/plugin marketplace add ./`, `/plugin install flow@flow`, `/flow:status`). The new paragraph contains the literal `pnpm dev:install` in a fenced ` ```bash ` code block — this **adds a literal** to the allowlist if the test parses `bash`-tagged blocks for `pnpm` literals. Check `readme-install.test.ts`'s regex (Task 5 of 1.10 specified slash-command literals matching `^/[a-z][\w:-]*` only — `pnpm dev:install` doesn't match that pattern and will be ignored). If the allowlist is broader than the slash-command pattern, update the allowlist in this story's PR. Verify before assuming.
...
PR #90 (Story 3.2) shipped via the **automated route** on its AC4 because the user-surface smoke step could not be satisfied: `/flow:scan` did not appear in the slash-command picker even after every documented workaround. The retro comment on PR #90 (cited verbatim above in "What this story fixes") is the canonical failure narrative. Every choice in this story's contract above is calibrated against that failure — particularly the "MUST resolve source as the working tree, not main" and the daemon-restart guidance in Task 1.2.

### Spike 8739bbf (PR #85) — directly relevant prior research

The spike concluded that for **current-branch** edits the live-source path already works (Claude Code runs MCP from `plugins/flow/mcp-server/dist/index.js` in the source tree). That conclusion does NOT extend to the worktree-branch case: the marketplace registers the **main** checkout, not the worktree, so `CLAUDE_PLUGIN_ROOT` resolves to the main checkout and the worktree's `dist/` and `skills/` are invisible. This story is the worktree case the spike explicitly left unsolved.
...
- `8739bbf docs(spike): symlink-based dev install for flow plugin (#85)` — the spike itself.
...
- `plugins/flow/scripts/` already exists with one POSIX shell script (`worktree-smoke.sh`). `dev-install.sh` joins it as the second script with the same conventions (executable bit, exit-code table in header, `set -e`, preflight checks).
- `plugins/flow/docs/spikes/` already exists for spike reports. The Task 1.3 decision doc fits the convention.
- `plugins/flow/mcp-server/tests/` is the canonical vitest home. `dev-install.test.ts` joins the suite per the established one-test-file-per-concern pattern.
- The root repo has **no** `package.json` (verified via `ls /Users/<root>/`). Task 3.2's workspace-root alias is a no-op — the engineer runs `pnpm --dir plugins/flow dev:install` or, if a root `package.json` is later added in another story, the alias can be wired then. The dev-loop doc (Task 4.2) names whichever form is wired.
...
- Spike report: [Source: plugins/flow/docs/spikes/symlink-dev-install.md]
- Worktree-smoke recipe (preserved): [Source: plugins/flow/docs/worktree-smoke.md]
- `worktree-smoke.sh` (preserved): [Source: plugins/flow/scripts/worktree-smoke.sh]
- User-surface tag rules: [Source: plugins/flow/docs/user-surface-acs.md]
...
- Build artefacts contract (preserved): [Source: plugins/flow/docs/README-install.md § "Build artefacts"]
- Existing vitest pattern: [Source: plugins/flow/mcp-server/tests/dist-shipping.test.ts, plugins/flow/mcp-server/tests/pre-pr-gate.test.ts]
...
- Task 1.2 (daemon re-scan trigger): empirical finding — no programmatic invalidation available in Claude Code 2.1.x; full restart required for skill-index refresh. Documented in `plugins/flow/docs/spikes/dev-install-decision.md`.
...
- Chose hybrid symlink mechanism (option 1d from spec). Cache entry becomes a symlink to the worktree's `plugins/flow/` — zero-overhead on re-runs, always-fresh for skill changes, build-check preflight catches stale dist.
- Script exits 3 (not auto-build) when `dist/index.js` is missing — keeps the script fast and predictable; operator runs `pnpm --dir plugins/flow/mcp-server build` once.
...
- `plugins/flow/scripts/dev-install.sh` (NEW, executable)
- `plugins/flow/docs/dev-loop.md` (NEW)
- `plugins/flow/docs/spikes/dev-install-decision.md` (NEW)
- `plugins/flow/mcp-server/tests/dev-install.test.ts` (NEW)
- `plugins/flow/package.json` (MODIFIED — added `dev:install` script)
- `plugins/flow/docs/README-install.md` (MODIFIED — added engineer-audience paragraph at top)
- `plugins/flow/docs/worktree-smoke.md` (MODIFIED — added one-command alternative callout under Recipe)
