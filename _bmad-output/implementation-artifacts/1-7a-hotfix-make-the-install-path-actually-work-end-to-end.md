...
1. **Wrong install command syntax.** Step 3 of `plugins/flow/docs/README-install.md` instructs the user to run `/plugin install plugins/flow`. Claude Code's `/plugin install` command does NOT take a path; it takes a `plugin-name@marketplace-name` reference that resolves against a marketplace previously registered via `/plugin marketplace add <path>`. (Verified against Claude Code docs — `/discover-plugins` and `/plugins-reference` pages.) A marketplace is a directory containing `.claude-plugin/marketplace.json` listing one or more plugins.
2. **`plugin.json` has `"skills": []`.** Even after a correct install, the new `plugins/flow/skills/status.md` is invisible to Claude Code because the plugin manifest's `skills` array doesn't register it. The `/flow:status` slash command never appears in tab-complete.
...
- (a) Change `renderStatus`, `getStatus`, `StatusReportSchema`, or the `/flow:status` skill body — Story 1.7's runtime contract is preserved exactly. The 1.7 unit/integration suite (`get-status.test.ts`, etc.) MUST stay green with zero modifications.
...
- (g) Touch any other plugin file under `plugins/flow/mcp-server/`, `plugins/flow/permissions/`, `plugins/flow/catalogue/`, or `plugins/flow/example/`.
...
**Then** `/flow:status` appears in the `/flow:` namespace,
...
**Given** the repo,
**When** any future story adds a new file under `plugins/flow/skills/`,
...
1. The file IS registered in `plugins/flow/.claude-plugin/plugin.json`'s `skills` array (the normal path), OR
2. The file is named `.gitkeep` (existing convention — placeholder for a tracked-empty directory), OR
3. The file is listed in a documented opt-out file at `plugins/flow/.claude-plugin/skills-opt-out.txt` (one path per line, relative to the plugin root, e.g. `skills/draft.md`) — this opt-out mechanism is the documented escape hatch for in-progress / experimental skills that should not yet be surfaced to Claude Code.
...
```
Orphaned skill file(s) detected under plugins/flow/skills/:
  - skills/<orphan-1>.md
  - skills/<orphan-2>.md
Register each file in plugins/flow/.claude-plugin/plugin.json's "skills" array,
or add it to plugins/flow/.claude-plugin/skills-opt-out.txt (one path per line).
```

**AC4 — vitest asserts the four static install contracts (epic AC4):**

`pnpm test` from `plugins/flow/` adds one new test file: `plugins/flow/mcp-server/tests/install-contract.test.ts`. The suite asserts:

- **AC4a — Root `marketplace.json` exists, parses, and lists the `crew` plugin at the expected path.**
  Read `<repo-root>/.claude-plugin/marketplace.json` via `readFileSync` (resolve `<repo-root>` as `path.resolve(__dirname, "../../../..")` — three levels up from `plugins/flow/mcp-server/tests/`). Assert:
  ...
  5. Exactly one entry in `manifest.plugins` has `{ name: "crew", source: "./plugins/flow" }`. Other entries are tolerated (forward-compat) but the `crew` entry MUST be present with that exact name and source.
...
- **AC4b — `plugin.json`'s `skills` array lists every non-`.gitkeep` `*.md` file under `plugins/flow/skills/`, modulo the opt-out file.**
  Read `plugins/flow/.claude-plugin/plugin.json`. Glob `plugins/flow/skills/**/*.md` (use Node's `fs.readdirSync` recursively or the existing glob pattern from other tests — do NOT introduce a new dependency). Normalise each found path to a forward-slash relative path from the plugin root (e.g. `skills/status.md`). Read the opt-out file `plugins/flow/.claude-plugin/skills-opt-out.txt` if present (otherwise treat as empty). Compute `expected = globbed_paths − opt_out_paths`. Assert `manifest.skills` is a superset of `expected` (every expected path is in the array). When it isn't, **the test failure message MUST match the wording in AC3 verbatim**, listing each orphan as `skills/<filename>`. Path comparisons are case-sensitive, forward-slash normalised, and use POSIX `path.posix.join` for assembly (so the test passes on Linux CI and macOS dev alike).

- **AC4c — The corrected README contains the two-command install sequence.**
  Read `plugins/flow/docs/README-install.md`. Assert:
  1. The file contains the literal substring `/plugin marketplace add .` (checkpoint 3a).
  2. The file contains the literal substring `/plugin install flow@flow` (checkpoint 3b).
  3. The file does NOT contain the obsolete literal `/plugin install plugins/flow` (the bug we're fixing — explicit negative assertion to prevent silent regression).
...
- **AC4d — Story 1.7's `get-status.test.ts` still passes unchanged.**
  This is a meta-assertion: the test file from 1.7 is not modified by this story, and `pnpm test` from `plugins/flow/` runs both `get-status.test.ts` and `install-contract.test.ts` green. Verification: the dev confirms the `get-status.test.ts` file's git diff is empty against `main` at the end of this story, and `pnpm test` exits 0 with both files included.
...
Story 1.7's README had six checkpoints. This story splits checkpoint 3 into two sub-steps (3a `/plugin marketplace add .` and 3b `/plugin install flow@flow`) and renumbers nothing else. The implementation MAY choose one of two equivalent encodings:
...
- [ ] **Task 1 — Rewrite `<repo-root>/.claude-plugin/marketplace.json`** (AC: 1, 4a)
  ...
  - [ ] Replace the entire file contents with the JSON in "Sample artifacts → marketplace.json" below. The file's `name` is `crew`, `owner.name` is `Jack McIntyre` (matches the existing stale file's owner — no change needed there), and `plugins` has exactly one entry pointing at `./plugins/flow`.
...
- [ ] **Task 2 — Update `plugins/flow/.claude-plugin/plugin.json` to register `skills/status.md`** (AC: 2, 4b)
  - [ ] Open `/Users/jackmcintyre/projects/crew/plugins/flow/.claude-plugin/plugin.json`. Today's contents:
    ```json
    {
      "name": "crew",
      "version": "0.1.0",
  ...
  - [ ] Path encoding: forward-slash, relative to the plugin root (`plugins/flow/`). No leading `./`. No trailing slash. (Claude Code accepts this shape per the plugin manifest docs.)
  - [ ] When future stories add new skill files, they extend this array. Story 1.7a does NOT add other skills.

- [ ] **Task 3 — Fix `plugins/flow/docs/README-install.md` checkpoint 3** (AC: 1, 4c)
  - [ ] Open `/Users/jackmcintyre/projects/crew/plugins/flow/docs/README-install.md`. Today's checkpoint 3 (lines 33–47) instructs `/plugin install plugins/flow` — this is the broken command.
...
- [ ] **Task 4 — Define `MarketplaceManifestSchema` and add `install-contract.test.ts`** (AC: 3, 4a, 4b, 4c)
  - [ ] Create `plugins/flow/mcp-server/src/schemas/marketplace-manifest.ts`. Export:
    ```ts
    import { z } from "zod";
  ...
  - [ ] Create `plugins/flow/mcp-server/tests/install-contract.test.ts`. Test layout mirrors the existing `get-status.test.ts` for fixture / import conventions. Imports:
    ```ts
    ...
    ```
  - [ ] Resolve repo root: `const repoRoot = path.resolve(__dirname, "../../../..");`. Resolve plugin root: `const pluginRoot = path.resolve(repoRoot, "plugins/flow");`. **Sanity-check** at the top of the file: `expect(existsSync(path.join(pluginRoot, ".claude-plugin/plugin.json"))).toBe(true)` — if this fails, the test file is in the wrong place; fix that before any other assertion.
  - [ ] **Test cases (one `it` per AC4 sub-clause, plus the orphan-guard test for AC3):**
    1. `it("AC4a — root marketplace.json exists, is valid JSON, satisfies the schema, lists crew@./plugins/flow")` — read, parse, validate against `MarketplaceManifestSchema`, assert the `crew` plugin entry is present with `source: "./plugins/flow"`, assert that source resolves to a directory containing `.claude-plugin/plugin.json`.
    ...
    3. `it("AC4c — README-install.md contains '/plugin marketplace add .' and '/plugin install flow@flow' and does NOT contain '/plugin install plugins/flow'")` — read the file, run three substring assertions (two positive, one negative).
...
- [ ] **Task 5 — (Optional) Create `plugins/flow/.claude-plugin/skills-opt-out.txt` as a placeholder?** (AC: 3)
...
- [ ] **Task 6 — Manual smoke step (post-merge — NOT a vitest gate)** (AC: 1, 2)
  - [ ] After the PR for this story merges, Jack performs the following manual sequence ONCE on a clean checkout (or in a fresh worktree) to confirm AC1's runtime claim and AC2's tab-complete claim:
    ...
    2. `/plugin install flow@flow` → expect a confirmation that the `crew@0.1.0` plugin is installed.
    3. Quit and reopen Claude Code.
    4. Open the slash-command tab-complete → expect `/flow:status` to be listed under the `/flow:` namespace.
    5. Run `/flow:status` from a directory that resolves to a valid target repo (or against this repo itself — same-repo case) → expect the five-line status block from Story 1.7 (`crew v0.1.0`, `target repo:`, `adapter:`, `standards:`, `cycle:`).
...
- `plugins/flow/mcp-server/src/schemas/marketplace-manifest.ts` (NEW — Zod schema for `marketplace.json`)
- `plugins/flow/mcp-server/tests/install-contract.test.ts` (NEW — install-contract acceptance gate)
...
- `/Users/jackmcintyre/projects/crew/plugins/flow/.claude-plugin/plugin.json` (UPDATE — `skills: []` → `skills: ["skills/status.md"]`; no other field changes)
- `/Users/jackmcintyre/projects/crew/plugins/flow/docs/README-install.md` (UPDATE — checkpoint 3 only; checkpoints 1, 2, 4, 5, 6 unchanged; forward-reference line unchanged)

### Files NOT to modify (read-only context — touching them is a regression)

- `plugins/flow/mcp-server/src/tools/get-status.ts` — Story 1.7. `renderStatus` and `getStatus` are frozen.
- `plugins/flow/mcp-server/src/schemas/status-report.ts` — Story 1.7. `StatusReportSchema` is frozen.
- `plugins/flow/skills/status.md` — Story 1.7. Skill body is frozen.
- `plugins/flow/mcp-server/tests/get-status.test.ts` — Story 1.7. **Must remain bit-identical to `main` at end of this story** (AC4d).
- `plugins/flow/mcp-server/src/server.ts`, `src/index.ts`, `src/tools/register.ts` — Stories 1.1, 1.4, 1.7. No changes.
- `plugins/flow/mcp-server/src/lib/plugin-version.ts` — Story 1.1.
- `plugins/flow/mcp-server/src/state/*.ts` — Stories 1.2, 1.2b, 1.3.
- `plugins/flow/permissions/`, `plugins/flow/catalogue/`, `plugins/flow/example/` — out of scope.
...
- `plugins[0].name === "crew"` — matches AC4a clause 4 and the `/plugin install flow@flow` reference in the README.
- `plugins[0].source === "./plugins/flow"` — matches AC4a clause 5 and resolves on disk to `<repo-root>/plugins/flow/.claude-plugin/plugin.json` (exists today) — AC4a clause 6 passes.

#### `plugins/flow/.claude-plugin/plugin.json` (after Task 2)
...
**Self-consistency check:**
- `skills` contains `skills/status.md` — AC4b passes (the only `*.md` file under `plugins/flow/skills/` today is `status.md`; `.gitkeep` is excluded per AC3 clause 2).
...
3. **Load the plugin into Claude Code.**
   ...
   (The exact version comes from `plugins/flow/.claude-plugin/plugin.json`; `<semver>` matches `^\d+\.\d+\.\d+(?:-[\w.]+)?$`.)
```

**Self-consistency check:**
- Contains literal `/plugin marketplace add .` — AC4c clause 1 passes.
- Contains literal `/plugin install flow@flow` — AC4c clause 2 passes.
- Does NOT contain `/plugin install plugins/flow` — AC4c clause 3 passes (negative assertion).
...
2. **`/plugin install flow@flow`** — exact literal in README checkpoint 3b; asserted by AC4c clause 2. The `flow@flow` notation is `<plugin-name>@<marketplace-name>` where both happen to be `crew` (plugin name from `plugin.json`, marketplace name from `marketplace.json.name`).
3. **`/plugin install plugins/flow`** — the obsolete bug-bait literal that MUST NOT appear in the file (AC4c clause 3 — negative assertion).
4. **`crew` (plugin name)** — `plugin.json.name === "crew"`, `marketplace.json.plugins[0].name === "crew"`. Both files agree.
5. **`./plugins/flow`** — `marketplace.json.plugins[0].source`. Resolves on disk to `<repo-root>/plugins/flow/.claude-plugin/plugin.json`. AC4a clause 6.
6. **`skills/status.md`** — `plugin.json.skills[0]`. The single file currently under `plugins/flow/skills/` (the `.gitkeep` is filtered by AC3 clause 2).
...
- Story 1.7 shipped `getStatus`, `renderStatus`, `StatusReportSchema`, `/flow:status` skill, `README-install.md`, root README + plugin README rewrites, and `get-status.test.ts`. All of those are frozen by this story except checkpoint 3 of `README-install.md`.
