...
- **Build/verify:** `pnpm --dir plugins/flow/mcp-server test` + `build` + `knip`. Tests deterministic via a stub enricher; no live network in tests.
...
- [Source: plugins/flow/mcp-server/src/adapters/adapter.ts L14-46] — the `PlanningAdapter` interface (`listSourceStories`/`readSourceStory`) the ingest reads.
- [Source: plugins/flow/mcp-server/src/tools/write-native-story.ts L110-180] — the native render → gate → round-trip → atomic-write internals to reuse (skipping the active-adapter guard).
- [Source: plugins/flow/mcp-server/src/adapters/registry.ts L82-121] — adapter resolution (bmad active at ingest time).
