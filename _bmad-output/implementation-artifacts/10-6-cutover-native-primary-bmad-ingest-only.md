...
vitest: plugins/flow/mcp-server/src/tools/__tests__/read-backlog-inventory.integration.test.ts

**AC3 — the cutover is reversible up to the flip (unit):**

Both adapters remain registered and coexist; native is additive until the flip. Flipping `.flow/config.yaml` back to `adapter: bmad` restores BMad as the active adapter and the BMad backlog as live. The BMad parser remains available as an ingest on-ramp after cutover (it is demoted, not removed).

vitest: plugins/flow/mcp-server/tests/workspace-resolver.test.ts
...
artifact: plugins/flow/docs/native-cutover-runbook.md

## Definition of Done

- [ ] All four ACs met.
- [ ] `pnpm --dir plugins/flow/mcp-server test` green — full suite.
- [ ] `pnpm --dir plugins/flow/mcp-server build` green; `dist/` rebuilt.
- [ ] `pnpm knip` green.
- [ ] PR opens against `main`. CI green.
- [ ] Reviewer cycle clean.
- [ ] The actual repo cutover (flipping crew's own `.flow/config.yaml`) is performed only when the operator is satisfied the backlog is reconciled — the runbook gates it; the code change (and tests) can merge ahead of the live flip.
...
- **Config flip:** `.flow/config.yaml` `adapter:` field (workspace-config.ts schema). `resolveWorkspace` (workspace-resolver.ts L68-203) + `getActiveAdapter` (registry.ts L82-121) resolve it — no new selection code.
- **Board:** `getBacklogDashboard` → `readBacklogInventory` (render-backlog-dashboard.ts L95-113, read-backlog-inventory.ts) already reads `.flow/state/**` + `.flow/native-stories/` uniformly; verify native rendering, don't rebuild.
- **Claim:** `claimNextStory` → `listClaimableTodos` (claim-next-story.ts L63-126) reads `.flow/state/to-do/` and applies the readiness brake (9.1); ref format is immaterial. Verify a native ready story claims.
...
- **Detection ambiguity:** with both `.flow/native-stories/` and a BMad backlog present, `detect()` could match both — that is exactly why the cutover uses an explicit `adapter: native` config (Branch A), not detection. Pin this.
...
- **The repo's own cutover is an operator act, not a test:** the test proves the mechanism on a fixture; flipping crew's live `.flow/config.yaml` is done deliberately per the runbook.
...
- **Build/verify:** `pnpm --dir plugins/flow/mcp-server test` + `build` + `knip`. Mechanism tests use temp fixtures.
...
- [Source: plugins/flow/mcp-server/src/state/workspace-resolver.ts L68-203, src/adapters/registry.ts L82-121] — adapter resolution the flip drives (no new code).
- [Source: plugins/flow/mcp-server/src/tools/render-backlog-dashboard.ts L95-113, read-backlog-inventory.ts] — the adapter-agnostic board data path.
- [Source: plugins/flow/mcp-server/src/tools/claim-next-story.ts L63-126] — the adapter-agnostic claim path + readiness brake.
