...
I want **all 10 v1 catalogue role templates shipped at `plugins/flow/catalogue/<role>.md` in a single canonical shape**,
...
- (e) Implement the hiring conversation, the `/flow:hire` skill, or any UI for proposing/approving roles (Stories 2.4–2.5).
- (f) Add `/flow:team` or `/flow:ask` skill files (Stories 2.6–2.7).
...
- (i) Add or modify any skill (`plugins/flow/skills/*.md`) — this story does not surface a slash command. The catalogue is read by Story 2.3+ tooling, not invoked directly by the operator.
...
>
> **User-surface judgement.** None of these ACs is `user-surface` per `plugins/flow/docs/user-surface-acs.md`. The catalogue files live under `plugins/flow/catalogue/` and are consumed by future MCP tools (Story 2.3) and the hiring-manager agent (Story 2.4) — the v1 operator (Maya) never types a path that names a catalogue file, never observes a Claude Code UI element produced by this story, and the install README does not instruct her to open or copy any catalogue file by name. The schema, the section headers, and the domain-uniqueness check are internal correctness gates, not surfaces. ACs that *do* surface the catalogue to the operator (e.g. `/flow:hire` listing the roster, `/flow:team` printing domains) live in Stories 2.4 and 2.6 and will carry the `user-surface` tag there.

**AC1:**
**Given** the catalogue directory `plugins/flow/catalogue/`,
**When** I list it,
...
<!-- user-surface: AC names a directory under plugins/flow/ but the operator never lists it; the README does not instruct anyone to copy or open these files by name. Rubric (iii) does not apply. Not user-surface. -->

**AC2:**
**Given** any catalogue file in `plugins/flow/catalogue/`,
**When** it is parsed against `CatalogueRoleSchema` (new file: `plugins/flow/mcp-server/src/schemas/catalogue.ts`),
...
**AC3:**
**Given** the `domain:` field of every catalogue file in `plugins/flow/catalogue/`,
...
- [ ] **Task 1 — Author the `CatalogueRoleSchema` Zod schema (AC: 2, 4)**
  - [ ] 1.1 Create `plugins/flow/mcp-server/src/schemas/catalogue.ts`. Mirror the conventions in the existing `role-permissions.ts`: named export only, `.strict()` at every object level, kebab-case `role` regex, JSDoc header naming the FR refs (FR82, FR83, FR98, FR99) and citing architecture §3.
  ...
  - [ ] 1.4 Add `CatalogueShapeError` to `plugins/flow/mcp-server/src/errors.ts` following the existing `extends DomainError` pattern in that file (mirror `StoryNotFoundError` or equivalent — check `errors.ts` first; do NOT reinvent). The `code` string is `CATALOGUE_SHAPE_ERROR`.
  - [ ] 1.5 No new top-level dependencies. Use `yaml` (eemeli) which is already in `plugins/flow/mcp-server/package.json` for frontmatter parsing (split on `^---$`, parse the YAML head, treat the rest as body). Match the parsing approach already used elsewhere — check `markdown-frontmatter.ts` in `lib/` (per architecture §6) and reuse it if present; if not present, the helper goes into `lib/markdown-frontmatter.ts` as a new file with named exports.

- [ ] **Task 2 — Author the 10 catalogue role files (AC: 1, 2, 3)**
  - [ ] 2.1 Create each file at `plugins/flow/catalogue/<role>.md` using the canonical skeleton from architecture/implementation-patterns-consistency-rules.md §3 — frontmatter block, then `## Domain`, `## Mandate`, `## Out of mandate`, `## Prompt`. **Do not add `# Knowledge`** — that is persona-only.
  ...
  - [ ] 2.6 Existing `plugins/flow/catalogue/.gitkeep` — leave in place OR remove if the directory now has ten tracked files. Either is acceptable; do not block on it.

- [ ] **Task 3 — Author the vitest harness (AC: 1, 2, 3, 4)**
  - [ ] 3.1 Add `plugins/flow/mcp-server/tests/catalogue-shape.test.ts`. New file. Pattern after existing tests like `permissions-enforcement.test.ts` (parses YAML against a Zod schema) and `dist-shipping.test.ts` (file-set assertions against an explicit allowlist).
  - [ ] 3.2 The test reads `plugins/flow/catalogue/` from disk. Resolve the path from `import.meta.url` or repo root — match the helper used in `dist-shipping.test.ts` / `ci-drift-check.test.ts`. Do NOT invent a new file-locating helper.
  ...
  - [ ] 3.8 Run `pnpm --dir plugins/flow test`. Confirm the new test passes, the existing suite still passes (zero new failures, zero new skips, zero new warnings).

- [ ] **Task 4 — Build & dist verification (AC: 4)**
  - [ ] 4.1 Run `pnpm --dir plugins/flow/mcp-server build`. Schema additions must compile; `tsc` errors fail the task.
  - [ ] 4.2 Per `plugins/flow/docs/README-install.md` § Build artefacts and Story 1.9's contract, the built `mcp-server/dist/` tree is committed. Regenerate `dist/` after Task 4.1 and stage it in the same commit. The `ci-drift-check.test.ts` enforces src-vs-dist alignment — confirm it still passes.
...
- [ ] **Task 5 — Verify no other story's contract drifted (AC: 1–4)**
  - [ ] 5.1 Open the existing `plugins/flow/mcp-server/src/schemas/role-permissions.ts` and confirm the kebab-case `role` regex `^[a-z0-9-]+$` matches every catalogue role authored in Task 2. The two schemas share a `role` identifier convention; if they diverge, Story 2.2 breaks.
  - [ ] 5.2 Skim the existing `plugins/flow/permissions/` directory. It is currently empty (Story 2.2 hasn't shipped). This story does NOT pre-populate it — but if it is already non-empty (someone landed Story 2.2 out of order), confirm every per-role YAML there names a role that exists in this story's catalogue. If a permission file references a role this story does not ship, raise — do NOT silently delete the permission file.
  - [ ] 5.3 No other tree is touched. Specifically: do NOT edit `plugins/flow/skills/*`, `plugins/flow/docs/README-install.md`, `plugins/flow/docs/standards-example.md`, root `README.md`, or any MCP tool file. Catalogue authorship is leaf work.
...
- **Story 1.1** scaffolded `plugins/flow/` including `catalogue/.gitkeep` and `mcp-server/src/schemas/`. The skeleton exists; this story drops content into it.
...
- **Story 1.4** also established `plugins/flow/mcp-server/src/errors.ts` with a `DomainError` base class. Task 1.4's `CatalogueShapeError` extends that base and follows the same code-string convention as existing errors in that file.
...
- **Story 1.7 / 1.7a** wired `/flow:status`. The status surface does NOT yet read the catalogue; that wiring may be picked up in Story 2.6 (`/flow:team`). Out of scope here.
...
- `_bmad-output/planning-artifacts/architecture/project-structure-boundaries.md` (plugin tree) — confirms `plugins/flow/catalogue/<role>.md` paths and the "catalogue is read-only at runtime" boundary.
...
- `plugins/flow/mcp-server/src/schemas/catalogue.ts` — `CatalogueRoleSchema` + `assertCatalogueBodySections` + `CatalogueRole` type.
- `plugins/flow/mcp-server/src/lib/markdown-frontmatter.ts` — **only if it does not already exist.** Check first; if present, reuse.
- `plugins/flow/mcp-server/tests/catalogue-shape.test.ts` — vitest harness.
- `plugins/flow/catalogue/hiring-manager.md`
- `plugins/flow/catalogue/planner.md`
- `plugins/flow/catalogue/generalist-dev.md`
- `plugins/flow/catalogue/generalist-reviewer.md`
- `plugins/flow/catalogue/retro-analyst.md`
- `plugins/flow/catalogue/orchestrator.md`
- `plugins/flow/catalogue/security-specialist.md`
- `plugins/flow/catalogue/test-specialist.md`
- `plugins/flow/catalogue/docs-specialist.md`
- `plugins/flow/catalogue/debugger.md`

### Files this story MAY modify (UPDATE, with care)

- `plugins/flow/mcp-server/src/errors.ts` — add `CatalogueShapeError extends DomainError` only. Do not touch other error classes; their `code` strings are pinned by Story 1.4.
- `plugins/flow/mcp-server/dist/**` — regenerated by `pnpm build`. Stage in the same commit (Task 4.2). Do not hand-edit.
- `plugins/flow/catalogue/.gitkeep` — may delete once the directory has tracked files. Not required.
...
- `plugins/flow/docs/README-install.md` — operator does not see catalogue files during install.
- `plugins/flow/permissions/*` — Story 2.2.
...
- **Test placement:** `plugins/flow/mcp-server/tests/<name>.test.ts` — matches every other test in the suite (no per-source co-location for these higher-level checks).
- **Run command:** `pnpm --dir plugins/flow test` — runs all suites. Single-file: `pnpm --dir plugins/flow/mcp-server exec vitest run tests/catalogue-shape.test.ts`.
...
- Catalogue lives under `plugins/flow/catalogue/` (architecture §"Plugin tree" line 18–28). The schema lives under `plugins/flow/mcp-server/src/schemas/catalogue.ts` (§"Plugin tree" lines 87–95).
...
- [Source: `plugins/flow/mcp-server/src/schemas/role-permissions.ts`] — sibling-schema pattern for `.strict()`, JSDoc, kebab-case role regex.
...
- 2026-05-20 rework pass (review verdict: request-changes):
  ...
  - Verification: `pnpm --dir plugins/flow install && pnpm --dir plugins/flow build && pnpm --dir plugins/flow test` — all 19 suites, 181 tests passing.
