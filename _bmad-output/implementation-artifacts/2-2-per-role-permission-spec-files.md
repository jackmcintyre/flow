...
I want **a `plugins/flow/permissions/<role>.yaml` file for each of the ten catalogue roles plus a placeholder `plugins/flow/permissions/gh-error-map.yaml`**,
...
- (b) Modify `plugins/flow/catalogue/*.md`. The catalogue is the source of truth (Story 2.1). If a discrepancy is found, fix `permissions/<role>.yaml`, not the catalogue. The one exception is documenting (in a comment header) the catalogue version each YAML was derived from — and even that is optional, see Task 3.5.
- (c) Modify `plugins/flow/mcp-server/src/schemas/role-permissions.ts`. The shipped schema is sufficient. Do not add fields, do not loosen `.strict()`, do not change the `role` regex.
- (d) Modify `plugins/flow/mcp-server/src/state/load-role-permissions.ts` or `plugins/flow/mcp-server/src/lib/gh.ts`. The dispatcher and gh-wrapper are correct as shipped in Story 1.4; this story only changes the data they load.
...
- (g) Add or modify any slash command (`plugins/flow/skills/*.md`) — this story does not surface a slash command. Permissions are read by the MCP dispatcher, not invoked by the operator.
- (h) Touch `plugins/flow/docs/README-install.md` — the install-path operator (Maya) never opens, copies, or sees a permissions file during install. The dispatcher reads them transparently.
...
**When** I list `plugins/flow/permissions/`,
...
<!-- user-surface: AC names a directory under plugins/flow/ but the operator never lists it; the README does not instruct anyone to open these files by name. Rubric (iii) does not apply. Not user-surface. -->

**AC2:**
**Given** any of the ten per-role permission specs in `plugins/flow/permissions/`,
**When** it is parsed against `RolePermissionsSchema` (shipped in Story 1.4 at `plugins/flow/mcp-server/src/schemas/role-permissions.ts`),
...
- [ ] **Task 2 — Author the eleven YAML files (AC: 1, 2, 3, 4)**
  - [ ] 2.1 Per-role values — pinned below. **`gh_allow` lists are copied verbatim from the matching `plugins/flow/catalogue/<role>.md` frontmatter** (verified against Story 2.1 as merged). Do not paraphrase, do not reorder (set equality is what AC4(c) asserts; order does not matter, but minimising diff in code review does).
  ...
  - [ ] 2.2 File template (every per-role YAML follows this exact shape; the example below is for `planner`):
    ...
    gh_allow_args: {}
    ```
    No frontmatter, no top-level comments other than the optional catalogue-version header (Task 3.5). One file per role under `plugins/flow/permissions/<role>.yaml`.
  - [ ] 2.3 Create `plugins/flow/permissions/hiring-manager.yaml` with `tools_allow: [heartbeat]`, `gh_allow: []`, `gh_allow_args: {}`.
  - [ ] 2.4 Create `plugins/flow/permissions/planner.yaml` with the values from the table.
  - [ ] 2.5 **Update** `plugins/flow/permissions/generalist-dev.yaml` if and only if its current contents do not match the table above. The current file (as shipped in Story 1.4) has `tools_allow: [claimStory, completeStory, blockStory, readSourceStory, lookupStandards, recordYield, heartbeat, classifyRiskTier]` and `gh_allow: [pr-create, pr-view, pr-comment, pr-checks, pr-edit]`. The catalogue pins `gh_allow: [pr-create, pr-view, pr-comment]` — so **remove `pr-checks` and `pr-edit`** from `gh_allow` to achieve catalogue parity (AC4(c)). Leave `tools_allow` as-is (it matches the table). After this edit, run `pnpm --dir plugins/flow test` once — note any `permissions-enforcement.test.ts` expectations that broke (Task 4 will reconcile).
  - [ ] 2.6 **Update** `plugins/flow/permissions/generalist-reviewer.yaml`. The current file has `gh_allow: [pr-view, pr-comment, pr-checks, api]`. The catalogue pins `gh_allow: [pr-view, pr-comment, pr-review]`. Replace with the catalogue values exactly: **add `pr-review`, remove `pr-checks` and `api`**. Leave `tools_allow` as-is (it matches the table). The negative-capability AC3 (no `pr-merge`, no `pr-close`, no `push`) is satisfied by this list — neither `pr-merge` nor `pr-close` nor any push-bearing subcommand appears.
  - [ ] 2.7 Create `plugins/flow/permissions/retro-analyst.yaml` with the values from the table.
  - [ ] 2.8 Create `plugins/flow/permissions/orchestrator.yaml` with the values from the table.
  - [ ] 2.9 Create `plugins/flow/permissions/security-specialist.yaml`, `plugins/flow/permissions/test-specialist.yaml`, `plugins/flow/permissions/docs-specialist.yaml`, and `plugins/flow/permissions/debugger.yaml` with the values from the table.
  ...
  - [ ] 2.11 Create `plugins/flow/permissions/gh-error-map.yaml`. v1 ships an **empty skeleton**:
    ```yaml
  ...
  - [ ] 2.12 Delete `plugins/flow/permissions/.gitkeep` if present (the directory now has eleven tracked files; the placeholder is no longer needed). If absent, no action.

- [ ] **Task 3 — Author the new parity test (AC: 1, 2, 3, 4)**
  - [ ] 3.1 Add `plugins/flow/mcp-server/tests/permissions-catalogue-parity.test.ts`. New file. Pattern after `catalogue-shape.test.ts` (Story 2.1) for the catalogue-discovery side and after `permissions-enforcement.test.ts` § "shipped role specs (AC5e)" for the permissions-loading side.
  - [ ] 3.2 Resolve `plugins/flow/catalogue/` and `plugins/flow/permissions/` from `import.meta.url` — match the helper used in `catalogue-shape.test.ts` and `dist-shipping.test.ts`. Do NOT invent a new path-resolving helper.
  ...
  - [ ] 3.6 **Catalogue-permissions `gh_allow` parity (AC4(c)).** For each role, parse the catalogue file's YAML frontmatter (reuse the `splitFrontmatter` helper from `plugins/flow/mcp-server/src/lib/markdown-frontmatter.ts` shipped in Story 2.1), extract `gh_allow`, and compare to the permission YAML's `gh_allow` as **set equality** (sort both arrays before comparing, or convert both to `Set` and compare sizes + membership). Diagnostic on mismatch shows both lists and the symmetric difference.
  ...
  - [ ] 3.8 **gh-error-map exists and parses (AC4(e)).** Read `plugins/flow/permissions/gh-error-map.yaml`, parse with the `yaml` library, assert the result is an object containing an `entries` key with an array value (length 0 is fine for v1). Diagnostic on parse failure names the file path.
  - [ ] 3.9 The test file header MUST cite this story (`Story 2.2 AC1–AC4`) and reference both `plugins/flow/docs/user-surface-acs.md` and the Story 2.1 catalogue test, mirroring the comment-header convention used in `catalogue-shape.test.ts` and `pre-pr-gate.test.ts`.
  - [ ] 3.10 Run `pnpm --dir plugins/flow test`. Confirm the new test passes alongside the existing suite. Expected: 18→19 suites (one new file), all tests green.

- [ ] **Task 4 — Reconcile the existing `permissions-enforcement.test.ts` (AC: 3)**
  - [ ] 4.1 Open `plugins/flow/mcp-server/tests/permissions-enforcement.test.ts`. The "shipped role specs (AC5e)" describe block at lines 184–209 currently has two tests:
  ...
  - [ ] 4.2 The two `describe("gh wrapper enforcement", …)` tests at lines 123–182 use `test-role` (a fixture). Do NOT touch the fixture at `plugins/flow/mcp-server/tests/fixtures/permissions/test-role.yaml`. Do NOT touch the fixture-driven assertions.
  - [ ] 4.3 Run `pnpm --dir plugins/flow test`. Confirm `permissions-enforcement.test.ts` passes with the rewritten reviewer test and no other test regressed.

- [ ] **Task 5 — Build & dist verification (AC: 4)**
  - [ ] 5.1 Run `pnpm --dir plugins/flow/mcp-server build`. The new test file references `loadRolePermissions` (existing) and may add a parity helper inline (no new source code under `src/`). `tsc` must compile cleanly.
  - [ ] 5.2 Per `plugins/flow/docs/README-install.md` § Build artefacts and Story 1.9's contract, the built `mcp-server/dist/` tree is committed. **This story adds no new source files under `src/`** — it adds tests under `tests/` and YAML data under `permissions/`. Neither `tests/` nor `permissions/` is in the `dist/` build output, so `dist/` should be unchanged. If `pnpm build` produces any `dist/` diff, investigate (likely cause: an inadvertent `src/` edit). The `ci-drift-check.test.ts` enforces src-vs-dist alignment — confirm it still passes.
...
- [ ] **Task 6 — Verify no other story's contract drifted (AC: 1–4)**
  - [ ] 6.1 Open `plugins/flow/catalogue/<role>.md` for each of the ten roles. Confirm the `gh_allow:` list pinned in Task 2.1's table is byte-for-byte what the catalogue ships. If it has changed since this spec was written, **update the YAML to match the catalogue, not the other way around** — the catalogue (Story 2.1) is the merged source of truth.
  - [ ] 6.2 Confirm `plugins/flow/mcp-server/src/schemas/role-permissions.ts` is unchanged. The shipped schema (`role` regex `^[a-z0-9-]+$`, `tools_allow.min(1)`, `.strict()` at every level) is sufficient.
  - [ ] 6.3 Confirm `plugins/flow/mcp-server/src/state/load-role-permissions.ts` is unchanged. The loader's behaviour (ENOENT → `RolePermissionsMissingError`, YAML/Zod failure → `RolePermissionsMalformedError`) is unchanged.
  - [ ] 6.4 No other tree is touched. Specifically: do NOT edit `plugins/flow/catalogue/*.md`, `plugins/flow/skills/*`, `plugins/flow/docs/*`, root `README.md`, or any MCP tool/server file. Permission YAML authorship and a single test rewrite are the only changes.
...
- **Story 1.1** scaffolded `plugins/flow/permissions/` (with `.gitkeep`) and the schemas directory under `mcp-server/src/`.
- **Story 1.4** is the load-bearing predecessor. It shipped:
  - `RolePermissionsSchema` at `plugins/flow/mcp-server/src/schemas/role-permissions.ts` — `.strict()`, kebab-case `role` regex, `tools_allow.min(1)`, `gh_allow` defaults to `[]`, `gh_allow_args` is a `record(string, string[])`.
  - `loadRolePermissions` at `plugins/flow/mcp-server/src/state/load-role-permissions.ts` — single-purpose IO wrapper, no caching, `pluginRoot` flows in as a parameter.
  - The dispatcher (`plugins/flow/mcp-server/src/server.ts`) refusal pattern `Role 'X' is not allowed to invoke tool 'Y'. (FR79/FR80/NFR12). See permissions/X.yaml.`.
  - The `gh` wrapper (`plugins/flow/mcp-server/src/lib/gh.ts`) refusal pattern `Role 'X' is not allowed to invoke 'gh Y'. (NFR17). See permissions/X.yaml.`.
...
- **Story 1.7** added `/flow:status`. Status does not read permissions; out of scope.
...
- **Story 2.1** shipped the ten catalogue files at `plugins/flow/catalogue/<role>.md` with the canonical `gh_allow` lists this story mirrors into `permissions/<role>.yaml`. **It also shipped `plugins/flow/mcp-server/src/lib/markdown-frontmatter.ts`** with a `splitFrontmatter` helper that Task 3.6 reuses for parsing catalogue frontmatter in the parity test.
...
- `_bmad-output/planning-artifacts/architecture/project-structure-boundaries.md` line 45 — confirms `plugins/flow/permissions/gh-error-map.yaml` is part of the shipped plugin tree.
...
- `plugins/flow/permissions/hiring-manager.yaml`
- `plugins/flow/permissions/planner.yaml`
- `plugins/flow/permissions/retro-analyst.yaml`
- `plugins/flow/permissions/orchestrator.yaml`
- `plugins/flow/permissions/security-specialist.yaml`
- `plugins/flow/permissions/test-specialist.yaml`
- `plugins/flow/permissions/docs-specialist.yaml`
- `plugins/flow/permissions/debugger.yaml`
- `plugins/flow/permissions/gh-error-map.yaml` (empty `entries: []` skeleton; Story 4.5 will populate)
- `plugins/flow/mcp-server/tests/permissions-catalogue-parity.test.ts`

### Files this story MAY modify (UPDATE, with care)

- `plugins/flow/permissions/generalist-dev.yaml` — remove `pr-checks` and `pr-edit` from `gh_allow` to achieve catalogue parity (Task 2.5). Leave `tools_allow` and `gh_allow_args` unchanged.
- `plugins/flow/permissions/generalist-reviewer.yaml` — set `gh_allow` to `[pr-view, pr-comment, pr-review]` exactly: add `pr-review`, remove `pr-checks`, remove `api` (Task 2.6). Leave `tools_allow` and `gh_allow_args` unchanged.
- `plugins/flow/mcp-server/tests/permissions-enforcement.test.ts` — rewrite one assertion in the "shipped role specs (AC5e)" reviewer test (Task 4.1). Do NOT modify any other test in this file. Do NOT modify the fixture-driven gh-wrapper tests at lines 123–182.
- `plugins/flow/permissions/.gitkeep` — delete if present, since the directory now has eleven tracked files. Optional.

### Files this story MUST NOT modify

- `_bmad-output/implementation-artifacts/sprint-status.yaml` (orchestrator owns).
- `plugins/flow/catalogue/*.md` — Story 2.1 is the source of truth for `gh_allow`. If the catalogue and this spec disagree, fix the YAML to match the catalogue, not the other way around.
- `plugins/flow/mcp-server/src/schemas/role-permissions.ts` — shipped schema is sufficient.
- `plugins/flow/mcp-server/src/state/load-role-permissions.ts` — loader behaviour is correct.
- `plugins/flow/mcp-server/src/lib/gh.ts` — gh-wrapper behaviour is correct.
- `plugins/flow/mcp-server/src/server.ts` — dispatcher behaviour is correct.
- `plugins/flow/mcp-server/tests/fixtures/permissions/test-role.yaml` — fixture is referenced by `permissions-enforcement.test.ts` line 4–6 and by `gh` wrapper tests; do not change it.
- `plugins/flow/docs/README-install.md` — operator does not see permission files during install.
- `plugins/flow/skills/*` — no slash command surface here.
- `plugins/flow/mcp-server/dist/**` — no source changes, so no rebuild expected.
- Root `README.md`.

### Pinned `gh_allow` values from the merged catalogue (source: `plugins/flow/catalogue/<role>.md` as of Story 2.1)
...
- **Test placement:** `plugins/flow/mcp-server/tests/<name>.test.ts` — matches every other test in the suite.
- **Run command:** `pnpm --dir plugins/flow test` — runs all suites. Single-file: `pnpm --dir plugins/flow/mcp-server exec vitest run tests/permissions-catalogue-parity.test.ts`.
...
- Permissions live under `plugins/flow/permissions/` (architecture §"Plugin tree" line 44–45). The dispatcher and gh-wrapper that consume them live under `plugins/flow/mcp-server/src/`.
...
- [Source: `plugins/flow/mcp-server/src/schemas/role-permissions.ts`] — the schema this story's YAMLs validate against.
- [Source: `plugins/flow/mcp-server/src/state/load-role-permissions.ts`] — the loader this story's parity test invokes.
- [Source: `plugins/flow/mcp-server/tests/permissions-enforcement.test.ts`] — the existing test whose reviewer assertion this story rewrites.
- [Source: `plugins/flow/mcp-server/tests/catalogue-shape.test.ts`] — the Story 2.1 sibling test whose patterns this story mirrors.
- [Source: `plugins/flow/catalogue/*.md`] — the ten catalogue files whose `gh_allow` lists this story mirrors.
