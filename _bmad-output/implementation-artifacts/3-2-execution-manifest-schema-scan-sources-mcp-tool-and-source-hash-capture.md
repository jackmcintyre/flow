...
I want **my planning-tool source stories projected into plugin-owned execution manifests under `<target-repo>/.flow/state/to-do/<ref>.yaml`, with source content fingerprinted on the way in**,
...
Add an execution-manifest Zod schema at `plugins/flow/mcp-server/src/schemas/execution-manifest.ts`, a `scan-sources` MCP tool that calls the active adapter's `listSourceStories()` and projects each `SourceStory` into a freshly-written `<target-repo>/.flow/state/to-do/<ref>.yaml` (idempotent on re-scan, hash-refresh on source-edit-in-to-do), a refuses-on-malformed reader for existing manifests, and a `skills/scan/SKILL.md` user-facing skill that invokes the tool so `/<plugin>:scan` works end-to-end.

### What this story fixes (and why it needs its own story)

After Story 3.1, the registry can resolve an adapter; after Story 3.3, the BMad adapter can list real `SourceStory` objects. But nothing yet projects those objects into the execution layer — `<target-repo>/.flow/state/to-do/` stays empty, and the dev loop has nothing to claim. The architecture pins the projection in two places (Architecture §Two-layer model, §Execution manifest) and the PRD pins it as FR9, FR13, and NFR10. This story is where the seam crosses: source-layer (read-only, owned by the planning tool) → execution-layer (plugin-owned, on-disk under `.flow/state/`).
...
3. **The `/<plugin>:scan` skill** — AC4 pins a user-surface skill at `plugins/flow/skills/scan/SKILL.md` whose behaviour is "invoke `scan-sources` and print the result". Without the skill, the tool is unreachable from the Claude Code TUI.
...
- (f) Move manifests between states. State transitions (`to-do` → `in-progress`, etc.) are owned by the atomic-rename primitive at `plugins/flow/mcp-server/src/state/manifest-state-machine.ts` (Story 1.6) and by the orchestrator. `scan-sources` only ever writes into `to-do/` (creating that directory if absent); it does not call `moveBetweenStates`.
...
- (j) Mutate `.flow/config.yaml`. The workspace resolver (Story 1.2) owns config writes.
...
- (m) Add new dependencies to `plugins/flow/mcp-server/package.json`. `node:crypto` (sha256 hashing) is already used by `parse-bmad-story.ts` for `source_hash`; the manifest writer just persists that hash verbatim. `yaml` is already a dep (used by the workspace resolver).
- (n) Re-author the existing skills under `plugins/flow/skills/` (`hire`, `skip-hiring`, `status`, `team`, `ask`). The new `scan/` skill is additive.
- (o) Hand-edit `plugins/flow/mcp-server/dist/` — `pnpm --dir plugins/flow build` regenerates it; the dist-shipping contract from Story 1.9 still applies.
- (p) Modify `README.md`, `plugins/flow/README.md`, or `plugins/flow/docs/README-install.md` beyond a single bullet under the "Available skills" list pointing at `/<plugin>:scan`. The install-walkthrough surface is not changed.

---

## Acceptance Criteria

> **Verbatim from epic** (numeric prefixes added for the tag-extraction regex; epic prose preserved). Seven ACs total (five from the epic plus AC6 integration test and AC7 deterministic content-structure anchor). **AC4** names a slash command literal (`/<plugin>:scan`) and the file path `skills/scan.md` that the README's "Available skills" list will reference — both rubric (i) and (iii) per `plugins/flow/docs/user-surface-acs.md` — so AC4 is tagged `(user-surface)`. The other ACs govern internal schema, idempotency semantics, hash-capture behaviour, and a typed-refusal error, none of which the operator types or observes directly. Per the rubric they are untagged. **AC7** is a deterministic content-structure check added per the user-surface-AC requirement (story has a `user-surface` shape, so at least one AC must be a structural anchor that reads a file and asserts a substring); it pins the exact frontmatter line and one body substring of `skills/scan/SKILL.md`.

**AC1:**
**Given** a target repo with an active adapter that returns source stories,
**When** the `scan-sources` MCP tool is invoked,
**Then** each new ref produces an execution manifest at `<target-repo>/.flow/state/to-do/<ref>.yaml` with the fields `ref`, `status: to-do`, `adapter`, `source_path`, `source_hash` (sha256 of source contents), and `depends_on` carried verbatim from the source. _(FR9 via execution-manifest layer; Architecture §Source-drift handling)_
<!-- Not user-surface: AC1 governs the on-disk shape of files under `<target-repo>/.flow/state/to-do/` that the operator does not open by name (the README references the *parent* directory at install-checkpoint time, not individual manifest paths). The tool name `scan-sources` is the MCP tool identifier, not a CLI literal the operator types. -->
...
<!-- User-surface: rubric (i) — slash command literal `/<plugin>:scan` (concretely `/flow:scan` for the v1 plugin); rubric (iii) — the README "Available skills" bullet will name `skills/scan/SKILL.md` as the file the user can browse; rubric (iv) — the slash-command picker in Claude Code will list `/flow:scan` after install. (The epic text says `skills/scan.md`; the v1 skills layout uses `skills/<name>/SKILL.md` per Story 2.4's precedent, so the actual file lives at `plugins/flow/skills/scan/SKILL.md`. Document the path correction in Task 6.) -->
...
**Given** the file `plugins/flow/skills/scan/SKILL.md`,
**When** its contents are read,
**Then** the first non-blank lines include a YAML frontmatter block whose `name:` field equals `flow:scan` (i.e. the file contains the substring `name: crew:scan` on a frontmatter line), and the body contains the substring `scan-sources` (the MCP tool name the skill invokes). This is verified by a vitest assertion in Task 8 that reads the file off disk and asserts both substrings — no LLM judgment.
...
The user-facing skill at `plugins/flow/skills/scan/SKILL.md` is the only LLM-driven surface this story ships. The MCP tool itself is deterministic — no prompt-level invariants apply. Below are the prompt-level invariants the SKILL.md MUST encode so behaviour is reproducible across Claude Code sessions:

- **MUST** invoke the `scan-sources` MCP tool exactly once per `/flow:scan` invocation, with `targetRepoRoot` set to the workspace root the skill resolves at entry.
- **MUST** print the tool's text response verbatim, without paraphrase, summarisation, or omission of any field the tool emits.
- **MUST NOT** invoke any other MCP tool from this skill — not `getStatus`, not `getTeamSnapshot`, not any future tool. If the operator wants status, they run `/flow:status`. Composition belongs in higher-level skills (`/plan`, `/ship-story`), not in `/scan`.
...
- **MUST NOT** mutate `.flow/config.yaml`, `.flow/state/**`, or any manifest. Manifests are written by the tool, not the skill.
...
- [x] **Task 1 — Author the execution-manifest Zod schema (AC: 1, 3, 5)**
  - [x] 1.1 Create `plugins/flow/mcp-server/src/schemas/execution-manifest.ts`. The file does not exist today (`ls plugins/flow/mcp-server/src/schemas/` returns the eight schema files listed in the project-structure tree; this is a NEW file).
...
- [x] **Task 2 — Implement the `scan-sources` MCP tool (AC: 1, 2, 3, 5)**
  ...
  - [x] 2.3 Algorithm (executed in this order — each step is a separate code block in the implementation so the test can inspect intermediate state):
    ...
    4. Compute the four state-dir presence map: for each ref, check whether `<targetRepoRoot>/.flow/state/<state>/<ref>.yaml` exists for each `state` in `STATE_NAMES`. Done via `Promise.all` + `fs.stat` swallowing ENOENT. Result is a `Map<string, StateName | null>` keyed by ref.
    5. For each ref, branch:
       - **Not in any state dir → CREATE** (AC1 path). Compose the manifest object (Task 2.4), parse it through `ExecutionManifestSchema` (defensive — catches a coding mistake in the composer), serialise via `yaml.stringify(parsed)`, and write to `<targetRepoRoot>/.flow/state/to-do/<ref>.yaml` via the canonical write helper at `lib/managed-fs.ts` (`writeManagedFile`). Push to `createdRefs`.
  ...
  - [x] 2.7 Directory creation: `<targetRepoRoot>/.flow/state/to-do/` may not exist on first scan (this is the canonical "first scan ever" path). `writeManagedFile` already calls `mkdir { recursive: true }` for the parent directory of the target file — confirm by reading `lib/managed-fs.ts`. If it does not, prefix each write with `await fs.mkdir(path.dirname(absPath), { recursive: true })` and document the choice in Task 2.6's body.
...
- [x] **Task 3 — Add `MalformedExecutionManifestError` to `errors.ts` (AC: 5)**
  - [x] 3.1 Edit `plugins/flow/mcp-server/src/errors.ts`. Add a new `MalformedExecutionManifestError` class extending `DomainError`, positioned adjacent to `InvalidWorkspaceConfigError` for thematic grouping.
  ...
  - [x] 3.4 Export the class. Confirm `grep -n "export class MalformedExecutionManifestError" plugins/flow/mcp-server/src/errors.ts` returns exactly one hit.
...
- [x] **Task 4 — Register the tool in `tools/register.ts` (AC: 1, 4)**
  ...
  - [x] 4.6 Permission allowlist: the role(s) that will invoke `scanSources` are the planner subagents (Story 3.4) and the orchestrator. The permission spec files at `plugins/flow/catalogue/permissions/<role>.yaml` are owned by Story 2.2 and updated by individual role stories. For v1, the new tool is in the registry but no role's `tools_allow` lists it explicitly — the skill at `skills/scan/SKILL.md` will run without a `_meta.role` (matching `/flow:status`'s pattern), bypassing the role gate. **Document this in a one-line comment** so a future maintainer adding the planner role knows to extend its `tools_allow`.

- [x] **Task 5 — Test fixture target repo for vitest (AC: 6)**
  - [x] 5.1 Create `plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/` (NEW directory). Inside it, scaffold a minimal BMad-shaped fixture target repo:
    - `.flow/config.yaml` with `adapter: bmad`, `adapter_config.stories_root: _bmad-output/planning-artifacts/stories`, an empty `plugin: {}` block.
    ...
    - Use the same Markdown shape as the BMad fixtures committed under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` (Story 3.3). Pull two files verbatim from there if possible; otherwise hand-author two minimal files referencing the BMad spike doc.
  ...
  - [x] 5.3 Add a `.gitkeep` to `.flow/state/` so the directory exists in the fixture before the first scan creates `to-do/`. Rationale: vitest's `tmp` copies the fixture into a scratch dir per test; absent `.gitkeep`, the empty `state/` dir does not survive the copy on some filesystems. (Tasks 6.5 / 8.5 — the test copies the fixture into a temp dir to keep `.flow/state/` writable without polluting the committed tree.)
  - [x] 5.4 The fixture's `.flow/config.yaml` is already valid against `WorkspaceConfigSchema` and `BmadAdapter.adapterConfigSchema`. Confirm by writing a one-line script comment in the file: `# Fixture for tests/scan-sources.test.ts — do not hand-edit; regenerate from the BMad spike if the format changes.`

- [x] **Task 6 — Author the `/flow:scan` skill (AC: 4, 7)**
  - [x] 6.1 Create `plugins/flow/skills/scan/` (NEW directory). Inside it, create `SKILL.md` (NEW file). Mirror the structure of `plugins/flow/skills/status/SKILL.md` exactly — same frontmatter keys, same H1/H2 layout.
  - [x] 6.2 Frontmatter (the deterministic-content-anchor in AC7 pins these exact lines):
    ...
    ---
    ```
    - `name:` MUST equal `flow:scan` exactly. This is the slash-command literal that lands in the Claude Code TUI picker. AC4 (user-surface) and AC7 (deterministic anchor) both depend on this value; do not change the prefix to match a future plugin rename without updating both ACs.
    - `description:` is a one-line operator-facing blurb. The Claude Code picker truncates at ~80 chars; keep it short.
    - `allowed_tools: [Read]` mirrors `/flow:status`. The skill needs only file-read authority because the MCP tool does the heavy lifting; the skill does not directly touch the filesystem.
  - [x] 6.3 Body sections (Markdown H1/H2), matching the `/flow:status` template:
    - `# /flow:scan` (H1)
    ...
    - `# Steps`
    - `# Failure modes`
  - [x] 6.4 Body content — exact prose, matching `/flow:status`'s tone:
    - **What this skill does:** One sentence — "Projects your active planning tool's source stories into per-story execution manifests under `<target-repo>/.flow/state/to-do/<ref>.yaml`. Idempotent: re-running this skill after no source changes is a no-op."
    - **Prerequisites:** A target repo with `.flow/config.yaml` resolved (auto-detected on first run by the workspace resolver — see `docs/README-install.md` checkpoint 5). At least one source story present under the active adapter's stories root (e.g. one BMad story under `_bmad-output/planning-artifacts/stories/` if you're on the BMad adapter).
  ...
  - [x] 6.6 Note on the epic's path text: the epic AC4 says "I look at `skills/scan.md`". The v1 skills layout (per Story 2.4's precedent and `plugins/flow/skills/status/SKILL.md`) is `skills/<name>/SKILL.md`, not `skills/<name>.md`. Add a one-line HTML comment in `SKILL.md` (after the body, before EOF) noting: `<!-- Path note: the epic at epic-3-...md § Story 3.2 AC4 refers to "skills/scan.md"; the actual v1 layout is skills/scan/SKILL.md per the precedent set by skills/status/SKILL.md. The slash-command surface (/flow:scan) is unaffected. -->`. This is the audit trail for the path-shape correction.

- [x] **Task 7 — README "Available skills" bullet (AC: 4)**
  - [x] 7.1 Edit `plugins/flow/README.md`. Locate the "Available skills" list (created by Story 1.10 and amended by Story 2.6 — read it first to see the current shape). Add one bullet:
  ...
  - [x] 7.4 Do NOT edit `README.md` (repo-root) or `plugins/flow/docs/README-install.md`. Those are reserved for install-path changes; this story's surface is post-install.

- [x] **Task 8 — Vitest coverage (AC: 1, 2, 3, 5, 6, 7)**
  - [x] 8.1 Create `plugins/flow/mcp-server/tests/scan-sources.test.ts` (NEW). Style: vitest with `describe`/`it`/`expect`, NodeNext ESM imports (`.js` extensions), `os.tmpdir()` + `fs.mkdtemp` for per-test scratch dirs, `fs.cp` to copy the committed fixture from Task 5 into the scratch dir. The closest precedent is `plugins/flow/mcp-server/tests/bmad-adapter.test.ts` for the fixture-copy pattern.
  - [x] 8.2 Test scenarios — exactly these `it` blocks:
    - **`AC1 — first scan creates manifests for every source story`**: copy fixture; call `scanSources({ targetRepoRoot: scratch })`. Assert: `result.createdRefs` contains `bmad:1.1` and `bmad:1.2` (in adapter-list order); `result.updatedRefs.length === 0`; `result.unchangedRefs.length === 0`; the on-disk file `${scratch}/.flow/state/to-do/bmad:1.1.yaml` exists and parses through `parseExecutionManifest` with `status === "to-do"`, the expected `source_hash` (sha256 of the fixture file's bytes, computed in the test), `adapter === "bmad"`, `depends_on` matching the source.
  ...
  - [x] 8.3 Do NOT test the AC4 user-surface (the slash command picker showing `/flow:scan`) in vitest. AC4's user-surface verification is owned by `ship.py pre-pr-gate` (Story 1.8); the gate accepts either an automated-e2e event or an operator-pasted-output event. For this story, the operator route is expected: a verbatim paste of `/flow:scan` running against a real fixture, captured in the run log as a `user_surface_verified` event. **This is not a Task — it's a heads-up to the dev agent that the smoke harness in `ship-story` will require operator paste-verbatim for AC4 before PR.**
  - [x] 8.4 Test isolation: each `it` block creates its own scratch dir via `await fs.mkdtemp(path.join(os.tmpdir(), "flow-scan-"))` and cleans up via `afterEach` with `fs.rm({ recursive: true, force: true })`. Do not share scratch dirs across tests — flake risk on cleanup races.
  - [x] 8.5 Fixture copy: use `await fs.cp(fixtureDir, scratch, { recursive: true })` (Node 16.7+). The fixture's `.flow/state/.gitkeep` ensures the empty state dir survives the copy.
  - [x] 8.6 Run `pnpm --dir plugins/flow test` and confirm all new + existing suites pass. Confirm `bmad-adapter.test.ts` is unaffected (the scan test does not mutate the committed BMad adapter fixtures).

- [x] **Task 9 — Schema-export sweep**
  - [x] 9.1 `grep -rn "ExecutionManifestSchema\|MalformedExecutionManifestError\|parseExecutionManifest" plugins/flow/mcp-server/src plugins/flow/mcp-server/tests` and confirm every consumer (the scan tool, the new test file, and any future-story stub if you happen to find one) imports from the canonical location: `schemas/execution-manifest.js` for the schema/helper, `errors.js` for the typed error.
...
- [x] **Task 10 — Rebuild and commit dist (Story 1.9 contract)**
  - [x] 10.1 Run `pnpm --dir plugins/flow build` from the plugin root after all source changes.
  - [x] 10.2 `git add plugins/flow/mcp-server/dist/`. CI's dist-drift check fails the PR otherwise.
  - [x] 10.3 `git status` must show staged changes including: the new `schemas/execution-manifest.ts`, the new `tools/scan-sources.ts`, the updated `tools/register.ts`, the updated `errors.ts`, the new `skills/scan/SKILL.md`, the README "Available skills" bullet, the new `tests/scan-sources.test.ts`, the new fixture under `tests/fixtures/scan-sources-fixture/`, optionally the updated `schemas/telemetry-events.ts` (if Task 2.10 added a new event kind), and the rebuilt `plugins/flow/mcp-server/dist/` tree.

- [x] **Task 11 — Self-check before handoff**
  - [x] 11.1 `pnpm --dir plugins/flow test` — all suites green, including the new `scan-sources.test.ts` and the SKILL.md content-anchor assertion.
  - [x] 11.2 `pnpm --dir plugins/flow typecheck` — zero errors.
  - [x] 11.3 `pnpm --dir plugins/flow build` — clean build; `dist/` updated.
  - [x] 11.4 `grep -n "ExecutionManifestSchema" plugins/flow/mcp-server/src/schemas/execution-manifest.ts` returns exactly one hit (the export).
  - [x] 11.5 `grep -n "scanSources" plugins/flow/mcp-server/src/tools/register.ts` returns at least two hits (the import and the descriptor block).
  - [x] 11.6 `grep -n "name: crew:scan" plugins/flow/skills/scan/SKILL.md` returns exactly one hit (the frontmatter line — AC7 anchor).
  - [x] 11.7 `grep -n "scan-sources" plugins/flow/skills/scan/SKILL.md` returns at least one hit (the body reference — AC7 anchor).
  - [x] 11.8 `grep -n "MalformedExecutionManifestError" plugins/flow/mcp-server/src/errors.ts` returns exactly one hit (the declaration); `grep -rn "MalformedExecutionManifestError" plugins/flow/mcp-server/src plugins/flow/mcp-server/tests` returns at least three hits (declaration, throw site, test site).
  ...
  - [x] 11.10 Smoke the user surface end-to-end **before opening the PR** (Story 1.8 pre-PR gate requirement for `(user-surface)` ACs): install the plugin into a clean target repo, run `/flow:scan` verbatim in Claude Code, paste the verbatim TUI output into the ship-story run log via the smoke harness, confirming the picker lists `/flow:scan` and the printed result matches the fixture-test expectations. Without this paste-verbatim event, the pre-PR gate refuses with exit 42.
...
This is the seam crossing between the **source layer** (read-only, owned by the planning tool — BMad in v1) and the **execution layer** (plugin-owned, on-disk under `.flow/state/`). The architecture pins the two layers in `_bmad-output/planning-artifacts/architecture/planning-adapter-model.md` § Two-layer model. Before this story: the adapter can *describe* the source (Story 3.3), and the registry can *find* the adapter (Story 3.1), but no on-disk artefact exists in the execution layer. After this story: every source story has a per-story manifest at `to-do/<ref>.yaml`, hashed at scan time so future dev-loop reads can detect drift.
...
`/flow:status` set the precedent: skills are one-screen prose that name the MCP tool, the prerequisites, the steps (almost always "invoke the tool and print the result verbatim"), and the failure modes. The skill is NOT the place for tutorial content, troubleshooting walkthroughs, or "what is this plugin" prose — those live in the README.
...
The slash-command surface (`/flow:scan`) is what the operator actually types. That surface is unaffected by the file-path-shape detail; the AC's user-surface promise is honoured.

### What's NEW vs UPDATE

**NEW files:**
- `plugins/flow/mcp-server/src/schemas/execution-manifest.ts` — Task 1.
- `plugins/flow/mcp-server/src/tools/scan-sources.ts` — Task 2.
- `plugins/flow/mcp-server/tests/scan-sources.test.ts` — Task 8.
- `plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/.flow/config.yaml` — Task 5.
- `plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/.flow/state/.gitkeep` — Task 5.
- `plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/_bmad-output/planning-artifacts/stories/1-1-fixture-story-a.md` — Task 5.
- `plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/_bmad-output/planning-artifacts/stories/1-2-fixture-story-b.md` — Task 5.
- `plugins/flow/skills/scan/SKILL.md` — Task 6.

**UPDATE files:**
- `plugins/flow/mcp-server/src/errors.ts` — add `MalformedExecutionManifestError`. Other classes unchanged.
- `plugins/flow/mcp-server/src/tools/register.ts` — add `scanSources` descriptor. Existing descriptors unchanged.
- `plugins/flow/mcp-server/src/schemas/telemetry-events.ts` — optional; only if Task 2.10 adds a new `kind` enum member. Otherwise unchanged.
- `plugins/flow/README.md` — add one bullet under "Available skills".
- `plugins/flow/mcp-server/dist/**` — rebuilt by `pnpm build`, committed per Story 1.9.
...
- `plugins/flow/mcp-server/src/adapters/**` — owned by Story 3.1, Story 3.3, and Story 3.4.
- `plugins/flow/mcp-server/src/state/workspace-resolver.ts` — owned by Story 1.2. The resolver's behaviour does not change.
- `plugins/flow/mcp-server/src/state/manifest-state-machine.ts` — owned by Story 1.6.
- `plugins/flow/mcp-server/src/state/validate-active-adapter.ts` — owned by Story 1.2b.
- `plugins/flow/mcp-server/src/server.ts` — server-creation contract unchanged.
- `plugins/flow/mcp-server/package.json` — no new deps.
- `plugins/flow/.claude-plugin/plugin.json`, `marketplace.json`.
- `plugins/flow/catalogue/**`, `plugins/flow/permissions/**` — no role / permission change.
- `plugins/flow/skills/{ask,hire,skip-hiring,status,team}/**` — existing skills unchanged.
- `plugins/flow/example/**` — example workspace unchanged.
- `README.md` (repo-root), `plugins/flow/docs/README-install.md`, `plugins/flow/docs/user-surface-acs.md` — no install-path or rubric change.
...
The BMad adapter's committed fixtures at `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` are owned by Story 3.3's adapter integration tests. Those tests assert on the adapter's interface methods (`detect`, `listSourceStories`, `readSourceStory`, `resolveSourcePath`) — not on what happens downstream in the scan tool.

The scan-sources fixture under `tests/fixtures/scan-sources-fixture/` adds the `.flow/config.yaml` and the empty `.flow/state/` skeleton that the scan flow needs. Sharing the fixture would either (a) bloat the BMad fixture with state-layer artefacts that the BMad tests don't care about, or (b) require the scan test to overlay state-layer files at runtime — fragile.
...
The `/flow:scan` skill invokes `scanSources` without setting `_meta.role` — matching `/flow:status`'s pattern. That means the permission gate at `server.ts:131–147` is bypassed (the `if (_meta?.role)` block doesn't fire). This is intentional for v1: skills invoked by the operator directly (vs. by a sub-agent persona) run as the operator and inherit operator authority.

When Story 3.4 lands the planner subagent, the planner WILL invoke `scanSources` under `_meta.role: "planner"` (or similar) — at that point the planner's permission spec at `plugins/flow/catalogue/permissions/planner.yaml` must list `scanSources` in `tools_allow`. That edit belongs to Story 3.4, not this story.

### Testing standards

- vitest, run via `pnpm --dir plugins/flow test`.
...
- `plugins/flow/mcp-server/src/schemas/` currently contains 10 schema files. The new `execution-manifest.ts` is the 11th and the first that schemas execution-layer artefacts (the others are catalogue/persona/permission/repo-signals/standards-doc/status-report/team-snapshot/telemetry-events/workspace-config). The naming convention (`<thing>.ts` exporting `<Thing>Schema`) is preserved.
- `plugins/flow/mcp-server/src/tools/` currently contains 8 tool files (`get-status`, `get-team-snapshot`, `instantiate-persona`, `lookup-role-by-domain`, `read-catalogue`, `read-custom-role`, `read-persona`, `read-repo-signals`) plus `register.ts`. The new `scan-sources.ts` is the 9th tool file. All are kebab-case filenames exporting camelCase functions.
- `plugins/flow/skills/` currently contains 5 skill directories (`ask`, `hire`, `skip-hiring`, `status`, `team`). The new `scan` directory is the 6th. All use the `<name>/SKILL.md` layout.
- `plugins/flow/mcp-server/tests/fixtures/` may not yet exist as a directory (the BMad adapter's fixtures live at `src/adapters/bmad/fixtures/`, not under `tests/`). If `tests/fixtures/` does not exist, create it as part of Task 5; if it does exist (from an earlier story), nest under it.
...
- Story 1.2 (workspace resolver — canonical config-reading entrypoint): [Source: _bmad-output/implementation-artifacts/1-2-workspace-resolver-and-per-target-repo-config.md; `plugins/flow/mcp-server/src/state/workspace-resolver.ts`]
- Story 1.4 (MCP tool registration pattern + role permission gate): [Source: _bmad-output/implementation-artifacts/1-4-permission-allowlist-scaffolding-and-tool-layer-enforcement.md; `plugins/flow/mcp-server/src/tools/register.ts`]
- Story 1.5 (pino jsonl telemetry — for Task 2.10's structured log lines): [Source: _bmad-output/implementation-artifacts/1-5-jsonl-telemetry-plumbing-via-pino.md; `plugins/flow/mcp-server/src/schemas/telemetry-events.ts`]
- Story 1.6 (atomic-rename state machine + `STATE_NAMES`): [Source: _bmad-output/implementation-artifacts/1-6-atomic-fs-rename-state-machine-primitive.md; `plugins/flow/mcp-server/src/state/manifest-state-machine.ts`]
- Story 1.7 (`/flow:status` skill template — closest precedent for `/flow:scan` shape): [Source: _bmad-output/implementation-artifacts/1-7-status-skill-and-readme-install-path-through-the-plugin-sees-my-repo.md; `plugins/flow/skills/status/SKILL.md`]
- Story 1.8 (`(user-surface)` AC type + pre-PR smoke gate — gates AC4's verification): [Source: _bmad-output/implementation-artifacts/1-8-user-surface-ac-type-and-smoke-gate-in-ship-story.md; `plugins/flow/docs/user-surface-acs.md`]
...
- User-surface AC rubric: [Source: plugins/flow/docs/user-surface-acs.md] — AC4 qualifies; the others do not.
- Existing typed-error precedents (`InvalidWorkspaceConfigError`, `NoAdapterMatchedError`): [Source: `plugins/flow/mcp-server/src/errors.ts`]
- Canonical-fs write boundary: [Source: `plugins/flow/mcp-server/src/lib/managed-fs.ts`; canonical-fs test guard at `plugins/flow/mcp-server/tests/canonical-fs-guard.test.ts`]
...
- Task 5: Fixture at `tests/fixtures/scan-sources-fixture/` with two BMad stories and `.flow/state/.gitkeep`.
- Task 6: `plugins/flow/skills/scan/SKILL.md` with `name: crew:scan` frontmatter and `scan-sources` body reference.
- Task 7: `plugins/flow/README.md` "Available skills" section added with `/flow:scan` bullet.
...
- plugins/flow/mcp-server/src/schemas/execution-manifest.ts (NEW)
- plugins/flow/mcp-server/src/tools/scan-sources.ts (NEW)
- plugins/flow/mcp-server/src/errors.ts (UPDATED)
- plugins/flow/mcp-server/src/tools/register.ts (UPDATED)
- plugins/flow/mcp-server/tests/scan-sources.test.ts (NEW)
- plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/.flow/config.yaml (NEW)
- plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/.flow/state/.gitkeep (NEW)
- plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/_bmad-output/planning-artifacts/stories/1-1-fixture-story-a.md (NEW)
- plugins/flow/mcp-server/tests/fixtures/scan-sources-fixture/_bmad-output/planning-artifacts/stories/1-2-fixture-story-b.md (NEW)
- plugins/flow/skills/scan/SKILL.md (NEW)
- plugins/flow/README.md (UPDATED)
- plugins/flow/mcp-server/tests/ask-mode-enforcement.test.ts (UPDATED)
- plugins/flow/mcp-server/tests/ask-skill.test.ts (UPDATED)
- plugins/flow/mcp-server/tests/get-team-snapshot.test.ts (UPDATED)
- plugins/flow/mcp-server/dist/schemas/execution-manifest.d.ts (NEW)
- plugins/flow/mcp-server/dist/schemas/execution-manifest.js (NEW)
- plugins/flow/mcp-server/dist/tools/scan-sources.d.ts (NEW)
- plugins/flow/mcp-server/dist/tools/scan-sources.js (NEW)
- plugins/flow/mcp-server/dist/tools/register.js (UPDATED)
- plugins/flow/mcp-server/dist/errors.d.ts (UPDATED)
- plugins/flow/mcp-server/dist/errors.js (UPDATED)
