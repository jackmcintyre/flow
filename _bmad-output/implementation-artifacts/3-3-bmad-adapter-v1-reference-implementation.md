...
**AC4:**
**Given** the BMad adapter's fixture target repo at `plugins/flow/mcp-server/src/adapters/bmad/fixtures/`,
...
- [ ] **Task 1 — BMad-format spike report (AC: 1)**
  - [ ] 1.1 Create `plugins/flow/docs/spikes/bmad-format.md`. The directory does not exist today (`ls plugins/flow/docs/spikes` returns "No such file or directory" as of story authoring); create it.
  ...
  - [ ] 1.3 The spike is a maintainer-facing reference, not a user doc. It must NOT be linked from `README.md`, `plugins/flow/README.md`, or `plugins/flow/docs/README-install.md`. It IS linked from this story's References block and from the BMad adapter's source via a TSDoc `@see` comment on the `BmadAdapter` export.
...
- [ ] **Task 2 — `detect()` implementation (AC: 4, 5)**
  - [ ] 2.1 Implement `BmadAdapter.detect(targetRepo: string): Promise<boolean>` in `plugins/flow/mcp-server/src/adapters/bmad/index.ts`. Replace the `NotImplementedError` throw.
...
- [ ] **Task 3 — `listSourceStories()` + the parser (AC: 2, 5)**
  ...
  - [ ] 3.3 Per-file parsing — extract a new helper at `plugins/flow/mcp-server/src/adapters/bmad/parse-bmad-story.ts` (this filename is pre-allocated by the architecture project-structure tree; see References). The helper exports a single function `parseBmadStory(absPath: string, fileContents: string): SourceStory`. It is pure (no I/O beyond what the caller passes in) so it is trivially unit-testable.
  - [ ] 3.4 Parser contract:
...
- [ ] **Task 4 — `readSourceStory()` + status reconciliation (AC: 2, 3, 5)**
  ...
  - [ ] 4.3 §Status reconciliation (AC3) — this is the load-bearing behaviour:
    - The adapter exposes a helper `reconcileStatus(sourceStatus: string, manifestStatus: string): ReconciliationOutcome` exported from `plugins/flow/mcp-server/src/adapters/bmad/map-bmad-status.ts` (filename pre-allocated by the architecture tree).
...
- [ ] **Task 6 — Typed errors (AC: 2, 3, 5)**
  - [ ] 6.1 Extend `plugins/flow/mcp-server/src/errors.ts` with three new error classes, each subclassing the existing `DomainError` (or whatever the canonical base is in that file — confirm by reading it before editing):
...
- [ ] **Task 7 — Fixture target repo (AC: 4, 5)**
  - [ ] 7.1 Create the fixture tree under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/`. The directory does not exist today; create it. The architecture tree spells it as `mcp-server/src/adapters/bmad/fixtures/`; the epic uses the same path (`mcp-server/src/adapters/bmad/fixtures/`) — match that exactly. Note the path in the epic AC says `mcp-server/src/adapters/bmad/fixtures/` (relative to the plugin root); resolve against `plugins/flow/`.
  ...
  - [ ] 7.6 The fixture is committed to git. It is NOT gitignored. (Confirm by checking `plugins/flow/.gitignore` after creating — the broad `dist/` rule is gone post-Story-1.9; nothing else should swallow `src/adapters/bmad/fixtures/`.)
...
- [ ] **Task 8 — Vitest integration suite (AC: 4, 5)**
  - [ ] 8.1 Add `plugins/flow/mcp-server/tests/bmad-adapter.test.ts`. Existing test files use vitest's `describe`/`test`/`expect`; match that style (see `workspace-resolver.test.ts`, `validate-active-adapter.test.ts` for the closest precedents). Imports use the `.js` extension convention (NodeNext ESM resolution).
  ...
  - [ ] 8.4 Run `pnpm --dir plugins/flow test` and confirm the new suite passes alongside every existing suite. Zero new skips, zero new flakes.

- [ ] **Task 9 — Wire-through and exports (AC: 2, 4)**
  - [ ] 9.1 The `BmadAdapter` export already exists in `plugins/flow/mcp-server/src/adapters/bmad/index.ts` and is already referenced by `mcp-server/src/adapters/registry.ts`. Do NOT rename or relocate it.
  ...
  - [ ] 9.3 Update the TSDoc comment block on `BmadAdapter` to remove the "lands in Story 3.3" placeholder and add a `@see plugins/flow/docs/spikes/bmad-format.md` reference.
  - [ ] 9.4 **Verify dist commit (Story 1.9 contract):** after all code changes, run `pnpm --dir plugins/flow build` from the plugin root and `git add plugins/flow/mcp-server/dist/`. The committed `dist/` must reflect the new source. CI's `git diff --exit-code mcp-server/dist` step will fail the PR otherwise. This is non-negotiable per Story 1.9.

- [ ] **Task 10 — Self-check before handoff**
  - [ ] 10.1 Run the full plugin test suite: `pnpm --dir plugins/flow test`. All suites green.
  - [ ] 10.2 Run `pnpm --dir plugins/flow typecheck` (or whichever script wraps `tsc --noEmit`). Zero errors.
  - [ ] 10.3 Confirm `git status` shows: new files under `plugins/flow/mcp-server/src/adapters/bmad/{parse-bmad-story.ts,map-bmad-status.ts,fixtures/...}`, new `plugins/flow/mcp-server/tests/bmad-adapter.test.ts`, updates to `index.ts` and `errors.ts`, new `plugins/flow/docs/spikes/bmad-format.md`, and the rebuilt `plugins/flow/mcp-server/dist/`. Nothing under `_bmad-output/implementation-artifacts/` should be staged.
...
**NEW files:**
- `plugins/flow/mcp-server/src/adapters/bmad/parse-bmad-story.ts` — pure parser, no I/O.
- `plugins/flow/mcp-server/src/adapters/bmad/map-bmad-status.ts` — `reconcileStatus` helper + the lifecycle-mapping table.
- `plugins/flow/mcp-server/src/adapters/bmad/fixtures/sample-target-repo/...` and `plugins/flow/mcp-server/src/adapters/bmad/fixtures/sample-malformed-repo/...` — committed fixture target repos (happy-path and error-path, respectively; see Task 7.2).
- `plugins/flow/mcp-server/tests/bmad-adapter.test.ts` — integration suite.
- `plugins/flow/docs/spikes/bmad-format.md` — the AC1 spike. The `spikes/` directory is also new.

**UPDATE files:**
- `plugins/flow/mcp-server/src/adapters/bmad/index.ts` — replace `NotImplementedError` stubs with real implementations; re-export helpers; update TSDoc.
- `plugins/flow/mcp-server/src/errors.ts` — add the three typed errors (Task 6.1).
- `plugins/flow/mcp-server/dist/**` — rebuilt by `pnpm build`, committed per Story 1.9's contract. Do not hand-edit.
...
- `plugins/flow/mcp-server/src/adapters/adapter.ts` — interface owned by Story 3.1.
- `plugins/flow/mcp-server/src/adapters/registry.ts` — registry behaviour owned by Story 3.1; this story only relies on `BmadAdapter` already being in the `adapters` array.
- `plugins/flow/mcp-server/src/adapters/native/**` — owned by Story 3.4.
- `plugins/flow/mcp-server/src/tools/scan-sources.ts` (when it exists) — owned by Story 3.2.
- `plugins/flow/mcp-server/src/state/source-hash.ts` (when it exists) — owned by Story 3.2.
- `plugins/flow/mcp-server/package.json` — no new deps unless absolutely necessary. Sha256 is `node:crypto`; YAML reading (for `.flow/config.yaml`) is already handled upstream by the workspace resolver. Path/fs are `node:*`.
...
- `README.md`, `plugins/flow/README.md`, `plugins/flow/docs/README-install.md` — the adapter is a non-user-surface internal seam; install docs do not change.
...
- `errors.ts` is at `plugins/flow/mcp-server/src/errors.ts` (14.9K today). Read the full file before editing to match the existing error-class pattern.
- The fixture path under `src/` does NOT confuse `tsc` because TypeScript only compiles `.ts` files by default and fixtures are `.md` + `.yaml`. Confirm by inspecting `plugins/flow/mcp-server/tsconfig.json`'s `include` — if it broadly globs `src/**/*` for non-`.ts` assets, narrow the include to `src/**/*.ts` so fixtures are not copied into `dist/`.
...
- Adapter scaffolding (Stories 1.1, 1.2): [Source: plugins/flow/mcp-server/src/adapters/adapter.ts, plugins/flow/mcp-server/src/adapters/bmad/index.ts, plugins/flow/mcp-server/src/adapters/registry.ts]
...
- Existing error patterns: [Source: plugins/flow/mcp-server/src/errors.ts]
...
- Spike doc target: plugins/flow/docs/spikes/bmad-format.md (created by Task 1)
