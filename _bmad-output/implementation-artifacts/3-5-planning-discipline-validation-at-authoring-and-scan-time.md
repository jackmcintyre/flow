...
- (h) Change BMad's source files. The BMad adapter validates the **normalised** `SourceStory` it produces — it does not write back to the BMad tree. A BMad source story missing an integration AC produces a blocked manifest; the operator fixes the source story in BMad, then re-runs `/flow:scan`.
...
> Verbatim from epic for AC1–AC5, with user-surface tags applied per `plugins/flow/docs/user-surface-acs.md`. AC6 is the deterministic content-structure check required by the spec brief (LLM outputs are non-deterministic; structural anchors make ACs verifiable without human judgement).

**AC1 (user-surface):**
**Given** a state-mutating native story authored without an integration AC during a `/flow:plan` conversation,
...
<!-- User-surface: AC1 governs observable behaviour of the planner subagent inside a `/flow:plan` (rubric i) session. The operator sees the refusal prompt in the Claude Code chat (rubric iv — a UI element the user observes). The validator itself is internal, but the AC asserts what the operator sees, which is the user-surface concern. -->
...
<!-- User-surface: same rationale as AC1 — observable planner refusal during a `/flow:plan` conversation (rubric i, rubric iv). -->

**AC3 (user-surface):**
**Given** a backlog being finalised by the planner with no story flagged as the ship-gate (no story in the pending-write batch carries `ship_gate: true` in its frontmatter / planner-input metadata, and no existing native story in `<target-repo>/.flow/native-stories/` carries it either),
...
<!-- User-surface: governs the final visible step of `/flow:plan` (rubric i) — the locked handoff phrase the operator reads at end-of-conversation. The refusal text and the prompt are both observed in chat (rubric iv). -->
...
**When** `/flow:scan` runs (invoking the `scanSources` MCP tool, which calls `BmadAdapter.validateAgainstDiscipline()`),
**Then** the resulting execution manifest is written to `<target-repo>/.flow/state/blocked/<ref>.yaml` (not `to-do/`) with `status: blocked`, `blocked_by: "planning-discipline"`, and a `discipline_violations:` block citing the missing field; the `/flow:scan` summary output names the blocked ref and the rule code (`missing-integration-ac`). _(Architecture Gap 1, FR5 scan-time mirror)_
<!-- User-surface: AC4 names the slash command `/flow:scan` (rubric i) AND the file-path pattern `<target-repo>/.flow/state/blocked/<ref>.yaml` which the install/README docs name as part of the state-machine surface the operator inspects (rubric iii). The summary line printed by the skill is observed in chat (rubric iv). -->
...
- **MUST** be a new MCP tool at `plugins/flow/mcp-server/src/tools/validate-planner-backlog.ts`, registered in `register.ts` alongside `writeNativeStory` and `scanSources`.
...
- **MUST**, on `validateAgainstDiscipline` returning a `DisciplineViolation`, write a `blocked_by: "planning-discipline"` manifest into `<target-repo>/.flow/state/blocked/<ref>.yaml` AND record the ref in `result.skippedRefs` with `reason: "discipline-violation"` and `detail` = the first violation's `detail` string (the existing seam in `scan-sources.ts` lines 188–196 already covers the `skippedRefs` half; this story extends it to also write the blocked manifest). The render-summary text MUST include the blocked refs as a separate line so `/flow:scan` output names them.
...
- **MUST NEVER** modify `sprint-status.yaml` or any file under `_bmad-output/implementation-artifacts/` during runtime. The validator is a read-only pure function; the MCP tool and the scan-time integration write only to `<target-repo>/.flow/state/blocked/` (manifest layer).
...
- [ ] **Task 1 — Discipline-validator pure module (AC: 1, 2, 3, 4, 5)**
  - [ ] 1.1 Create `plugins/flow/mcp-server/src/validators/planning-discipline.ts` exporting two pure functions: `validateStoryAgainstDiscipline(story: SourceStory, opts?: { stateMutating?: boolean }): SourceStory | DisciplineViolation` and `validateBacklogAgainstDiscipline(stories: SourceStory[], opts: { existingStories: SourceStory[]; backlogPseudoRef?: string }): DisciplineViolation[]`. Pure (no I/O); deterministic.
...
- [ ] **Task 2 — `NativeAdapter.validateAgainstDiscipline` real implementation (AC: 1, 2, 5)**
  - [ ] 2.1 Edit `plugins/flow/mcp-server/src/adapters/native/index.ts`. Replace the pass-through `validateAgainstDiscipline` (lines 175–177 today) with a call to `validateStoryAgainstDiscipline(story)` from Task 1. Per-story call only — no backlog-level ship-gate check here (that's `validatePlannerBacklog`'s job). The `stateMutating` heuristic runs against the story text.
...
- [ ] **Task 3 — `BmadAdapter.validateAgainstDiscipline` real implementation (AC: 4, 5)**
  - [ ] 3.1 Edit `plugins/flow/mcp-server/src/adapters/bmad/index.ts`. Replace the pass-through `validateAgainstDiscipline` (lines 292–294 today) with a call to `validateStoryAgainstDiscipline(story)` from Task 1. Per-story call.
...
- [ ] **Task 4 — BMad parser: surface `ship_gate` from frontmatter / tags (AC: deferred-to-Task-5 ship-gate path; consumed if/when BMad backlog-level enforcement is added)**
  ...
  - [ ] 4.3 Add a unit test in `plugins/flow/mcp-server/src/adapters/bmad/__tests__/parse-bmad-story.ship-gate.test.ts` that asserts the field is surfaced (or `undefined`) per the chosen behaviour.

- [ ] **Task 5 — `validatePlannerBacklog` MCP tool (AC: 1, 2, 3, 5)**
  - [ ] 5.1 Create `plugins/flow/mcp-server/src/tools/validate-planner-backlog.ts`. Zod input schema:
    ```
    {
  ...
  - [ ] 5.4 Register the tool in `plugins/flow/mcp-server/src/tools/register.ts` next to `writeNativeStory`. Allowlist it in `plugins/flow/permissions/planner.yaml` under `tools_allow`.
  - [ ] 5.5 Add an integration test at `plugins/flow/mcp-server/src/tools/__tests__/validate-planner-backlog.integration.test.ts` covering each of AC1, AC2, AC3 against synthetic pending batches in a tmpdir target repo configured as `adapter: native`.

- [ ] **Task 6 — `scan-sources` integration: write blocked manifest on discipline-violation (AC: 4, 5)**
  - [ ] 6.1 Edit `plugins/flow/mcp-server/src/tools/scan-sources.ts`. Locate the existing `discipline-violation` branch (lines 188–196 today). Currently it only records the ref in `result.skippedRefs` and `continue`s. Extend it to also: (a) compose a manifest from the source story (use the existing `composeManifest` helper but override `status` to `"blocked"`), (b) attach a new `blocked_by: "planning-discipline"` field, (c) attach `discipline_violations: <violations array from the DisciplineViolation result>`, (d) write to `<target-repo>/.flow/state/blocked/<ref>.yaml` via the same `writeManagedFile` path used for `to-do/`, (e) only write if no manifest exists in any state dir for that ref (preserves the existing "scan does not touch claimed work" invariant from `scan-sources.ts` lines 261–267).
  - [ ] 6.2 Extend `ExecutionManifestSchema` in `plugins/flow/mcp-server/src/schemas/execution-manifest.ts` to add two optional fields: `blocked_by?: "planning-discipline" | "source-drift" | string` (string fallback for forward-compat) and `discipline_violations?: Array<{ code: string, field: string, detail: string }>`. Both default to absent; only populated for blocked manifests.
...
- [ ] **Task 7 — Planner catalogue prompt extension (AC: 1, 2, 3, 6)**
  ...
  - [ ] 7.3 Add the literal string `validatePlannerBacklog` to `tools_allow` in `plugins/flow/permissions/planner.yaml` (if not already added by Task 5.4).
...
- [ ] **Task 8 — `/flow:scan` skill: surface blocked refs verbatim (AC: 4)**
  - [ ] 8.1 Inspect `plugins/flow/skills/scan/SKILL.md`. The skill's `# Steps` section already prints `renderScanResult`'s output verbatim per Story 3.2. Confirm the Task 6.3 render change is automatically surfaced; no skill-level edit is needed beyond confirming the verbatim-print contract.
...
- [ ] **Task 9 — Wire-up + documentation (AC: 4, 6)**
  - [ ] 9.1 Update `plugins/flow/docs/README-install.md`: add a one-paragraph section on planning-discipline blocks. Name the four codes and the operator's remediation path (edit the source story; re-run `/flow:scan`).
  - [ ] 9.2 Confirm `plugins/flow/README.md` (if present) does not need editing — the install README is the canonical surface for state-machine semantics.
  - [ ] 9.3 Rebuild and commit `plugins/flow/mcp-server/dist/` per the project's "build output is tracked in git" rule (CLAUDE.md §Process notes). CI fails on drift.
  - [ ] 9.4 Add a fixture under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` (or extend an existing one) containing a state-mutating BMad story with zero integration ACs. Used by the Task 6.4 integration test.
  - [ ] 9.5 Add a fixture under `plugins/flow/mcp-server/src/adapters/native/fixtures/` containing a native story with an implicit `depends_on` (references another ref in prose but omits from `depends_on`). Used by the Task 5.5 integration test.
...
- `plugins/flow/mcp-server/src/validators/planning-discipline.ts` (note: `validators/` directory already exists per `ls`)
- `plugins/flow/mcp-server/src/validators/__tests__/planning-discipline.test.ts`
- `plugins/flow/mcp-server/src/tools/validate-planner-backlog.ts`
- `plugins/flow/mcp-server/src/tools/__tests__/validate-planner-backlog.integration.test.ts`
- `plugins/flow/mcp-server/src/adapters/bmad/__tests__/parse-bmad-story.ship-gate.test.ts` (or co-located in the existing BMad test file)
- A BMad fixture story file under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` exercising the missing-integration-AC violation (Task 9.4).
- A native fixture story file under `plugins/flow/mcp-server/src/adapters/native/fixtures/.flow/native-stories/` exercising the implicit-depends-on violation (Task 9.5).

UPDATE files (exist today; story modifies):

- `plugins/flow/mcp-server/src/adapters/adapter.ts` — only if Task 1.6 widens the `code` union (not recommended in v1).
- `plugins/flow/mcp-server/src/adapters/native/index.ts` — replace pass-through `validateAgainstDiscipline`.
- `plugins/flow/mcp-server/src/adapters/bmad/index.ts` — replace pass-through `validateAgainstDiscipline`.
- `plugins/flow/mcp-server/src/adapters/bmad/parse-bmad-story.ts` — surface `ship_gate` from frontmatter/tags (Task 4).
- `plugins/flow/mcp-server/src/schemas/execution-manifest.ts` — add optional `blocked_by`, `discipline_violations` fields.
- `plugins/flow/mcp-server/src/tools/scan-sources.ts` — write blocked manifest on `discipline-violation`; extend `ScanResult` with `blockedRefs`; extend `renderScanResult` with the `blocked:` line.
- `plugins/flow/mcp-server/src/tools/scan-sources.integration.test.ts` (or wherever the existing scan-sources tests live) — assert AC4 path.
- `plugins/flow/mcp-server/src/tools/register.ts` — register `validatePlannerBacklog`.
- `plugins/flow/catalogue/planner.md` — append the discipline pre-write subsection (AC6 anchor).
- `plugins/flow/permissions/planner.yaml` — add `validatePlannerBacklog` to `tools_allow`.
- `plugins/flow/skills/scan/SKILL.md` — confirm verbatim-print of `renderScanResult`; add the one-line operator note.
- `plugins/flow/docs/README-install.md` — add the planning-discipline section.
...
- AC6 (catalogue prompt content-structure) is covered by a deterministic grep-style test — recommend a new `plugins/flow/mcp-server/src/catalogue/__tests__/planner-prompt-shape.test.ts` (or co-locate with similar existing tests) that loads `plugins/flow/catalogue/planner.md` from disk and asserts the literal strings from Task 7.5 are present in the `## Prompt` section.
- Two-pass idempotency assertion is required on the AC4 path: when `source_hash` is unchanged, re-running `/flow:scan` against an already-blocked BMad story MUST NOT rewrite the manifest (mtime-stable). This mirrors the existing `to-do/` idempotency contract (`scan-sources.ts` AC2/NFR10 commentary). A third path must also be covered: when the source story is edited (new `source_hash`) and the validator still fails, the blocked manifest MUST be rewritten with the new hash and latest violations (not mtime-stable in this case).
...
- **Story 1.8** introduced the user-surface AC tag and the pre-PR smoke gate (`plugins/flow/docs/user-surface-acs.md`). This story is `user-surface` because the planner's refusal prompts and the `/flow:scan` blocked-ref surface are both observable in Claude Code; the pre-PR gate will require either an automated harness covering AC1–AC4 (the Task 5.5 + 6.4 integration tests qualify) OR operator-pasted verbatim Claude Code output of each user-surface AC's flow.
...
- Do NOT modify `sprint-status.yaml` or any file under `_bmad-output/implementation-artifacts/` during implementation. The orchestrator owns status. The validator and the MCP tool only write into `<target-repo>/.flow/state/blocked/` (manifest layer) and only via the existing `writeManagedFile` path.
