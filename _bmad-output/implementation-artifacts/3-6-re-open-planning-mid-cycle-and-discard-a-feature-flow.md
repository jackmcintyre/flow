...
- (b) Change the `PlanningAdapter` interface signature in `mcp-server/src/adapters/adapter.ts`. `markWithdrawn` is a tool-layer concern, not an adapter-interface concern — both adapters' manifests live in the same execution layer (`.flow/state/`), so manifest mutation is adapter-agnostic at the file level.
- (c) Implement a hand-edit allowance for execution manifests in `to-do/` or `blocked/`. That is Story 3.7's scope (FR14 second half). This story's edit-pending path mutates the **source story file** (the native `.md` under `.flow/native-stories/<ref>.md` for native, or — out of scope here — the BMad story file for BMad). Edit-pending is native-only in v1 for the same reason authoring is native-only: external adapters own their source tree.
...
> Verbatim from epic for AC1–AC4, with user-surface tags applied per `plugins/flow/docs/user-surface-acs.md`. AC5 is the deterministic content-structure check required by the spec brief (LLM outputs are non-deterministic; structural anchors make ACs verifiable without human judgement).

**AC1 (user-surface):**
**Given** a target repo with stories present in `<target-repo>/.flow/state/to-do/`, `<target-repo>/.flow/state/in-progress/`, and `<target-repo>/.flow/state/done/` (i.e. a primed, partially-executed backlog) and `adapter: native` configured,
**When** I run `/flow:plan` a second time,
...
<!-- User-surface: AC1 governs the observable behaviour of `/flow:plan` (rubric i — slash command literal) on its second invocation. The backlog summary and the three-action prompt are observed in the Claude Code chat (rubric iv — UI element the operator observes). The refusal string is verbatim observable surface. -->
...
**Then** the planner calls `writeNativeStory` with a new story whose `title` begins with the literal prefix `revert/deprecate: ` followed by the original story's title, whose `narrative` cites the original ref in `depends_on`, whose `acceptance_criteria` describes the reversal at user-value level, and whose `implementation_notes` names the files / surfaces to undo; the original native story file (`<target-repo>/.flow/native-stories/<original-ULID>.md`) and the original execution manifest are NOT deleted, modified, or moved. The new revert story enters the backlog as a fresh `to-do/` manifest on the next `/flow:scan`. _(FR78, Architecture Gap 3, `planning-adapter-model.md` §FR78 row)_
...
**AC3:**
**Given** the operator chooses `discard` against a ref `bmad:<source-id>` (or any external-adapter ref) whose execution manifest exists anywhere under `<target-repo>/.flow/state/`,
...
- **MUST**, on the BMad branch, still print the existing Story 3.4 BMad-pointer block verbatim AND additionally append a one-line discard offer: `"To withdraw a story from execution, run /flow:plan and choose 'discard' against the ref — the plugin will mark the manifest withdrawn (the source story in <adapter-name> remains your responsibility to close)."` This append happens only in re-open mode (i.e. when at least one manifest exists). On a fresh BMad repo with no manifests yet, the unchanged Story 3.4 BMad-pointer block prints verbatim with no append.
...
- **MUST** be a new MCP tool at `plugins/flow/mcp-server/src/tools/mark-withdrawn.ts`, registered in `register.ts` alongside `writeNativeStory` and `scanSources`. The architecture's planned path is `mark-withdrawn.ts` (per `project-structure-boundaries.md` line 86); use that filename verbatim.
...
- **MUST NEVER** delete a source story file (`.md` under `.flow/native-stories/`) or an execution manifest (`.yaml` under `.flow/state/`). Discard authors / mutates; it does not delete.
...
- [ ] **Task 1 — Backlog-inventory builder in the `/flow:plan` skill (AC: 1, 5)**
  - [ ] 1.1 Edit `plugins/flow/skills/plan/SKILL.md`. Extend Step 4 (`adapter: native` branch) and add equivalent Step 4b logic for `adapter: bmad` re-open mode. The skill MUST, before spawning the subagent, read every `.yaml` under `<targetRepoRoot>/.flow/state/{to-do,in-progress,blocked,done}/`, parse each via `parseExecutionManifest`, and accumulate a `backlog_inventory` array of `{ ref, title, state, withdrawn }`. On the native branch, also list every `.md` under `<targetRepoRoot>/.flow/native-stories/` matching the ULID pattern whose `native:<ULID>` ref does not appear in any manifest; add those as `state: "native-source-only"`, `withdrawn: false`, with `title` parsed from the file's first H1.
  ...
  - [ ] 1.4 Add a skill-side guard: if reading any `.yaml` under `.flow/state/` throws `MalformedExecutionManifestError`, surface the error verbatim and stop. Do not silently drop malformed manifests — the operator needs to fix them before re-opening planning (per the existing FR13 contract).

- [ ] **Task 2 — Planner catalogue prompt extension (AC: 1, 2, 3, 5)**
  - [ ] 2.1 Edit `plugins/flow/catalogue/planner.md`. Append a new subsection `### Re-open mode — backlog review and discard flow` to the `## Prompt` section, AFTER the existing `### Discipline validation — pre-write check` subsection and BEFORE the existing `### Scope reminder` subsection. This is the AC5 anchor.
  - [ ] 2.2 Subsection content MUST include verbatim (these literal strings are what the AC5 grep-test asserts):
    ...
    - The literal refusal string `"Edit-pending is native-only in v1. Edit the source story in <adapter-name> and run /flow:scan."`
  ...
  - [ ] 2.5 Add `markWithdrawn` to `tools_allow` in `plugins/flow/permissions/planner.yaml`.
...
- [ ] **Task 3 — `markWithdrawn` MCP tool (AC: 3, 4)**
  - [ ] 3.1 Create `plugins/flow/mcp-server/src/tools/mark-withdrawn.ts`. Zod input schema: `{ targetRepoRoot: z.string().min(1), ref: z.string().min(1) }`. Output type: `{ ref: string; alreadyWithdrawn: boolean; state: StateName; absPath: string }`.
...
- [ ] **Task 4 — `isClaimable` predicate (AC: 4)**
  - [ ] 4.1 Add `export function isClaimable(manifest: ExecutionManifest): boolean { return manifest.withdrawn === false && manifest.status === "to-do"; }` to `plugins/flow/mcp-server/src/state/manifest-state-machine.ts`. Pure; no I/O.
...
- [ ] **Task 5 — End-to-end integration tests (AC: 4)**
  ...
  - [ ] 5.3 Add a fixture under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` with a manifest in `done/` carrying `withdrawn: false`; used by the Task 3.4 idempotency test and the Task 5.1 BMad-discard branch.
  - [ ] 5.4 Add a fixture under `plugins/flow/mcp-server/src/adapters/native/fixtures/` with a pre-seeded `.flow/state/` tree (one ref each in `to-do/`, `in-progress/`, `done/`) and matching native source files in `.flow/native-stories/`; used by the native-branch AC4 paths.

- [ ] **Task 6 — `/flow:plan` skill: re-open branch documentation (AC: 1)**
  - [ ] 6.1 Edit `plugins/flow/skills/plan/SKILL.md`. Add a new section `# Re-open mode` after the existing `# Failure modes` section explaining (i) the re-open detection rule (any `.yaml` under `.flow/state/` OR any ULID `.md` under `.flow/native-stories/`), (ii) the `backlog_inventory` shape passed to the planner, (iii) the action menu the planner presents.
...
- [ ] **Task 7 — Documentation and wire-up (AC: 1, 3, 5)**
  - [ ] 7.1 Update `plugins/flow/docs/README-install.md`: add a one-paragraph section on the discard flow. Name both branches (native = revert story; external = `mark-withdrawn` + manual close in source tool). Cite FR78. Add a one-line bullet on how to confirm a withdrawal landed: inspect the manifest under `.flow/state/<state>/<ref>.yaml` for `withdrawn: true`.
  - [ ] 7.2 Confirm `plugins/flow/README.md` (if present) does not need editing — the install README is the canonical operator-facing surface.
  ...
  - [ ] 7.5 Add an AC5 grep-style test at `plugins/flow/mcp-server/src/catalogue/__tests__/planner-prompt-shape.reopen.test.ts` (or extend an existing planner-prompt-shape test) that loads `plugins/flow/catalogue/planner.md` from disk and asserts the literal strings from Task 2.2 are all present in the `## Prompt` section.
...
- `plugins/flow/mcp-server/src/tools/mark-withdrawn.ts` (Task 3).
- `plugins/flow/mcp-server/src/tools/__tests__/mark-withdrawn.integration.test.ts` (Task 3.4).
- `plugins/flow/mcp-server/src/skills/__tests__/plan-reopen.integration.test.ts` or co-located equivalent (Task 5.1).
- `plugins/flow/mcp-server/src/catalogue/__tests__/planner-prompt-shape.reopen.test.ts` or co-located equivalent (Task 7.5).
- New BMad fixture manifest under `plugins/flow/mcp-server/src/adapters/bmad/fixtures/` (Task 5.3).
- New native fixture tree under `plugins/flow/mcp-server/src/adapters/native/fixtures/` with pre-seeded `.flow/state/` and `.flow/native-stories/` (Task 5.4).
- `plugins/flow/mcp-server/src/lib/manifest-yaml.ts` — optional shared YAML writer (Task 3.5 — recommended default is to co-locate with the schema instead; if the existing scan-sources writer is already exportable from `execution-manifest.ts` or a sibling, reuse it and skip this new file).

UPDATE files (exist today; story modifies):

- `plugins/flow/skills/plan/SKILL.md` — backlog-inventory builder, re-open detection, BMad-branch discard offer (Task 1, Task 6).
- `plugins/flow/catalogue/planner.md` — new `### Re-open mode` subsection; add `markWithdrawn` to `tools_allow` (Task 2).
- `plugins/flow/permissions/planner.yaml` — add `markWithdrawn` to `tools_allow` (Task 2.5).
- `plugins/flow/mcp-server/src/tools/register.ts` — register `markWithdrawn` (Task 3.3).
- `plugins/flow/mcp-server/src/state/manifest-state-machine.ts` — add `isClaimable` predicate (Task 4).
- `plugins/flow/mcp-server/src/tools/scan-sources.ts` — only if the YAML writer needs extracting for reuse (Task 3.5). If the writer is already exportable, no edit needed.
- `plugins/flow/docs/README-install.md` — discard-flow section (Task 7.1).
- `plugins/flow/mcp-server/dist/` — rebuild and commit per `CLAUDE.md` §Process notes (Task 7.3).
...
- **Story 3.4** landed the native adapter, the planner subagent, the `/flow:plan` skill, and the `writeNativeStory` MCP tool. This story extends the skill's `<initial-context>` and the planner's prompt — it does not modify `writeNativeStory` itself. The Story 3.4 BMad-branch (no-subagent-spawn) behaviour is preserved on first-run; re-open mode spawns the subagent on BMad too so the discard flow has a conversational surface.
...
- **Story 1.6 / Story 1.8 / Story 1.11** — the atomic-rename primitive, the user-surface AC tag convention, the dev:install loop. This story is `user-surface` because AC1 names `/flow:plan` (rubric i) and the chat-observable action menu (rubric iv); the pre-PR smoke gate will require an automated harness covering AC1 OR operator-pasted verbatim Claude Code output of the action-menu flow.
...
- Do NOT modify `sprint-status.yaml` or any file under `_bmad-output/implementation-artifacts/` during implementation. The orchestrator owns status. Withdrawal writes to `<target-repo>/.flow/state/<state>/<ref>.yaml` only (manifest layer); native discard writes to `<target-repo>/.flow/native-stories/<new-ULID>.md` only (source layer).
