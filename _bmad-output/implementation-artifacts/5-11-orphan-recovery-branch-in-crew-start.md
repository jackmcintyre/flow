...
I want **`/flow:start` to detect an orphaned in-progress manifest (one whose `claimed_by` ULID is not the current session's ULID) and offer to replay its persisted dev transcript instead of silently moving on to the next claimable story**,
...
Add an outer-loop **orphan-recovery branch** to `/flow:start` that runs **before** `claimNextStory`, scans `<targetRepoRoot>/.flow/state/in-progress/`, surfaces any manifest whose `claimed_by` ULID differs from the current session ULID as a one-line `[orphan] <ref> — claimed_by <stale-ulid>` chat surface, asks the operator to choose `reattach` or `skip`, and routes the chosen path through (a) **transcript replay** if the Story 5.10 transcript file exists for the stale session, or (b) atomic move to `blocked/` with `blocked_by: orphan-no-transcript` if it does not, or (c) **no-op preservation** if the operator skips.

### What this story does (and why it needs its own story)

The 2026-05-25 dogfood postmortem (`§ L1, defect #2`) names this defect:

> When MCP died after the dev subagent returned, the in-progress manifest stayed pinned to a now-dead session ULID. The next `/flow:start` invocation called `claimNextStory`, which only inspects `to-do/`. It walked right past the stranded `in-progress/` manifest. No surface line. No prompt. The PR was already open with a locked handoff phrase — but the orchestrator had no path back to it.

Story 5.10 (shipped 2026-05-25) added the durable seam — the dev transcript is now persisted to `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/dev-transcript.txt` **before** any MCP call. 5.10 writes; **5.11 reads**.

The fix is structural: `/flow:start`'s outer claim loop must check `in-progress/` **before** asking `claimNextStory` for the next to-do candidate. If any in-progress manifest has a `claimed_by` ULID that does not match the current session's ULID, that manifest is an orphan. The orchestrator surfaces it, the operator decides whether to reattach (replay the transcript and resume from the reviewer step) or skip (leave it for human inspection), and the loop continues.
...
The product trade-off Jack accepted: shipping degraded closes L1 defect #2 (the rollback's "stale claims ignored, story stranded" defect) without blocking on the full Story 5.2 heartbeat surface. The worst-case false positive in the degraded design is **one operator prompt** during a (very rare) concurrent-session scenario — two operators running `/flow:start` against the same target repo at the same time. Acceptable: the operator sees one extra `[orphan]` line and chooses skip; no state is corrupted.
...
Once Story 5.2 ships the heartbeat surface (`<targetRepoRoot>/.flow/sessions/<session-id>.json` with a `2× interval` staleness threshold), AC1's orphan-detection criterion should be tightened from:

> **5.11 (this story):** `manifest.claimed_by !== currentSessionUlid`

to:

> **5.11 + 5.2:** `manifest.claimed_by !== currentSessionUlid` **AND** (the heartbeat file at `<targetRepoRoot>/.flow/sessions/<manifest.claimed_by>.json` is absent OR has not been refreshed within the staleness window).
...
- **Transcript-file garbage collection.** Once an orphan reaches `done/` (via reattach + green reviewer) or `blocked/` (via reattach-no-transcript or operator manual route), the transcript file at `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/dev-transcript.txt` is no longer load-bearing. A future story under Epic 5 (likely 5.9 or a sibling retention story) should sweep stale session directories.
...
- **Orphan inside a sub-skill (e.g. `/flow:watch`).** Once Story 5.3 ships `/flow:watch`, that skill could surface `[orphan]` lines too (since it also reads `in-progress/`). The detection logic should be extracted into a shared helper at that point. For now, the logic lives inline in `/flow:start`'s SKILL.md prose.

---

## Acceptance Criteria

> AC1 is `user-surface` per `plugins/flow/docs/user-surface-acs.md` rubric (iv) — it names a Claude Code chat-line surface and a prompt the operator types into. AC2–AC4 describe internal state transitions verified by vitest. AC5 is the integration suite.

**AC1 (user-surface):**

<!-- rubric (iv): any Claude Code UI element the user is expected to observe (chat-surface line + operator prompt) -->
**Given** an in-progress manifest at `<targetRepoRoot>/.flow/state/in-progress/<ref>.yaml` whose `claimed_by` session ULID is not the current session's ULID,
**When** the outer loop of `/flow:start` begins a new claim cycle (i.e., before invoking `claimNextStory`),
...
**AC2:**
**Given** the operator chooses `reattach` **AND** a persisted Story 5.10 transcript file exists at `<targetRepoRoot>/.flow/state/sessions/<stale-ulid>/dev-transcript.txt`,
...
**AC3:**
**Given** the operator chooses `reattach` **AND** no persisted transcript exists at `<targetRepoRoot>/.flow/state/sessions/<stale-ulid>/dev-transcript.txt` (the file is missing — e.g. the orphan was created before Story 5.10 deployed, or the session ULID directory was manually cleaned),
**When** the loop proceeds,
**Then** the loop atomically moves the manifest from `<targetRepoRoot>/.flow/state/in-progress/<ref>.yaml` to `<targetRepoRoot>/.flow/state/blocked/<ref>.yaml` via `moveBetweenStates`, and the moved manifest carries `blocked_by: orphan-no-transcript`. The chat line surfaced to the operator is:
...
**When** the loop proceeds,
**Then** the orphan manifest is left in `<targetRepoRoot>/.flow/state/in-progress/<ref>.yaml` **untouched** (no `claimed_by` rewrite, no move, no field mutation), and the outer loop advances to the next orphan-or-claim step (i.e., re-runs the orphan scan against the remaining in-progress manifests, then falls through to `claimNextStory`). The skipped orphan MUST surface again on the next `/flow:start` invocation (since its state has not changed) — `skip` is a per-invocation deferral, not a permanent dismissal. _(vitest: — skip preserves orphan state; covered by AC5 fixture C)_
...
- **The orphan scan MUST run at the top of every outer-loop iteration**, immediately before each `claimNextStory` call. It MUST NOT run inside the inner cycle (mid-dev, mid-reviewer). It MUST NOT run only once per `/flow:start` invocation — a newly-appearing orphan (e.g. from a concurrent session that died between iterations) MUST be picked up on the next iteration.
...
- **On `skip`, the manifest MUST NOT be mutated.** No `claimed_by` rewrite, no field add, no move. Byte-identity preservation is the contract — the next `/flow:start` invocation MUST re-surface the orphan if it is still orphaned.
...
- [x] **Task 1: Add the `scanOrphanedInProgress` MCP read-only helper** (AC: #1)
  - [x] 1.1 Create `plugins/flow/mcp-server/src/tools/scan-orphaned-in-progress.ts`. Signature: `scanOrphanedInProgress({ targetRepoRoot, sessionUlid }): Promise<{ orphans: { ref: string; staleUlid: string; manifestPath: string; transcriptPath: string; hasTranscript: boolean }[] }>`.
  - [x] 1.2 Behaviour: enumerate `<targetRepoRoot>/.flow/state/in-progress/*.yaml`, parse each via `parseExecutionManifest`, filter to those whose `claimed_by` is defined AND `!== sessionUlid`, return them in alphabetical ref order (sort by ref). For each, compute `transcriptPath = <targetRepoRoot>/.flow/state/sessions/<staleUlid>/dev-transcript.txt` and stat it to set `hasTranscript`.
...
- [x] **Task 3: Add the `blockOrphanNoTranscript` MCP tool for the no-transcript path** (AC: #3)
  ...
  - [x] 3.4 Unit tests in `plugins/flow/mcp-server/src/tools/__tests__/block-orphan-no-transcript.test.ts` cover: (a) successful move + `blocked_by` stamp, (b) manifest no longer present in `in-progress/<ref>.yaml`, (c) chat line matches AC3's literal shape.
...
- [x] **Task 5: Integration test suite** (AC: #5)
  - [x] 5.1 Add `plugins/flow/mcp-server/src/__tests__/orphan-recovery.test.ts`. Use `tmp` directory fixtures per `beforeEach`; clean up per `afterEach` via `fs.rm({ recursive: true, force: true })`.
...
- [x] **Task 6: Build, vitest, dist** (AC: all)
  ...
  - [x] 6.3 Commit `plugins/flow/mcp-server/dist/` per `CLAUDE.md § Plugin build output is tracked in git`.
...
The transcript file at `<targetRepoRoot>/.flow/state/sessions/<staleUlid>/dev-transcript.txt` is a plain UTF-8 text file. Story 5.10 deliberately did not register a `readDevTranscript` MCP tool (its § Deferred work documents this as a future option if a non-prose caller appears). 5.11 has a prose caller — `start/SKILL.md` — and the built-in `Read` tool is sufficient. Adding an MCP tool would introduce an abstraction with one caller and no second consumer in sight.

### Why the orphan scan runs before every `claimNextStory`, not once per session

A concurrent session running against the same target repo may die between the current session's outer-loop iterations. If the scan only ran at the top of `/flow:start`, a newly-appearing orphan would be invisible until the next `/flow:start` invocation. Running the scan at the top of every outer-loop iteration costs one directory listing per iteration — negligible overhead, much stronger recovery guarantee.
...
- `plugins/flow/mcp-server/src/tools/process-dev-transcript.ts` (Stories 4.3b / 4.5 / 4.6 / 4.8b / 5.10) — NOT touched. The reattach-with-transcript path feeds bytes into this tool unchanged.
- `plugins/flow/mcp-server/src/tools/claim-next-story.ts` (Story 4.3b) — NOT touched. The orphan branch is a separate outer-loop step.
- `plugins/flow/mcp-server/src/tools/run-dev-terminal-action.ts` (Stories 4.4 / 4.8b) — NOT touched. The dev subagent is NOT re-spawned on reattach.
- `plugins/flow/mcp-server/src/tools/complete-story.ts` (Story 4.1) — NOT touched. The `claimed_by` rewrite is performed by `reattachOrphan`, not `completeStory`; the post-rewrite `claimed_by` matches the current session ULID, so `completeStory`'s `WrongClaimantError` check is satisfied when the green branch later runs.
- `plugins/flow/mcp-server/src/lib/managed-fs.ts` (Story 1.6) — NOT touched. The new tools route through the existing `atomicWriteFile` / `writeManagedFile` seams.
- `plugins/flow/mcp-server/src/state/manifest-state-machine.ts` (Story 1.6) — NOT touched. The new tools call `moveBetweenStates` through its existing exports.
- `plugins/flow/mcp-server/src/lib/manifest-io.ts` (Story 4.3b) — NOT touched. The new tools call `readManifest` / `writeManifest` through their existing exports.

### Declared-locked-file changes (explicit exceptions)

- **`plugins/flow/skills/start/SKILL.md`** (Stories 4.2 / 4.3b / 4.3c / 4.6 / 4.6b / 4.7 / 4.8 / 5.10) — Tasks 4.1–4.5. The frontmatter `allowed_tools` array gains `scanOrphanedInProgress`, `reattachOrphan`, `blockOrphanNoTranscript`, and `Read`. A new step 3.5 is inserted in `# Steps` between current step 3 and current step 4. Two invariant blocks are added in `# Inner cycle`. Three new failure-mode entries are appended to `# Failure modes`. All changes are additive; existing prose is not renumbered (step 4 stays step 4 — the new step is "3.5", matching Story 5.10's "4.5" pattern).
- **`plugins/flow/mcp-server/src/tools/register.ts`** (every prior tool-bearing story) — Task 1.4 / 2.3 / 3.3. Three new tool registrations are added. No existing registration is modified.
...
- `plugins/flow/mcp-server/src/tools/scan-orphaned-in-progress.ts` (Task 1) — pure-read scan helper, alphabetical ref order.
- `plugins/flow/mcp-server/src/tools/reattach-orphan.ts` (Task 2) — atomic `claimed_by` rewrite.
- `plugins/flow/mcp-server/src/tools/block-orphan-no-transcript.ts` (Task 3) — atomic move + `blocked_by` stamp.
- `plugins/flow/mcp-server/src/tools/__tests__/scan-orphaned-in-progress.test.ts` (Task 1.5).
- `plugins/flow/mcp-server/src/tools/__tests__/reattach-orphan.test.ts` (Task 2.4).
- `plugins/flow/mcp-server/src/tools/__tests__/block-orphan-no-transcript.test.ts` (Task 3.4).
- `plugins/flow/mcp-server/src/__tests__/orphan-recovery.test.ts` (Task 5) — integration suite covering AC5's five fixtures.

### Files this story will modify

- `plugins/flow/skills/start/SKILL.md` (Task 4 — `allowed_tools`, step 3.5, invariants, failure modes).
- `plugins/flow/mcp-server/src/tools/register.ts` (Tasks 1.4 / 2.3 / 3.3 — three new registrations).
- `plugins/flow/mcp-server/src/skills/__tests__/start-skill-content.test.ts` (Task 6.2 — tool-count assertion update).
...
- **`plugins/flow/skills/start/SKILL.md`** (per Story 5.10's already-shipped state):
...
- **`plugins/flow/mcp-server/src/tools/register.ts`**: register pattern is established (every existing tool follows the same shape). Tasks 1.4 / 2.3 / 3.3 follow the same shape for the three new tools.

- **`plugins/flow/mcp-server/src/tools/claim-next-story.ts`**: read-only context for this story. The orphan branch is SEPARATE from `claimNextStory`. `listClaimableTodos` (the helper that scans `to-do/`) already reports `inProgressCount` — the orphan branch does NOT need to reuse it; it has its own scan via `scanOrphanedInProgress`.

- **State directory layout (read-only context):**
  - `<targetRepoRoot>/.flow/state/to-do/<ref>.yaml` — scanned by `claimNextStory`.
  - `<targetRepoRoot>/.flow/state/in-progress/<ref>.yaml` — scanned by `scanOrphanedInProgress` (this story).
  - `<targetRepoRoot>/.flow/state/blocked/<ref>.yaml` — destination for the no-transcript path.
  - `<targetRepoRoot>/.flow/state/done/<ref>.yaml` — destination for the reattach-with-transcript-and-green-reviewer path (via `completeStory` called internally by `processReviewerTranscript`).
  - `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/dev-transcript.txt` — written by Story 5.10; READ by this story on the reattach path.
  - `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/dev-outcome.json` — written by Story 4.8b; not read by this story.
  - `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/reviewer-result.json` — written by Story 4.6 revision 2; not read by this story.
...
- [Source: `plugins/flow/skills/start/SKILL.md § Steps` step 3 → step 4] — the seam where step 3.5 is inserted.
- [Source: `plugins/flow/skills/start/SKILL.md § Inner cycle` invariants block] — pattern for the new invariants in Task 4.3.
- [Source: `plugins/flow/mcp-server/src/tools/claim-next-story.ts`] — outer-loop seam pattern; the orphan branch is a sibling, not a modification.
- [Source: `plugins/flow/mcp-server/src/tools/list-claimable-todos.ts`] — alphabetical-sort pattern (`yamlEntries.sort()`) reused in `scanOrphanedInProgress`.
- [Source: `plugins/flow/mcp-server/src/tools/complete-story.ts`] — `claimed_by` validation pattern; the post-reattach `claimed_by` must match the current ULID so this tool's check passes when the green branch later runs.
- [Source: `plugins/flow/mcp-server/src/lib/manifest-io.ts`] — `readManifest` / `writeManifest` seams used by the new tools.
- [Source: `plugins/flow/mcp-server/src/lib/managed-fs.ts:115`] — `atomicWriteFile` primitive (Story 1.6); used transitively via `writeManifest`.
- [Source: `plugins/flow/mcp-server/src/state/manifest-state-machine.ts:76`] — `moveBetweenStates` primitive (Story 1.6); used by `blockOrphanNoTranscript`.
- [Source: `plugins/flow/docs/user-surface-acs.md § What counts as a user-surface § (iv)`] — rubric for AC1's `(user-surface)` tag.
...
- Story 5.10 wrote the transcript file at `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/dev-transcript.txt` via Claude Code's built-in `Write` tool, **before** any MCP call, so the file survives an MCP reap. This story READS that file via Claude Code's built-in `Read` tool — the symmetric pattern. The `Read` is from the SKILL.md prose layer, not from inside an MCP tool, so the read is not gated by MCP availability either.
...
- Established the session-directory pattern: `<targetRepoRoot>/.flow/state/sessions/<sessionUlid>/<artefact>.json`. 5.11 reads from the same directory.
...
- The session ULID is minted once per `/flow:start` invocation via `mintSessionUlid`. 5.11 uses this ULID as the `currentSessionUlid` argument throughout the orphan branch.
...
- `plugins/flow/mcp-server/src/tools/scan-orphaned-in-progress.ts` (new)
- `plugins/flow/mcp-server/src/tools/reattach-orphan.ts` (new)
- `plugins/flow/mcp-server/src/tools/block-orphan-no-transcript.ts` (new)
- `plugins/flow/mcp-server/src/tools/__tests__/scan-orphaned-in-progress.test.ts` (new)
- `plugins/flow/mcp-server/src/tools/__tests__/reattach-orphan.test.ts` (new)
- `plugins/flow/mcp-server/src/tools/__tests__/block-orphan-no-transcript.test.ts` (new)
- `plugins/flow/mcp-server/src/__tests__/orphan-recovery.test.ts` (new)
- `plugins/flow/mcp-server/src/errors.ts` (modified — NotAnOrphanError added)
- `plugins/flow/mcp-server/src/tools/register.ts` (modified — 3 new tool registrations)
- `plugins/flow/skills/start/SKILL.md` (modified — allowed_tools, step 3.5, invariants, failure modes)
- `plugins/flow/mcp-server/src/skills/__tests__/start-skill-content.test.ts` (modified — tool count 12 → 16)
- `plugins/flow/mcp-server/src/tools/__tests__/compute-agreement.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/src/tools/__tests__/inner-cycle.integration.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/src/tools/__tests__/run-auto-merge-gate.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/tests/ask-mode-enforcement.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/tests/ask-skill.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/tests/get-team-snapshot.test.ts` (modified — tool count 32 → 35)
- `plugins/flow/mcp-server/tests/canonical-fs-guard.test.ts` (modified — whitelist 4 new test files)
- `plugins/flow/mcp-server/dist/` (rebuilt — all new and modified TS compiled)
