...
server — every MCP call from every Claude Code session against the flow plugin
...
- Resuming `/flow:start` autonomous-drain (currently paused — see memory `project_reviewer_toolchain_gaps`) requires the cascade not to fire mid-drain. D2 is the structural fix; without it, autonomous drain is bounded by the first subagent return.

### What this story does NOT

- (a) Modify any tool-layer code in `plugins/flow/mcp-server/src/tools/`. The shim is transport-layer infrastructure; no tool semantics change.
- (b) Remove the halt seam from Story 5.30 (`plugins/flow/skills/start/SKILL.md` + `McpDisconnectedError`). The seam stays as the safety net for non-cascade MCP disconnects (idle reap, crash, OS kill). D2 only retires the cascade-specific restart cost.
...
- (d) Add a token-handshake or any TLS/auth scheme to the socket. Per Q5's verdict (`socket-auth: filesystem-permission-only`), `0600` on the socket + `0700` on `~/.flow/` is the auth control. Defence-in-depth peer-EUID check via `getsockopt(LOCAL_PEEREUID)` is wired but optional.
...
- (g) Migrate the build to a different bundler. The proxy source lives under `plugins/flow/mcp-proxy/src/` and reuses the existing tsc + `normalise-dist.mjs` pattern that `mcp-server` uses today. No new build tooling.
...
- **Graceful daemon shutdown on operator command.** If an operator wants to manually kill the daemon today, `pkill -f mcp-server/dist/index.js` or `kill $(cat ~/.flow/mcp-daemon.pid)` works. A `/flow:stop-daemon` skill is a v1.2 nicety, not v1.1 baseline.
...
> ACs are reproduced from this story's epic block (`epic-5 § Story 5.32`) with per-AC implementation detail added below each one. AC markers (`artifact:` / `vitest:`) use plain unbacked-tick form per memory `project_reviewer_toolchain_gaps` (entry 1). Each `vitest:` marker resolves to a file under `plugins/flow/mcp-server/src/__tests__/` (unit) or `plugins/flow/mcp-server/tests/` (integration) so the existing `pnpm -F @flow/mcp-server test` script picks them up.
...
so the test can inject:
  ...
  - daemonArgs: string[]
  - socketPath: string (under a per-test tmpdir, not ~/.flow/)
...
Test outline:
  - beforeAll: ensure plugins/flow/mcp-server/dist/index.js exists (otherwise run pnpm build
    OR skip with a clear message — prefer skip to avoid CI flakiness from build timing)
  - beforeAll: ensure plugins/flow/mcp-proxy/bin/mcp-proxy.js exists (same)
  - Test body:
      1. Create per-test tmpdir; export HOME=tmpdir so ~/.flow/ resolves under tmp
      ...
      3. Wait for the proxy to emit a "ready" signal on stdout, OR for the PID file
         (tmpdir/.flow/mcp-daemon.pid) to appear (poll every 100ms, timeout 5s)
...
proxy's ~/.flow/ resolves to tmpdir/.flow/. Otherwise the test will leak state into the real
user's ~/.flow/ and conflict with the real plugin runtime.
...
`plugins/flow/.claude-plugin/plugin.json` is updated so `mcpServers.crew.command` is `${CLAUDE_PLUGIN_ROOT}/mcp-proxy/bin/mcp-proxy.js` (no `args`, no `node` wrapper — the shim file begins with a `#!/usr/bin/env node` shebang and is executable). The `cwd` field stays as `${CLAUDE_PLUGIN_ROOT}`. The manifest file's structure is otherwise unchanged.
...
Verify after edit:
  - JSON is valid (`node -e "JSON.parse(require('fs').readFileSync('plugins/flow/.claude-plugin/plugin.json'))"`)
...
The file `plugins/flow/mcp-proxy/bin/mcp-proxy.js` exists, begins with `#!/usr/bin/env node`, and has mode `0755` (executable bit set; `fs.statSync(path).mode & 0o111` is truthy). The source is checked in under `plugins/flow/mcp-proxy/src/` (TypeScript) and built/normalised into `plugins/flow/mcp-proxy/bin/` per the existing `mcp-server/scripts/normalise-dist.mjs` pattern. The committed `bin/mcp-proxy.js` is the runtime artefact the manifest points at.
artifact: plugins/flow/mcp-proxy/bin/mcp-proxy.js

<!--
Implementation: new sub-package under plugins/flow/mcp-proxy/. Suggested layout:

  plugins/flow/mcp-proxy/
    package.json              — name: @flow/mcp-proxy, type: module, main: bin/mcp-proxy.js
    ...
    src/
      ...
      daemon-paths.ts         — resolves ~/.flow/{mcp-daemon.sock, mcp-daemon.pid, mcp-daemon.lock}
...
proxy source changes. Update plugins/flow/mcp-server/scripts/watch-and-normalise.mjs (the
workspace's build orchestrator) to also watch plugins/flow/mcp-proxy/src/.

Verify after build:
  - `head -1 plugins/flow/mcp-proxy/bin/mcp-proxy.js` is `#!/usr/bin/env node`
  - `stat -f "%Lp" plugins/flow/mcp-proxy/bin/mcp-proxy.js` (darwin) returns 755 (or any
    mode with 0o111 bits set — the executable bit is what AC5 asserts)
  - `node plugins/flow/mcp-proxy/bin/mcp-proxy.js < /dev/null` runs without ImportError or
...
Locked-file note: this story commits new files under plugins/flow/mcp-proxy/. The proxy
package's bin/ output is tracked in git for the same reason mcp-server/dist/ is — /plugin
install copies the tree as-is and won't run a build step. See plugins/flow/docs/README-install.md
§ Build artefacts.
-->

**AC6 (unix socket bound at mode 0600 under ~/.flow/):**
...
<!--
Implementation: a new module `plugins/flow/mcp-server/src/lib/socket-server.ts` exposes
`startSocketServer(opts): Promise<net.Server>` that:
  1. Computes ~/.flow/ path from opts.home (default process.env.HOME)
...
The daemon's main() (plugins/flow/mcp-server/src/index.ts) calls startSocketServer() in place
...
The vitest test at AC6's path:
  ...
  - Asserts:
      ...
      * fs.statSync(`${tmpdir}/.flow/mcp-daemon.sock`).mode & 0o777 === 0o600
...
- [ ] **Task 1: Scaffold the mcp-proxy sub-package** (AC: #5)
  - [ ] 1.1 Create `plugins/flow/mcp-proxy/` with `package.json` (name: `@flow/mcp-proxy`, type: module, main: `bin/mcp-proxy.js`, build script per AC5 implementation comment).
  - [ ] 1.2 Create `plugins/flow/mcp-proxy/tsconfig.json` extending `../tsconfig.base.json`, outDir `bin/`.
  - [ ] 1.3 Create `plugins/flow/mcp-proxy/scripts/normalise-dist.mjs` (copy from `mcp-server/scripts/normalise-dist.mjs`, adapt: rename `index.js` → `mcp-proxy.js`, prepend `#!/usr/bin/env node\n`, `fs.chmodSync(target, 0o755)`).
  - [ ] 1.4 Update `plugins/flow/pnpm-workspace.yaml` to include `mcp-proxy` (verify it already uses `mcp-*` glob; if so, no change needed).
  - [ ] 1.5 Run `pnpm install` from `plugins/flow/` to register the new workspace member.

- [ ] **Task 2: Build the daemon-side socket transport** (AC: #6)
  - [ ] 2.1 Create `plugins/flow/mcp-server/src/lib/socket-server.ts` exporting `startSocketServer(opts: { home?: string }): Promise<net.Server>`.
  ...
  - [ ] 2.3 Create `plugins/flow/mcp-server/src/lib/socket-transport.ts` exporting `SocketServerTransport` — implements the SDK's Transport contract over a `net.Socket`, using the SDK's `ReadBuffer` for framing (line-delimited JSON-RPC per Q3 verdict). Reference: `node_modules/@modelcontextprotocol/sdk/dist/esm/shared/stdio.js`.
...
- [ ] **Task 3: Refactor the daemon's main() to use socket transport** (AC: #6)
  - [ ] 3.1 In `plugins/flow/mcp-server/src/index.ts`, replace the `StdioServerTransport` bootstrap with `startSocketServer({ home: process.env.HOME })`.
  - [ ] 3.2 Write the daemon's pid to `~/.flow/mcp-daemon.pid` after the socket binds successfully. Unlink on `process.on('exit')`.
  ...
  - [ ] 3.4 Remove the `stdin.on('end')` and `stdin.on('close')` handlers that triggered `server.close() + process.exit(0)` — they're irrelevant once stdio is detached. The daemon's clean-shutdown trigger is now SIGTERM via `kill $(cat ~/.flow/mcp-daemon.pid)`, handled by the existing SIGTERM handler.

- [ ] **Task 4: Build the proxy's daemon-acquisition logic** (AC: #2)
  - [ ] 4.1 Create `plugins/flow/mcp-proxy/src/daemon-paths.ts` exporting `resolveDaemonPaths(home?: string): { sockPath, pidPath, lockPath, crewDir }`. Default home: `process.env.HOME`.
  - [ ] 4.2 Create `plugins/flow/mcp-proxy/src/acquire-daemon.ts` exporting `acquireDaemon(opts): Promise<{ socket, daemonPid, spawned }>` per the AC2 implementation comment. The function takes injected `spawn`, `kill`, `connect`, `fs` ports so the AC2 vitest can mock them.
...
- [ ] **Task 5: Build the proxy's byte-forwarding glue** (AC: #1)
  - [ ] 5.1 Create `plugins/flow/mcp-proxy/src/index.ts` — the shim entry. Calls `acquireDaemon()`, then `process.stdin.pipe(socket); socket.pipe(process.stdout)`. Per Q3, the SDK's ReadBuffer on both ends handles framing — the shim is pure byte-forwarding.
...
- [ ] **Task 6: Write the proxy spawn unit test** (AC: #1)
  - [ ] 6.1 Create `plugins/flow/mcp-server/src/__tests__/proxy-spawn.test.ts` — note this lives in mcp-server's tests dir (the workspace uses one vitest config for all plugin tests).
  - [ ] 6.2 Create `plugins/flow/mcp-server/tests/fixtures/echo-daemon.mjs` — tiny daemon that binds the supplied socket, echoes one frame, writes pid to env-supplied file.
...
- [ ] **Task 7: Write the lockfile lifecycle unit test** (AC: #2)
  - [ ] 7.1 Create `plugins/flow/mcp-server/src/__tests__/proxy-lockfile.test.ts`.
...
- [ ] **Task 8: Write the integration test** (AC: #3)
  - [ ] 8.1 Create `plugins/flow/mcp-server/tests/proxy-daemon-survives-sigterm.integration.test.ts`.
...
- [ ] **Task 9: Write the daemon socket mode unit test** (AC: #6)
  - [ ] 9.1 Create `plugins/flow/mcp-server/src/__tests__/daemon-socket-mode.test.ts`.
  - [ ] 9.2 Test asserts ~/.flow/ at 0700, socket at 0600, verify hook wired (listenerCount >= 1).

- [ ] **Task 10: Update the plugin manifest** (AC: #4)
  - [ ] 10.1 Edit `plugins/flow/.claude-plugin/plugin.json` per the AC4 implementation comment diff. Validate JSON via `node -e "JSON.parse(...)"`.

- [ ] **Task 11: Wire proxy into build:watch and top-level scripts** (AC: #5)
  - [ ] 11.1 Update `plugins/flow/mcp-server/scripts/watch-and-normalise.mjs` (or wherever the workspace's build:watch lives) to also watch `plugins/flow/mcp-proxy/src/` and chain its normaliser per Story 5.28's pattern.
  - [ ] 11.2 Run `pnpm -F @flow/mcp-proxy build` once; verify `plugins/flow/mcp-proxy/bin/mcp-proxy.js` exists, has shebang, has mode 0755.
  - [ ] 11.3 Run `pnpm -F @flow/mcp-server build`; verify the daemon still compiles after the socket-transport refactor.
  - [ ] 11.4 Commit both `bin/` outputs (`mcp-proxy/bin/mcp-proxy.js` and `mcp-server/dist/index.js`) along with the source. Per the project rule "Plugin build output is tracked in git" — see `plugins/flow/docs/README-install.md` § Build artefacts.

- [ ] **Task 12: Verify acceptance** (AC: all)
  - [ ] 12.1 `pnpm -F @flow/mcp-server test` — all six new tests pass plus existing suite.
  - [ ] 12.2 `cat plugins/flow/.claude-plugin/plugin.json | jq '.mcpServers.crew.command'` — equals `"${CLAUDE_PLUGIN_ROOT}/mcp-proxy/bin/mcp-proxy.js"`.
  - [ ] 12.3 `head -1 plugins/flow/mcp-proxy/bin/mcp-proxy.js` — equals `#!/usr/bin/env node`.
  - [ ] 12.4 `stat -f "%Lp" plugins/flow/mcp-proxy/bin/mcp-proxy.js` (darwin) — has executable bit set.
  - [ ] 12.5 Smoke-test in a real Claude Code session: install the plugin via `--plugin-dir`, run any MCP tool (e.g., `/flow:status`), confirm the tool call succeeds. Then spawn a subagent Task, return from it, confirm the next MCP tool call still succeeds (cascade no longer kills the daemon).
...
3. **Future portability.** If a different MCP server in the future wants to reuse the proxy pattern (process-group escape on cascade-prone hosts), `@flow/mcp-proxy` can be lifted out as a standalone npm package. Embedded in `mcp-server`, it would need extraction.
...
- `plugins/flow/mcp-server/src/tools/**` — NOT touched. Tool semantics are unchanged; D2 is transport-layer.
- `plugins/flow/skills/start/SKILL.md` — NOT touched. Story 5.30's halt seam stays as written.
- `plugins/flow/skills/**/SKILL.md` (other skills) — NOT touched.
- `plugins/flow/permissions/**` — NOT touched. No new permission allowlist entries; the proxy is internal infrastructure.
- `plugins/flow/example/**` — NOT touched.
- `plugins/flow/catalogue/**` — NOT touched.
...
- `plugins/flow/.claude-plugin/plugin.json` — AC4. Single-line change to `mcpServers.crew.command`, removal of `args` array.
- `plugins/flow/mcp-server/src/index.ts` — Task 3. Refactor from `StdioServerTransport` to `startSocketServer`. Lifecycle log, keepalive, signal handlers stay.
- `plugins/flow/mcp-server/src/lib/socket-server.ts` (new) — Task 2.
- `plugins/flow/mcp-server/src/lib/socket-transport.ts` (new) — Task 2.
- `plugins/flow/mcp-server/src/__tests__/proxy-spawn.test.ts` (new) — AC1, Task 6.
- `plugins/flow/mcp-server/src/__tests__/proxy-lockfile.test.ts` (new) — AC2, Task 7.
- `plugins/flow/mcp-server/src/__tests__/daemon-socket-mode.test.ts` (new) — AC6, Task 9.
- `plugins/flow/mcp-server/tests/proxy-daemon-survives-sigterm.integration.test.ts` (new) — AC3, Task 8.
- `plugins/flow/mcp-server/tests/fixtures/echo-daemon.mjs` (new) — Task 6.2.
- `plugins/flow/mcp-server/scripts/watch-and-normalise.mjs` — Task 11. Extend to watch the proxy's source dir per Story 5.28's chaining pattern.
- `plugins/flow/mcp-proxy/` (new sub-package) — Tasks 1, 4, 5. New `package.json`, `tsconfig.json`, `src/`, `scripts/normalise-dist.mjs`, `bin/mcp-proxy.js` (build output).
- `plugins/flow/pnpm-workspace.yaml` — Task 1.4. Add `mcp-proxy` if the glob doesn't already cover it.
...
- `plugins/flow/mcp-proxy/package.json` — sub-package manifest. name `@flow/mcp-proxy`.
- `plugins/flow/mcp-proxy/tsconfig.json` — extends `../tsconfig.base.json`.
- `plugins/flow/mcp-proxy/src/index.ts` — shim entry, byte-forwards stdio↔socket.
- `plugins/flow/mcp-proxy/src/acquire-daemon.ts` — pure function, hybrid liveness logic.
- `plugins/flow/mcp-proxy/src/daemon-paths.ts` — path resolver honouring HOME env.
- `plugins/flow/mcp-proxy/scripts/normalise-dist.mjs` — post-tsc step: prepend shebang, chmod 0755, rename.
- `plugins/flow/mcp-proxy/bin/mcp-proxy.js` — build output, committed (manifest target).
- `plugins/flow/mcp-server/src/lib/socket-server.ts` — daemon-side socket setup.
- `plugins/flow/mcp-server/src/lib/socket-transport.ts` — SDK-compatible socket transport.
- `plugins/flow/mcp-server/src/__tests__/proxy-spawn.test.ts` — AC1.
- `plugins/flow/mcp-server/src/__tests__/proxy-lockfile.test.ts` — AC2.
- `plugins/flow/mcp-server/src/__tests__/daemon-socket-mode.test.ts` — AC6.
- `plugins/flow/mcp-server/tests/proxy-daemon-survives-sigterm.integration.test.ts` — AC3.
- `plugins/flow/mcp-server/tests/fixtures/echo-daemon.mjs` — test fixture daemon.

### Files this story will modify

- `plugins/flow/.claude-plugin/plugin.json` — AC4. Point manifest at proxy.
- `plugins/flow/mcp-server/src/index.ts` — Task 3. Swap StdioServerTransport for startSocketServer. Remove stdin.end/close handlers (irrelevant once detached).
- `plugins/flow/mcp-server/scripts/watch-and-normalise.mjs` — Task 11. Extend to watch proxy src.
- `plugins/flow/mcp-server/dist/index.js` — rebuild output. Committed per the dist-tracked convention.
- `plugins/flow/pnpm-workspace.yaml` — Task 1.4. Add mcp-proxy if not already covered by glob.
...
- `plugins/flow/mcp-server/src/index.ts` (before edit) — reference for what lifecycle/signal/keepalive code must be preserved across the transport swap.
...
- `plugins/flow/.claude-plugin/plugin.json` (before edit) — reference for what fields survive the AC4 swap.
- `plugins/flow/mcp-server/scripts/normalise-dist.mjs` — pattern reference for the proxy's normaliser script.
- `~/.flow/mcp-lifecycle.log` — observability surface that will validate D2 is working (cascade SIGTERMs should disappear from the log in steady state).
...
5. **Q5 — socket-auth: filesystem-permission-only.** Confirm at Task 2.2 + Task 9 (AC6). Mode 0700 on `~/.flow/`, mode 0600 on the socket. The verify-EUID hook is wired as a no-op; a real `getsockopt` call is deferred.
...
- **Blast radius:** every MCP call from every Claude Code session against the flow plugin flows through the new proxy. If the proxy is broken, the plugin's MCP surface is completely unreachable in every session, immediately on manifest pickup.
...
- All `vitest:` ACs land under `plugins/flow/mcp-server/src/__tests__/` (unit) or `plugins/flow/mcp-server/tests/` (integration), matching the existing convention.
...
- AC6 must not leak into the real `~/.flow/` — always set `process.env.HOME = tmpdir` and assert against `tmpdir/.flow/` paths.
...
- [Source: `plugins/flow/.claude-plugin/plugin.json`] — current manifest; the AC4 diff target.
- [Source: `plugins/flow/mcp-server/src/index.ts`] — current daemon main; Task 3 refactor target.
- [Source: `plugins/flow/mcp-server/scripts/normalise-dist.mjs`] — pattern reference for the proxy normaliser (Task 1.3).
...
- Story 5.30 ships `McpDisconnectedError` and the verbatim `[mcp-cascade-halted]` halt line in `plugins/flow/skills/start/SKILL.md`. This story does NOT modify either; the halt seam stays as the safety net for non-cascade MCP disconnects.
- 5.30 also added lifecycle-log fields `ppid` and `pgid` to every event. After D2 ships, these fields confirm in the log that the daemon is in a different pgid than the host — the operational verification that detachment held. The dev can grep `~/.flow/mcp-lifecycle.log` post-deploy and look for daemon `pgid` ≠ host `pgid`.

### From Story 5.25 (always-on lifecycle logging)

- The lifecycle log is the evidence source that validated the cascade pattern (8/8 paired SIGTERMs). After D2 ships, the cascade pattern should disappear from the log in steady state. The dev should run `/flow:start` once after merge, return from a subagent Task, and grep the log: zero `signal: SIGTERM` events between subagent return and the next MCP call.
...
- The dogfood pause was lifted 2026-05-27 after L1 fixes (5.10/5.11/5.12). The next blocker on resuming `/flow:start` autonomous drain is the cascade — D2 is the fix. After this story merges, the next gate is the install-canary (Epic 7).
