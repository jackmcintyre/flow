...
A locator resolves a proposal id to the single `.flow/retro-proposals/<ISO>.md` file that contains it, returning the file's absolute path, the parsed file, and the matched proposal object. It scans every proposal file in the directory, splits frontmatter, and re-reads each through the canonical `parseRetroProposalFile` parser (never the rendered body). An id that matches no proposal raises a typed `ProposalNotFoundError` naming the id and how many files were scanned. An id that somehow matches in two files raises a typed `AmbiguousProposalIdError` (ids are minted unique; a collision is a bug, not a silent pick-first). A vitest seeds two proposal files with several proposals and asserts: a known id resolves to the right file and proposal, an unknown id raises `ProposalNotFoundError`, and a duplicated id raises `AmbiguousProposalIdError`.
vitest: plugins/flow/mcp-server/src/tools/__tests__/accept-proposal.test.ts
...
**Builds (the gate):** the `acceptProposal` tool, the proposal locator, the two-phase preview/confirm contract, the per-type handler registry + interface, the single-commit apply through the git wrapper, the `applied`-block stamp, the idempotency no-op, the new telemetry event, the typed errors, and the `/flow:accept-proposal` skill.
...
**NEW:**
- `plugins/flow/mcp-server/src/tools/accept-proposal.ts` — the gate tool.
- `plugins/flow/mcp-server/src/lib/proposal-apply-registry.ts` — the `ProposalApplyHandler` interface + registry + `ProposalKindNotApplicableYetError` story-pointer map.
- `plugins/flow/mcp-server/src/lib/locate-proposal.ts` — the id locator (scan + parse + match), or co-locate in `accept-proposal.ts` if cleaner.
- `plugins/flow/mcp-server/src/tools/__tests__/accept-proposal.test.ts` — AC1–AC6.
- `plugins/flow/skills/accept-proposal/SKILL.md` — the operator skill (AC7).

**UPDATE:**
- `plugins/flow/mcp-server/src/schemas/retro-proposal.ts` — add the optional `applied` block to the proposal base (additive).
- `plugins/flow/mcp-server/src/schemas/telemetry-events.ts` — add the `retro.proposal.applied` event variant to the discriminated union (AC5).
- `plugins/flow/mcp-server/src/tools/register.ts` — register `acceptProposal` (AC8). Group with the other retro-path registrations.
- `plugins/flow/mcp-server/src/errors.ts` — add `ProposalNotFoundError`, `AmbiguousProposalIdError`, `ProposalKindNotApplicableYetError` (mirror the existing `RetroProposalAlreadyExistsError` constructor pattern).

### Existing seams to wire into (do not reinvent)

- **Git wrapper:** `gitCommit({ targetRepoRoot, paths, message, role, execaImpl? })` in `plugins/flow/mcp-server/src/lib/git.ts`. Returns `{ commitSha }`. Use `messageShape: "plugin-internal"` with a kebab message (e.g. `accept-proposal: <id>`). It refuses `--force`/`--no-verify` by construction. Inject `execaImpl` in tests.
- **Telemetry:** `logTelemetryEvent({ targetRepoRoot, event })` in `plugins/flow/mcp-server/src/lib/logger.ts`. It stamps `ts`, validates against the union, and appends one JSONL line. Mirror the call shape used by `post-reviewer-comments.ts`.
- **Managed-fs guard:** `writeManagedFile({ absPath, contents, targetRepoRoot, mcpToolContext: { toolName, role } })` in `plugins/flow/mcp-server/src/lib/managed-fs.ts`. The proposal path under `.flow/retro-proposals/**` is canonical, so the `mcpToolContext` is required.
- **Frontmatter helpers:** `splitFrontmatter(raw, sourcePath)` in `plugins/flow/mcp-server/src/lib/markdown-frontmatter.ts`, plus `parse`/`stringify` from the `yaml` package (`lineWidth: 0` for byte-stable output).
- **Proposal parse + types:** `parseRetroProposalFile` and `RetroProposal` in `plugins/flow/mcp-server/src/schemas/retro-proposal.ts`. The locator pattern to mirror is the done-manifest scan in `plugins/flow/mcp-server/src/tools/gather-retro-inputs.ts` (list a dir, parse each file, collect).
- **Skill shape:** mirror `plugins/flow/skills/retro/SKILL.md` (frontmatter `name`, `description`, `allowed_tools`; prose body that calls the MCP tool by name; a status check first).
- **Idempotency precedent:** the manifest-marker no-op in `plugins/flow/mcp-server/src/tools/process-reviewer-transcript.ts` (read a persisted marker, early-return if present) is the pattern for AC4.

### Edge cases worth surfacing in dev/review

- **Empty/absent retro-proposals dir.** Locating any id when `.flow/retro-proposals/` is empty or missing must raise `ProposalNotFoundError`, not crash — the operator gets a clean message.
...
- Code change touching tool + schema + telemetry seams: rebuild and commit `dist/` in the same change; run the full `pnpm build` and `pnpm test` from `plugins/flow/mcp-server` green before opening the PR. Keep the diff scoped to the files above.
- Do not write or edit any execution manifest or `.flow/state` file — the tools own that ledger. The only `.flow` surface this story writes is `.flow/retro-proposals/<ISO>.md` (the `applied` stamp) and `.flow/telemetry` (via the logger).
