...
**Docs-only — do NOT write any code or tests.** Create exactly one new file, `plugins/flow/docs/risk-tiering-worked-examples.md`, with real, accurate prose covering both ACs. The authoritative rules live in `plugins/flow/docs/risk-tiering.md` and the gate decision table in `plugins/flow/mcp-server/src/lib/auto-merge-gate.ts` — read them for ground truth. Do not modify any `.ts` file, the build output (`dist/`), the execution manifest, or any `.flow/state` file — the PR's diff must contain only the new `.md` file (this keeps it classified `low`-risk). No build step is needed for a docs-only change.
