...
**Docs-only — do NOT write any code or tests.** Create exactly one new file, `plugins/flow/docs/drain-result-and-exit-reasons.md`, with real, accurate prose covering both ACs. The return shape and exit reasons are defined at the bottom of `plugins/flow/workflows/drain.workflow.js` (the `return { ... }` object and the loop's `drainedReason` assignments) — read it for ground truth. Do not modify any `.ts` file, the build output (`dist/`), the execution manifest, or any `.flow/state` file — the PR's diff must contain only the new `.md` file (this keeps it classified `low`-risk). No build step is needed for a docs-only change.
