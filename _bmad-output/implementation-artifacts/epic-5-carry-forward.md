...
**Touch site:** `plugins/flow/mcp-server/src/tools/scan-sources.ts` — the `readFile` call in the to-do branch (around lines 579-590) currently lets errors propagate. Lower-severity recovery would be to warn and skip the manifest rather than abort the whole scan pass.
...
**Touch site:** `plugins/flow/mcp-server/src/tools/scan-sources.ts` (search for `renderScanResult`). Add a test-side assertion that rendered output lines have no leading whitespace (cosmetic guarantee for terminal output).
...
**Fold into:** Epic 5 backlog only if `/flow:start` on a real story (Phase 3) surfaces a defect; otherwise defer to Epic 6→7 boundary.
**Unscheduled.**
...
**Issue:** `/flow:start`'s orphan-recovery branch only handled the dev-incomplete + replay shape; when an orphan was found with dev already shipped (PR open and green) and only the reviewer side missing, the loop fell through to Path B manual closeout instead of respawning just the reviewer.
...
**Touch site:** new MCP tool `markStoryShipped(ref: string)` in `plugins/flow/mcp-server/src/tools/`. Atomic write semantics; consumed by an operator-facing slash command (e.g. `/flow:mark-shipped <ref>`) or surfaced as a one-shot CLI helper.
...
**Touch site:** `plugins/flow/mcp-server/src/adapters/bmad/parse-bmad-story.ts` and `plugins/flow/mcp-server/src/adapters/native/parse-native-story.ts` — replace chain-of-regexes with markdown-AST extraction (remark/mdast or equivalent). Drop-in: same `parseSourceStory` interface and return shape.
...
**Touch site:** `plugins/flow/mcp-server/src/tools/run-reviewer-session.ts § runVitestCheck`. Hardcoded `cwd: targetRepoRoot`.
...
**Fold into:** sequencing decision for next planning round. Entries 13 + 14 should land before any further Epic 6 stories ship via /flow:start. Entry 7 (marker classifier) is partially mitigated by spec authoring discipline (the 6.1/6.2/6.3 specs already use clean markers); a permanent fix is its own substrate story.
...
**Workaround applied:** `git restore plugins/flow/mcp-server/dist/` + kill watchers before retrying preflight. Same as the pre-5.24 workaround; recurs every time `build:watch` runs.
