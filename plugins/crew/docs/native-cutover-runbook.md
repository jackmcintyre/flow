...
`.crew/config.yaml` is an operator act done **only when the backlog is reconciled** —
...
- **The cutover is one config line.** `resolveWorkspace`
  ...
  `.crew/config.yaml`. Setting it to `native` binds the native adapter; setting
...
- **The board and claim paths don't care about the ref format.**
  `getBacklogDashboard` → `readBacklogInventory` reads `.crew/state/**` and
  `.crew/native-stories/` uniformly; `claimNextStory` reads `.crew/state/to-do/`
...
Tier-0 validator, writes the survivors to `.crew/native-stories/<ULID>.md`, and
...
3. **Verify the reconciliation count.** Confirm the native-stories directory is a
   ...
   checking — **count it.**
4. **Check for in-flight work.** Any story currently in `.crew/state/in-progress/`
...
Set the active adapter to native in `.crew/config.yaml`:
...
> detection.** With both `.crew/native-stories/` and a BMad stories tree present,
...
Run `/crew:scan`. The native adapter projects each `.crew/native-stories/<ULID>.md`
into an execution manifest under `.crew/state/to-do/`. Scan is idempotent and
...
`/crew:ready` to mark each story you want the drain to pick up as `ready: true`.

### 6. Verify the board and the drain run on native

This is the proof that the live pipeline runs end-to-end on native:

- **Board:** run `/crew:board`. It should render the backlog **grouped by epic
...
`adapter: bmad` back in `.crew/config.yaml` and BMad is the live adapter again,
