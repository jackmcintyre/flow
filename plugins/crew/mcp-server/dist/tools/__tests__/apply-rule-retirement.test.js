...
import { RuleNotFoundError, RetirementWouldEmptyRegistryError, } from "../../errors.js";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
async function seedStandards(contents) {
...
function retirementProposalObj(id, targetRuleId, action, opts = {}) {
    return {
...
function retirementProposal(id, targetRuleId, action, opts = {}) {
    return retirementProposalObj(id, targetRuleId, action, opts);
}
// ---------------------------------------------------------------------------
...
describe("makeRuleRetirementApplyHandler — retire (AC4)", () => {
    it("removes the target rule, preserving the other rule + comments", async () => {
        ...
        const result = await handler.apply(retirementProposal(ULID_PROP_RETIRE, RULE_ID_A, "retire"), { targetRepoRoot: tmpRoot, role: "operator" });
        // Returns both changed paths.
    ...
    it("regenerates docs/standards.md to match the updated registry", async () => {
        ...
        await seedStandards(MINIMAL_STANDARDS);
        const handler = makeRuleRetirementApplyHandler({ standardsNow: () => FIXED_NOW });
        await handler.apply(retirementProposal(ULID_PROP_RETIRE, RULE_ID_A, "retire"), { targetRepoRoot: tmpRoot, role: "operator" });
    ...
    it("previewDiff renders a diff and writes nothing", async () => {
        ...
        const diff = await handler.previewDiff(retirementProposal(ULID_PROP_RETIRE, RULE_ID_A, "retire"), { targetRepoRoot: tmpRoot, role: "operator" });
...
describe("makeRuleRetirementApplyHandler — relax (AC4)", () => {
    it("demotes the target rule's level to advisory, leaving other rules intact", async () => {
        ...
        await seedStandards(MINIMAL_STANDARDS);
        const handler = makeRuleRetirementApplyHandler({ standardsNow: () => FIXED_NOW });
        const result = await handler.apply(retirementProposal(ULID_PROP_RELAX, RULE_ID_B, "relax"), { targetRepoRoot: tmpRoot, role: "operator" });
        expect(result.changedPaths).toContain(REGISTRY_REL);
        expect(result.changedPaths).toContain("docs/standards.md");
        const after = await readRegistry();
        const { data } = parseRuleRegistry(after);
    ...
    it("all rules in the registry pass DisciplineRuleSchema after a relax", async () => {
        ...
        await seedStandards(MINIMAL_STANDARDS);
        const handler = makeRuleRetirementApplyHandler({ standardsNow: () => FIXED_NOW });
        await handler.apply(retirementProposal(ULID_PROP_RELAX, RULE_ID_B, "relax"), { targetRepoRoot: tmpRoot, role: "operator" });
...
describe("makeRuleRetirementApplyHandler — error cases (AC4)", () => {
    it("raises RuleNotFoundError when target_rule_id does not match any rule — no mutation (AC4)", async () => {
        ...
        await expect(handler.apply(retirementProposal(ULID_PROP_MISSING, NON_EXISTENT_RULE_ID, "retire"), { targetRepoRoot: tmpRoot, role: "operator" })).rejects.toBeInstanceOf(RuleNotFoundError);
    ...
    it("RuleNotFoundError carries the missing rule id (AC4)", async () => {
        ...
        await handler
            .apply(retirementProposal(ULID_PROP_MISSING, MISSING_ID, "retire"), { targetRepoRoot: tmpRoot, role: "operator" })
            .catch((err) => {
    ...
    it("raises RetirementWouldEmptyRegistryError when retiring the last rule — no mutation (AC4)", async () => {
        ...
        const beforeRegistry = await readRegistry();
        const handler = makeRuleRetirementApplyHandler({ standardsNow: () => FIXED_NOW });
        await expect(handler.apply(retirementProposal(ULID_PROP_LAST, RULE_ID_ONLY, "retire"), { targetRepoRoot: tmpRoot, role: "operator" })).rejects.toBeInstanceOf(RetirementWouldEmptyRegistryError);
        // Registry is byte-identical — no mutation.
        expect(await readRegistry()).toBe(beforeRegistry);
    });
    it("relax on the last rule is allowed (only retire is blocked)", async () => {
        ...
        const result = await handler.apply(retirementProposal(ULID_PROP_RELAX, RULE_ID_ONLY, "relax"), { targetRepoRoot: tmpRoot, role: "operator" });
        expect(result.changedPaths).toHaveLength(2);
        const { data } = parseRuleRegistry(await readRegistry());
        expect(data.rules).toHaveLength(1);
