 ...
 */
function labelForRef(label, ref) {
    if (!ref)
        return true;
...
async function runDrain(opts = {}) {
    ...
    const refOfLabel = (label) => queue.map((s) => s.ref).find((r) => label.includes(r));
    ...
    const seamResult = (label) => {
        ...
        if (label === "orphan-scan")
            return { orphans: [] };
        if (label.startsWith("clean-root-guard:"))
            return { dirty: false };
        ...
        if (label.startsWith("progress-start:")) {
            ...
            return drainPhaseStart({ ref: refOfLabel(label) ?? "x", phase: phase });
        }
...
function prNumberForRef(ref) {
    let h = 0;
    for (const c of String(ref ?? ""))
 ...
 */
function assertHonestyInvariant(result, thrown, claimedRefs) {
    ...
    const mergedRefs = new Set(result.merged.map((m) => m.ref));
    const pausedRefs = new Set(result.pausedForHuman.map((p) => p.ref));
    const blockedRefs = new Set(result.blocked.map((b) => b.ref));
    ...
    for (const ref of claimedRefs) {
        ...
        const terminalCount = Number(gated) + Number(completedNoGate) + Number(isBlocked);
        ...
        if (pausedRefs.has(ref)) {
            const p = result.pausedForHuman.find((x) => x.ref === ref);
            expect(typeof p.reason).toBe("string");
            expect(p.reason.length).toBeGreaterThan(0);
        }
        if (isBlocked) {
            const b = result.blocked.find((x) => x.ref === ref);
...
describe("drain fault-injection — honest reporting under misbehaving seams (native:01KT19N3H7WZCF1SKQSWDGARF4)", () => {
    ...
    it("AC2: a garbled VERDICT relay buckets that story in blocked with the verdict-step reason, run keeps going", async () => {
        ...
        const a = result.blocked.find((b) => b.ref === REF_A);
        ...
        expect(result.pausedForHuman.some((p) => p.ref === REF_B)).toBe(true);
        // And the run drained.
        expect(result.drainedReason).toBe("queue-drained");
        assertHonestyInvariant(result, thrown, [REF_A, REF_B]);
    });
    it("AC2: a garbled GATE relay (after a green verdict) buckets that story in pausedForHuman with the gate-step reason, run keeps going", async () => {
        ...
        const a = result.pausedForHuman.find((p) => p.ref === REF_A);
        ...
        expect(result.merged.some((m) => m.ref === REF_A)).toBe(false);
    ...
    it("AC3: under concurrency, one story's hard failure blocks ONLY it (worker-threw) — the sibling completes and the run never aborts", async () => {
        ...
        expect(result.completed).toContain(REF_B);
        expect(result.pausedForHuman.some((p) => p.ref === REF_B)).toBe(true);
        expect(result.blocked.some((b) => b.ref === REF_B)).toBe(false);
    ...
    it("AC4: a story that genuinely needs a decision pauses for the human AND delivers a notify payload carrying the ref + verbatim question", async () => {
        ...
        expect(result.merged.some((m) => m.ref === REF)).toBe(false);
    ...
    it("AC5: a failing INFORMATIONAL status seam drops only its line — buckets + reasons are identical to a clean run", async () => {
        ...
        const isProgressLine = (l) => /(dev-build|review|gate): (start|done)/.test(l);
        expect(clean.logs.filter(isProgressLine).length).toBeGreaterThan(0);
        expect(degraded.logs.filter(isProgressLine).length).toBe(0);
        const stripObs = (logs) => logs.filter((l) => !isProgressLine(l) && !PROGRESS_LABEL.test(l));
