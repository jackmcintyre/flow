...
describe("writeNativeStory AC4 (Story 10.3) — writable-time Tier-0 gate", () => {
    it("rejects a cited source that does not resolve on disk (T0-5), nothing written", async () => {
        let caught;
        try {
            await writeNativeStory(candidate10_2({ cited_sources: ["src/parser.ts", "src/does-not-exist.ts"] }));
        }
        catch (err) {
            caught = err;
        }
    ...
    it("rejects an invented-flag verification target (pure part of T0-6), nothing written", async () => {
        let caught;
        try {
            await writeNativeStory(candidate10_2({
                acceptance_criteria: [
                    {
                        text: "**Given** a state, **When** an action, **Then** an outcome.",
                        kind: "unit",
                        // An invented flag — not a path. T0-6 rejects this shape.
                        verification: { type: "vitest", target: "vitest --grep foo" },
                    },
                    {
                        text: "**Given** a system, **When** integrated, **Then** an artifact appears.",
                        kind: "integration",
            ...
            await writeNativeStory(candidate10_2({
                acceptance_criteria: [
                    ...
                    },
                    {
                        ...
                        verification: { type: "artifact", target: "build/missing.json" },
                    },
                ],
            }));
        }
            ...
            acceptance_criteria: [
                {
                    text: "**Given** a state, **When** an action, **Then** an outcome.",
                    kind: "unit",
                    verification: { type: "vitest", target: "src/__tests__/brand-new-a.test.ts" },
                },
                {
                    text: "**Given** a system, **When** integrated, **Then** an outcome.",
                    kind: "integration",
                    verification: { type: "vitest", target: "src/__tests__/brand-new-b.test.ts" },
                },
            ],
        }));
        expect(result.ref).toMatch(/^native:/);
