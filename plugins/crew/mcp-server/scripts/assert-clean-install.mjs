...
const PLUGIN_ROOT = path.resolve(MCP_SERVER, ".."); // plugins/crew
// A target for getStatus. The crew repo root is two up from the plugin root; its
// .crew/ is gitignored, so getStatus may return a typed adapter error in a fresh
...
const work = await mkdtemp(path.join(tmpdir(), "crew-clean-install-"));
try {
  const dest = path.join(work, "crew");
