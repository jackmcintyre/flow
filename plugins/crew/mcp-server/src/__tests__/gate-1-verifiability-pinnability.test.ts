...
describe("Wording anchor — gate-1.workflow.js LENS_RUBRIC.verifiability", () => {
  it("contains the pinnability-once-built instruction (not the present-tense code-exists check)", async () => {
    ...
    expect(content).toContain(
      "once the proposed work is built, could a correctly-written test be made to fail if the described behaviour were missing",
    );
...
const PINNABLE_ONCE_BUILT_DRAFT: JudgeDraft = {
...
const UNPINNABLE_IN_PRINCIPLE_DRAFT: JudgeDraft = {
...
const EXISTING_CODE_BUG_PIN_DRAFT: JudgeDraft = {
...
function makeVerifiabilityRunner(opts: {
  verifiabilityPass: boolean;
  verifiabilityMissed: string;
}): JudgeRunner {
  return async ({ lens, role, draft }) => {
    const pass = lens === "verifiability" ? opts.verifiabilityPass : true;
    const missed =
      lens === "verifiability" ? opts.verifiabilityMissed : "nothing missed";
...
describe("AC1 — pinnable-once-built plan is APPROVED; unpinnable-in-principle plan is REJECTED", () => {
  it("(regression-of-the-bug) pinnable-once-built additive plan: Verifiability PASSES", async () => {
    ...
    const runner = makeVerifiabilityRunner({
      ...
      verifiabilityMissed:
        "nothing missed — the AC describes behaviour pinnable once built: the test fails before the gate is added and passes after",
    });
  ...
  it("unpinnable-in-principle plan: Verifiability REJECTS and the missed reason names in-principle unprovability", async () => {
    ...
    const missedReason =
      'AC1 uses pattern "status.*\\"failed\\""  — a string-presence check that can never be made to fail even in principle if the behaviour is wrong; it proves nothing about whether the write used the correct status value';
...
describe("AC2 — existing-code bug-pin plan stays APPROVED", () => {
  it("a plan that pins a flaw in already-existing behaviour is APPROVED", async () => {
    ...
    const runner = makeVerifiabilityRunner({
      verifiabilityPass: true,
      verifiabilityMissed:
        "nothing missed — AC1 exercises the real claim path end-to-end with real fixtures; the assertion fails today (withdrawn filter missing) and passes once the fix is applied",
    });
...
describe("AC3 — a success condition that is unmet before the change and met after is treated as sound", () => {
  it("a behaviour-pinning check described as 'fails before, passes after' is NOT faulted as 'holding equally before and after'", async () => {
    ...
    const runner = makeVerifiabilityRunner({
      verifiabilityPass: true,
      verifiabilityMissed:
        "nothing missed — the AC describes a check that fails before the safety gate exists and passes after it; that is the definition of behaviour-pinning",
    });
...
describe("AC4 — when a plan is rejected, the reason names in-principle unprovability, not missing not-yet-written code", () => {
  it("the rejection reason for an unpinnable plan cites unprovability in principle", async () => {
    const missedReason =
      "AC uses a string-presence pattern — this success condition can never be pinned even in principle; green forever regardless of whether the behaviour is correct";
  ...
  it("a pinnable-once-built plan that FAILS is rejected for not being truly pinnable, not for code absence", async () => {
    ...
    const missedReason =
      "AC1 asserts the tool is called but does not assert the outcome changes — the test would pass even if the gate returned incorrect results; not truly behaviour-pinning";
