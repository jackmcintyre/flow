...
import { StandardsDocSchema, type Criterion } from "../schemas/standards-doc.js";
 ...
 */
function readCapFromSchema(): number {
  ...
  const maxLength = (criteriaField._def as unknown as { maxLength?: { value: number } })
    .maxLength;
...
export const STANDARDS_CRITERIA_CAP: number = readCapFromSchema();
...
export function bumpPatchVersion(version: string): string {
  ...
  if (parts.length !== 3) {
    throw new Error(
      `bumpPatchVersion: expected a semver string 'x.y.z', got '${version}'`,
    );
  }
  const patch = parseInt(parts[2]!, 10);
  if (Number.isNaN(patch)) {
    throw new Error(
      `bumpPatchVersion: patch segment '${parts[2]}' is not a number in '${version}'`,
    );
  }
 ...
 */
export async function regenerateStandards(
  opts: RegenerateStandardsOptions,
): Promise<void> {
  const { registry, targetVersion, updatedTimestamp, targetRepoRoot, mcpToolContext } =
    opts;
  ...
  for (const name of names) {
    if (seen.has(name)) {
      throw new Error(
        `regenerate-standards: duplicate criterion name '${name}' in projection. ` +
  ...
  if (!result.success) {
    throw new Error(
      `regenerate-standards: projected doc failed StandardsDocSchema validation: ` +
