...
describe("readBacklogInventory (9) — includeSpecText + ref single-item fetch", () => {
  ...
  async function buildRepo(): Promise<string> {
    ...
    await atomicWriteFile(
      path.join(root, ".crew", "native-stories", `${SPEC_ULID}.md`),
      SPEC_CONTENT,
    );
    const manifest = {
    ...
    await atomicWriteFile(
      path.join(root, ".crew", "state", "to-do", `native:${SPEC_ULID}.yaml`),
      yamlStringify(manifest),
    );

    // A native-source-only story (no manifest).
    await atomicWriteFile(
      path.join(root, ".crew", "native-stories", `${SOURCE_ONLY_ULID}.md`),
      SOURCE_ONLY_CONTENT,
    );
    return root;
  }
