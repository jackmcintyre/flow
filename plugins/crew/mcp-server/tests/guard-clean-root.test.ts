 ...
 *   - operational `.crew/state/**` is never stashed (it is the ledger's domain);
...
beforeEach(async () => {
  repo = await fs.mkdtemp(path.join(os.tmpdir(), "crew-guard-clean-root-"));
  await execa("git", ["-C", repo, "init", "-b", "main"]);
  await git(["config", "user.email", "test@example.com"]);
  await git(["config", "user.name", "Test"]);
  await git(["config", "commit.gpgsign", "false"]);
  // `.crew/` is gitignored — exactly as in crew's own repo — so operational ledger
  // state never shows in `git status` and the guard never stashes it. A committed
  // tracked file gives us a clean HEAD baseline.
  await fs.writeFile(path.join(repo, ".gitignore"), ".crew/\n", "utf8");
...
describe("guardCleanRoot — clean-root guard (Fix 2b)", () => {
  ...
  it("stashes leaked tracked+untracked paths, leaves .crew/state alone, restores a clean base", async () => {
    ...
    await fs.mkdir(path.join(repo, ".crew", "state", "to-do"), { recursive: true });
    await fs.writeFile(path.join(repo, ".crew", "state", "to-do", "x.yaml"), "ref: x\n", "utf8");

    const result = await guardCleanRoot({ targetRepoRoot: repo, ref: "native:01TEST" });

    expect(result.dirty).toBe(true);
    expect(result.stashed).toBe(true);
    // Exactly the leaked source paths — the .crew/state ledger file is excluded.
    expect([...result.paths].sort()).toEqual(["leaked.txt", "src.txt"]);
    expect(result.paths).not.toContain(".crew/state/to-do/x.yaml");
    ...
    await expect(
      fs.stat(path.join(repo, ".crew", "state", "to-do", "x.yaml")),
    ).resolves.toBeTruthy();

    // The work is recoverable — one labelled stash entry.
    const stashList = await git(["stash", "list"]);
    expect(stashList).toContain("crew-drain clean-root guard: native:01TEST");
  });
  ...
  it("ignores a root dirty ONLY with .crew/state changes (ledger churn is not a leak)", async () => {
    await fs.mkdir(path.join(repo, ".crew", "state", "to-do"), { recursive: true });
    await fs.writeFile(path.join(repo, ".crew", "state", "to-do", "x.yaml"), "ref: x\n", "utf8");
