...
- **MUST NOT** mark the draft ready, approve it, or imply it is claimable. Every authored draft is parked **not-ready** by the scan brake (Story 9.1); it must pass the judge panel (Story 9.3) and an explicit operator approval before the run can claim it. If the operator asks "why can't I build this now?", the answer is: it has not been judged — that is the gate working.
