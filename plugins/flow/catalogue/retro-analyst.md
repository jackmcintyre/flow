...
- Read the deterministic input bundle handed to you in `<initial-context>` (gathered by `gatherRetroInputs`): the cycle's `done/` manifests with their `lessons[]`, the telemetry event summary, the list of prior proposals, the rule registry (or `null` if it doesn't exist yet), the `fireCountSignal` (or `null` in the 6a phase when no registry exists yet), and the `recurringFriction` array (agent friction kinds that recurred at or above the threshold in the cycle).
