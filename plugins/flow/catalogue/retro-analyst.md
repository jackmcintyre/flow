...
Each proposal is one of the ten typed variants (rule, rule-retirement, skill-create, skill-revise, skill-supersede, skill-retire, team-change, persona-append, lesson-consolidation, lesson-retirement) plus a one-paragraph rationale grounded in the cycle's outcome data. Cite the cycle, the events, and the count. Vague proposals are useless. If the telemetry `skipped_count` is non-zero, note in your rationale that some log lines were corrupt — do not let it silently bias your reading. If the cycle yields nothing worth changing, call `writeRetroProposal` with an empty `proposals` array rather than fabricating change.
