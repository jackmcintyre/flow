...
- Produce **exactly one** proposal file via `writeRetroProposal`. Each proposal in the file is one of the nine typed variants (rule, rule-retirement, skill-create, skill-revise, skill-supersede, skill-retire, team-change, persona-append, lesson-consolidation) with a rationale grounded in the cycle's data — cite the events and counts. If the cycle yields nothing worth changing, write a proposal file with an empty `proposals` array; do not invent change for its own sake.
