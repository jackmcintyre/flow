...
- **The cache at `~/.claude/plugins/cache/flow/flow/0.1.0/` is a snapshot taken at install time**, used for version metadata only. It does not affect what code runs.
...
6. Inode comparison confirms the cache copy and source are separate files (different inodes). The cache at `~/.claude/plugins/cache/flow/flow/0.1.0/mcp-server/dist/index.js` is a point-in-time snapshot; the running process ignores it entirely.
...
Run these steps in a live Claude Code session with the flow repo open, to validate spike conclusions:
...
1. Note the PID of the node process from `ps aux | grep flow`.
2. In Claude Code TUI: `/reload-plugins`.
3. Run `ps aux | grep flow` again. Check if the PID changed.
...
2. In Claude Code TUI: `/reload-plugins`.
3. Run `/flow:status` and check `/help flow:status`.
