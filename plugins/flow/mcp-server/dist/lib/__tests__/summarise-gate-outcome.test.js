...
import { summariseGateOutcome, } from "../summarise-gate-outcome.js";
describe("summariseGateOutcome", () => {
    ...
    it("renders an auto-merged outcome as a one-line summary", () => {
        ...
        expect(summariseGateOutcome(outcome)).toBe("bmad:8.11 PR#211: auto-merged (low-risk-met-threshold)");
    });
    ...
    it("renders a paused outcome with the 'paused for human' wording", () => {
        ...
        expect(summariseGateOutcome(outcome)).toBe("bmad:8.4 PR#200: paused for human (ci-not-green)");
    });
    ...
    it("uses the outcome word from `merged`, independent of the decision field", () => {
        ...
        expect(() => summariseGateOutcome(outcome)).not.toThrow();
        expect(summariseGateOutcome(outcome)).toBe("bmad:8.99 PR#0: paused for human ()");
    });
});
