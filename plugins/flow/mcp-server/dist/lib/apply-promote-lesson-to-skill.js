...
import { writeNewSkill, } from "./apply-skill-proposal.js";
 ...
 * `## Skills` section body.
 */
export function serialiseSkillRef(ref) {
 ...
 */
export function extractSkillRefs(skillsBody) {
    const refs = [];
    for (const line of skillsBody.split("\n")) {
        const trimmed = line.trimStart();
        if (!trimmed.startsWith(SKILL_REF_BLOCK_PREFIX) ||
 ...
 * Exported for unit testing.
 */
export function appendSkillRef(existingBody, ref) {
 ...
 * Exported for unit testing.
 */
export function extractSkillsSection(raw) {
 ...
 */
function reconstructPersonaFileWithSkills(parsed, newKnowledgeBody, newSkillsBody) {
 ...
 */
export function makePromoteLessonToSkillHandler(deps) {
    return {
        ...
        async previewDiff(proposal, ctx) {
            ...
            const lines = [];
            lines.push(`# promote-lesson-to-skill proposal ${proposal.id}`);
            lines.push(``);
            if (raw === null) {
            ...
            lines.push(`Effect 1 — create skill: ${proposal.proposed_skill_path}`);
            lines.push(`    name: ${skillName}`);
            lines.push(`    description: ${proposal.skill_description}`);
            lines.push(`    version: 0.1.0`);
            lines.push(``);
            lines.push(`Effect 2 — append skill reference to ${relPath} ## Skills section:`);
        ...
        async apply(proposal, ctx) {
            ...
            const newContents = reconstructPersonaFileWithSkills(parsed, parsed.sections.Knowledge, newSkillsBody);
 ...
 */
function assertPromoteLessonProposal(proposal) {
    if (proposal.type !== "promote-lesson-to-skill") {
        throw new Error(`promote-lesson-to-skill apply handler received a proposal of type '${proposal.type}'; ` +
