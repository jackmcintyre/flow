...
function makeLesson(id, useCount, lastUsedAt) {
    return {
        id,
        kind: "pattern",
...
describe("buildPersonaSpawnPrompt — briefing budget cap (AC1)", () => {
    ...
    it("does not rewrite the persona file when no overflow exists", async () => {
        ...
        const statBefore = await fs.stat(path.join(tmpRoot, "team", "generalist-dev", "PERSONA.md"));
        await buildPersonaSpawnPrompt({
            targetRepoRoot: tmpRoot,
            role: "generalist-dev",
            briefingBudget: 10,
        ...
        const statAfter = await fs.stat(path.join(tmpRoot, "team", "generalist-dev", "PERSONA.md"));
