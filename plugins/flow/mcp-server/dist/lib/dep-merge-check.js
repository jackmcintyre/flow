...
export async function isDependencyPrMerged(opts) {
    ...
    try {
        const result = await execaImpl("gh", ["pr", "list", "--head", branch, "--state", "merged", "--json", "number", "--limit", "1"], { cwd: opts.targetRepoRoot });
...
export async function areDependenciesMerged(opts) {
    ...
    for (const dep of opts.deps) {
        ...
        let title;
        try {
            const manifest = parseExecutionManifest(yamlParse(raw), { absPath: depPath });
