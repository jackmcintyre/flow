 ...
 * `<parent>/.flow-worktrees/<sessionUlid>/dev-<ref>-worktree`, NOT nested inside
...
export function devStoryWorktreesRoot(targetRepoRoot, sessionUlid) {
    return path.join(path.dirname(targetRepoRoot), ".flow-worktrees", sessionUlid);
}
 ...
 * only those under the dev-story worktrees parent (`<parent>/.flow-worktrees/`)
...
export async function reapStaleDevStoryWorktrees(opts) {
    ...
    //   <parent>/.flow-worktrees/<sessionUlid>/dev-<ref>-worktree
    ...
    const worktreesParent = path.join(path.dirname(canonRoot), ".flow-worktrees");
