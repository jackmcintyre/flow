 ...
 */
export function extractLessonsFromBody(body) {
    const lessons = [];
 ...
 * Exported for unit testing.
 */
export function rankLessons(body, budget = DEFAULT_BRIEFING_BUDGET) {
 ...
 */
export function serialiseLessonBlock(lesson) {
    const obj = {
        id: lesson.id,
        kind: lesson.kind,
    ...
    if (lesson.use_count !== undefined)
        obj["use_count"] = lesson.use_count;
 ...
 * Exported for unit testing.
 */
export function demoteLessonsFromBody(body, overflowIds) {
            ...
            try {
                ...
                if (raw !== null &&
                    ...
                    if (overflowIds.has(id))
                        continue; // Demote — skip this line.
                }
            }
            catch {
 ...
 */
export async function archiveLessons(targetRepoRoot, role, lessons, now = () => new Date()) {
    const changedPaths = [];
 ...
 * Exported for unit testing.
 */
export function rebuildBodyWithTopLessons(originalBody, topLessons) {
...
export async function findArchivedLessonById(targetRepoRoot, role, id) {
    ...
    if (typeof obj["use_count"] === "number")
        archived.use_count = obj["use_count"];
    if (typeof obj["last_used_at"] === "string")
        archived.last_used_at = obj["last_used_at"];
    if (typeof obj["failure_class"] === "string")
        archived.failure_class = obj["failure_class"];
    if (typeof obj["source_ref"] === "string")
        archived.source_ref = obj["source_ref"];
    if (typeof obj["source_pr"] === "string")
        archived.source_pr = obj["source_pr"];
    return archived;
}
