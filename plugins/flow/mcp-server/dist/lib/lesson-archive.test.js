...
import { extractLessonsFromBody, rankLessons, demoteLessonsFromBody, archiveLessons, findArchivedLessonById, serialiseLessonBlock, } from "./lesson-archive.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("rankLessons", () => {
    ...
    it("uses DEFAULT_BRIEFING_BUDGET (10) when no budget is specified", () => {
        const lessons = Array.from({ length: 15 }, (_, i) => makeLesson({ id: `01KT000000000000000000${String(i).padStart(4, "0")}`, use_count: i }));
...
describe("archiveLessons + findArchivedLessonById (integration)", () => {
    ...
    it("(j) writes JSON files to team/<role>/_archived/<id>.json", async () => {
        ...
        for (const lesson of [L1, L2]) {
            const archivePath = path.join(tmpRoot, "team", "generalist-dev", "_archived", `${lesson.id}.json`);
    ...
    it("(l) findArchivedLessonById returns null when absent (soft miss)", async () => {
        const result = await findArchivedLessonById(tmpRoot, "generalist-dev", "01NONEXISTENT0000000000000");
