...
import { appendArchivedLessons, readArchivedLessons, archivedLessonsPath, } from "./lesson-archive.js";
let tmpRoot;
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
describe("readArchivedLessons", () => {
    ...
    it("returns parsed entries from an existing archive", async () => {
        ...
        const archived = { ...lesson, archived_at: FIXED_NOW_STR };
...
describe("appendArchivedLessons", () => {
    it("(a) stamps archived_at on each demoted lesson", async () => {
        const lesson = makeLesson();
        const result = await appendArchivedLessons(tmpRoot, "generalist-dev", [lesson], () => FIXED_NOW);
        expect(result).toHaveLength(1);
    ...
    it("(c) is idempotent — demoting the same lesson twice does NOT duplicate", async () => {
        ...
        const secondResult = await appendArchivedLessons(tmpRoot, "generalist-dev", [lesson], () => FIXED_NOW);
    ...
    it("(d) preserves existing entries when appending new ones", async () => {
        ...
        const result = await appendArchivedLessons(tmpRoot, "generalist-dev", [lesson2], () => FIXED_NOW);
    ...
    it("no lesson is ever permanently deleted — overflow is archived, not dropped", async () => {
        ...
        const archived = await appendArchivedLessons(tmpRoot, "generalist-dev", lessons, () => FIXED_NOW);
