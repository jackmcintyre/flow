 ...
 * Override via: FLOW_MCP_LIFECYCLE_LOG env var
 * Back-compat:  FLOW_MCP_DIAG env var (used as path if LIFECYCLE_LOG unset)
 *
 * References:
...
export function createLifecycleLog(opts) {
    ...
    function buildLine(event, fields) {
        ...
        // optional — included only when FLOW_SESSION_ULID is set in the
        // environment (fail-open: absence is documented, not an error).
        const sessionUlid = process.env["FLOW_SESSION_ULID"];
        const line = {
            event,
            ts: Date.now(),
            pid: process.pid,
 ...
 *   2. FLOW_MCP_LIFECYCLE_LOG env var
 *   3. FLOW_MCP_DIAG env var (back-compat: if set, use its value as path)
 *   4. Default: ~/.flow/mcp-lifecycle.log
 */
function resolveLogPath(explicitPath) {
    if (explicitPath)
        return explicitPath;
    const envPath = process.env["FLOW_MCP_LIFECYCLE_LOG"] ?? process.env["FLOW_MCP_DIAG"];
    if (envPath)
        return envPath;
