...
import { DisciplineRuleSchema, DisciplineRulesFileSchema, parseRuleRegistry, serializeRuleRegistry, appendRuleNode, } from "../discipline-rules.js";
...
describe("parseRuleRegistry — comment-preserving round-trip (AC1)", () => {
    it("rewrites a commented registry with no logical change, preserving comments byte-for-byte", () => {
        ...
        expect(rewritten).toContain("# Discipline rules — human-authored ground truth.");
        expect(rewritten).toContain("# Each rule earns its slot; do not delete without a retirement proposal.");
...
`;
  ...
  - id: ${ULID}
    ...
    surprise_key: nope
`;
        expect(() => parseRuleRegistry(malformed)).toThrow(RuleRegistryMalformedError);
    });
    it("raises RuleRegistryMalformedError on YAML syntax errors", () => {
        expect(() => parseRuleRegistry("rules:\n  - id: [unterminated\n")).toThrow(RuleRegistryMalformedError);
    });
});
// ---------------------------------------------------------------------------
// Rule schema shape
...
describe("DisciplineRuleSchema", () => {
    ...
    it("rejects a non-ULID id", () => {
        expect(() => DisciplineRuleSchema.parse({
            id: "not-a-ulid",
            text: "t",
            target_failure_class: "fc",
            introduced_at: "2026-05-31T10:00:00.000Z",
    ...
    it("file schema accepts an empty rules array and is strict on the wrapper", () => {
        ...
        expect(() => DisciplineRulesFileSchema.parse({ rules: [], extra: 1 })).toThrow();
    });
});
