...
import { readArchivedLessons } from "../../lib/lesson-archive.js";
let tmpRoot;
// ---------------------------------------------------------------------------
...
describe("buildPersonaSpawnPrompt", () => {
    ...
    // -------------------------------------------------------------------------
    // Story native:01KT6QSW4W7SMAHAT4EAKCCC65 — budget cap and archiving (AC1)
    // -------------------------------------------------------------------------
    it("(AC1/h) always-shown index is capped at the budget when over-budget", async () => {
        // Build a persona with budget+2 lessons, use_count=0 for all (equal rank).
        const budget = 3;
        const totalLessons = 5;
        const lessonBlocks = Array.from({ length: totalLessons }, (_, i) => {
            const id = `01ARZ3NDEKTSV4RRFFQ69G5F${String.fromCharCode(65 + i)}`;
            return `<!-- lesson:json {"id":"${id}","kind":"pattern","applies_when":"lesson ${i + 1}","detail":"Detail ${i + 1}.","learned_at":"2026-01-0${i + 1}T00:00:00.000Z","use_count":0,"last_used_at":null} -->`;
        }).join("\n");
        const personaMd = FIXTURE_PERSONA_MD.replace(`## Knowledge\n\nAccumulated knowledge goes here.`, `## Knowledge\n\n${lessonBlocks}`);
        const dir = await makePersonaDir(tmpRoot, "generalist-dev");
        await writePersonaFile(dir, personaMd);
        const { systemPrompt } = await buildPersonaSpawnPrompt({
            targetRepoRoot: tmpRoot,
            role: "generalist-dev",
            briefingBudget: budget,
        });
        // Count the index lines in the Knowledge section.
        const knowledgeStart = systemPrompt.indexOf("## Knowledge");
        const lockedStart = systemPrompt.indexOf("## Locked phrases");
        const knowledgeSection = systemPrompt.slice(knowledgeStart, lockedStart);
        const indexLines = knowledgeSection
            .split("\n")
            .filter((l) => l.trimStart().startsWith("- ["));
        expect(indexLines).toHaveLength(budget);
    });
    it("(AC1/i) overflow lessons are written to _archived/lessons.json with archived_at", async () => {
        const budget = 2;
        const lessonBlocks = [
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FA","kind":"pattern","applies_when":"lesson A","detail":"Detail A.","learned_at":"2026-01-01T00:00:00.000Z","use_count":10,"last_used_at":"2026-06-01T00:00:00.000Z"} -->`,
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FB","kind":"pattern","applies_when":"lesson B","detail":"Detail B.","learned_at":"2026-01-02T00:00:00.000Z","use_count":5,"last_used_at":"2026-05-01T00:00:00.000Z"} -->`,
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FC","kind":"pattern","applies_when":"lesson C (overflow)","detail":"Detail C.","learned_at":"2026-01-03T00:00:00.000Z","use_count":0,"last_used_at":null} -->`,
        ].join("\n");
        const personaMd = FIXTURE_PERSONA_MD.replace(`## Knowledge\n\nAccumulated knowledge goes here.`, `## Knowledge\n\n${lessonBlocks}`);
        const dir = await makePersonaDir(tmpRoot, "generalist-dev");
        await writePersonaFile(dir, personaMd);
        ...
        // The overflow lesson (use_count=0) must be in the archive.
        const archived = await readArchivedLessons(tmpRoot, "generalist-dev");
        expect(archived).toHaveLength(1);
        expect(archived[0].id).toBe("01ARZ3NDEKTSV4RRFFQ69G5FC");
        expect(archived[0].archived_at).toBeTruthy();
    });
    it("(AC1/j) overflow lessons are removed from the live Knowledge section", async () => {
        const budget = 2;
        const lessonBlocks = [
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FA","kind":"pattern","applies_when":"lesson A","detail":"Detail A.","learned_at":"2026-01-01T00:00:00.000Z","use_count":10,"last_used_at":"2026-06-01T00:00:00.000Z"} -->`,
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FB","kind":"pattern","applies_when":"lesson B","detail":"Detail B.","learned_at":"2026-01-02T00:00:00.000Z","use_count":5,"last_used_at":"2026-05-01T00:00:00.000Z"} -->`,
        ...
        // Re-read the persona file from disk — the overflow lesson must be gone.
        const updatedPersona = await fs.readFile(path.join(dir, "PERSONA.md"), "utf8");
        expect(updatedPersona).not.toContain("01ARZ3NDEKTSV4RRFFQ69G5FC");
        // But the kept lessons must remain.
        expect(updatedPersona).toContain("01ARZ3NDEKTSV4RRFFQ69G5FA");
        expect(updatedPersona).toContain("01ARZ3NDEKTSV4RRFFQ69G5FB");
    });
    it("(AC1/h) top lessons are ordered by use_count desc then last_used_at desc", async () => {
        const budget = 2;
        // Lesson C has highest use_count, lesson B second, lesson A lowest.
        const lessonBlocks = [
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FA","kind":"pattern","applies_when":"lesson A","detail":"Detail A.","learned_at":"2026-01-01T00:00:00.000Z","use_count":1,"last_used_at":"2026-03-01T00:00:00.000Z"} -->`,
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FB","kind":"pattern","applies_when":"lesson B","detail":"Detail B.","learned_at":"2026-01-02T00:00:00.000Z","use_count":5,"last_used_at":"2026-05-01T00:00:00.000Z"} -->`,
            `<!-- lesson:json {"id":"01ARZ3NDEKTSV4RRFFQ69G5FC","kind":"pattern","applies_when":"lesson C","detail":"Detail C.","learned_at":"2026-01-03T00:00:00.000Z","use_count":10,"last_used_at":"2026-06-01T00:00:00.000Z"} -->`,
        ].join("\n");
        const personaMd = FIXTURE_PERSONA_MD.replace(`## Knowledge\n\nAccumulated knowledge goes here.`, `## Knowledge\n\n${lessonBlocks}`);
        const dir = await makePersonaDir(tmpRoot, "generalist-dev");
        await writePersonaFile(dir, personaMd);
        ...
        // The top-2 by use_count are C (10) and B (5). A (1) is overflow.
        expect(systemPrompt).toContain("01ARZ3NDEKTSV4RRFFQ69G5FC");
        expect(systemPrompt).toContain("01ARZ3NDEKTSV4RRFFQ69G5FB");
        expect(systemPrompt).not.toContain("01ARZ3NDEKTSV4RRFFQ69G5FA");
        // Lesson A must be in the archive.
        const archived = await readArchivedLessons(tmpRoot, "generalist-dev");
        expect(archived.map((e) => e.id)).toContain("01ARZ3NDEKTSV4RRFFQ69G5FA");
    });
});
