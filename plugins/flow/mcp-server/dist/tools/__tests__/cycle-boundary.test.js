 ...
 */
function buildDoneManifest(ref, completedAt) {
    return {
        ref,
        status: "done",
        adapter: "native",
...
beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tmpRoot, ".flow", "config.yaml"), "adapter: native\n", "utf8");
});
afterEach(async () => {
...
describe("AC1: openCycle → getStatus shows cycle ULID", () => {
    ...
    it("getStatus reports the cycle ULID (not none) after openCycle", async () => {
        ...
        await fs.writeFile(path.join(tmpRoot, "docs", "standards.md"), "---\nversion: 1.0.0\n---\n# Standards\n", "utf8");
...
describe("AC2: gatherRetroInputs filters to cycle window", () => {
    it("excludes manifests and events that pre-date the cycle's opened_at", async () => {
        ...
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:AAAAAAAAAAAAAAAAAAAAAAAAAA.yaml"), buildDoneManifest("native:AAAAAAAAAAAAAAAAAAAAAAAAAA", "2026-06-01T10:00:00.000Z"));
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:BBBBBBBBBBBBBBBBBBBBBBBBBB.yaml"), buildDoneManifest("native:BBBBBBBBBBBBBBBBBBBBBBBBBB", "2026-06-07T10:00:00.000Z"));
    ...
    it("excludes done manifests with no completed_at when a cycle is active", async () => {
        ...
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:CCCCCCCCCCCCCCCCCCCCCCCCCC.yaml"), buildDoneManifest("native:CCCCCCCCCCCCCCCCCCCCCCCCCC"));
        // Manifest with completed_at after cycle.
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:DDDDDDDDDDDDDDDDDDDDDDDDDD.yaml"), buildDoneManifest("native:DDDDDDDDDDDDDDDDDDDDDDDDDD", "2026-06-06T10:00:00.000Z"));
        const bundle = await gatherRetroInputs({
            targetRepoRoot: tmpRoot,
            cycleOpenedAt: cycleOpen,
        });
    ...
    it("includes manifests completed exactly at opened_at (boundary is inclusive)", async () => {
        const cycleOpen = "2026-06-05T00:00:00.000Z";
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:EEEEEEEEEEEEEEEEEEEEEEEEEE.yaml"), buildDoneManifest("native:EEEEEEEEEEEEEEEEEEEEEEEEEE", cycleOpen));
        const bundle = await gatherRetroInputs({
            targetRepoRoot: tmpRoot,
            cycleOpenedAt: cycleOpen,
        });
    ...
    it("reads cycle from disk when no cycleOpenedAt override is passed", async () => {
        ...
        await fs.writeFile(path.join(tmpRoot, ".flow", "cycle-state.json"), JSON.stringify({ cycle_id: "01HX000000000000000000TEST", opened_at: cycleOpen }) + "\n", "utf8");
        // After-cycle manifest.
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:FFFFFFFFFFFFFFFFFFFFFFFFFF.yaml"), buildDoneManifest("native:FFFFFFFFFFFFFFFFFFFFFFFFFF", "2026-06-09T00:00:00.000Z"));
        // Before-cycle manifest.
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:GGGGGGGGGGGGGGGGGGGGGGGGGG.yaml"), buildDoneManifest("native:GGGGGGGGGGGGGGGGGGGGGGGGGG", "2026-06-01T00:00:00.000Z"));
...
describe("AC3: prior cycle archived when openCycle is called again", () => {
    it("writes a YAML archive record for the prior cycle and then activates the new one", async () => {
        ...
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:HHHHHHHHHHHHHHHHHHHHHHHHHH.yaml"), buildDoneManifest("native:HHHHHHHHHHHHHHHHHHHHHHHHHH", "2026-06-03T10:00:00.000Z"));
        await fs.writeFile(path.join(tmpRoot, ".flow", "retro-proposals", "2026-06-05T00:00:00.000Z.md"), "# A proposal\n", "utf8");
        ...
        const archiveFileName = archiveEntries[0];
        ...
        const archiveContent = await fs.readFile(path.join(archiveDir, archiveFileName), "utf8");
...
describe("AC4: no-cycle baseline (cycle-state.json absent)", () => {
    ...
    it("gatherRetroInputs returns ALL done manifests when no cycle is active", async () => {
        ...
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:IIIIIIIIIIIIIIIIIIIIIIIIII.yaml"), buildDoneManifest("native:IIIIIIIIIIIIIIIIIIIIIIIIII", "2026-01-01T00:00:00.000Z"));
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:JJJJJJJJJJJJJJJJJJJJJJJJJJ.yaml"), buildDoneManifest("native:JJJJJJJJJJJJJJJJJJJJJJJJJJ", "2026-06-01T00:00:00.000Z"));
    ...
    it("gatherRetroInputs returns full history when cycleOpenedAt is explicitly null", async () => {
        await writeYaml(path.join(tmpRoot, ".flow", "state", "done", "native:KKKKKKKKKKKKKKKKKKKKKKKKKK.yaml"), buildDoneManifest("native:KKKKKKKKKKKKKKKKKKKKKKKKKK", "2020-01-01T00:00:00.000Z"));
        const bundle = await gatherRetroInputs({
            targetRepoRoot: tmpRoot,
