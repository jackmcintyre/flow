 ...
 */
async function runDrain(extraArgs = {}) {
    ...
    const seamResult = (label) => {
        ...
        if (label.startsWith("progress-start:"))
            return { atMs: Date.now(), line: `${REF} phase: start` };
        if (label.startsWith("progress-done:"))
            return { line: `${REF} phase: done (0ms)` };
        // clean-root-guard
...
function devAndReviewerCalls(calls) {
    return calls.filter((c) => !c.opts.schema &&
