...
const QUESTION = "Should the cap default to per-run or per-story? The two diverge on a crashed-resume and change operator-visible behaviour.";
/**
...
describe("drain pause-on-ambiguity (Story 8.19 AC2 + AC3)", () => {
    it("files the ambiguous story into the human-needed bucket with its question, and the normal story still completes", async () => {
        ...
        const paused = result.pausedForHuman.find((p) => p.ref === AMBIGUOUS_REF);
        ...
        expect(result.blocked.some((b) => b.ref === AMBIGUOUS_REF)).toBe(false);
        ...
        expect(result.pausedForHuman.some((p) => p.ref === NORMAL_REF && p.prNumber === NORMAL_PR)).toBe(true);
    });
    it("does NOT open a PR or run review/gate for the paused story", async () => {
        ...
        const ambiguousReviewer = calls.find((c) => (c.opts.label ?? "").startsWith(`rev:${AMBIGUOUS_REF}`));
        expect(ambiguousReviewer).toBeUndefined();
        // No verdict/gate seam for the ambiguous story.
        expect(calls.some((c) => (c.opts.label ?? "").startsWith(`verdict:${AMBIGUOUS_REF}`))).toBe(false);
        expect(calls.some((c) => (c.opts.label ?? "").startsWith(`gate:${AMBIGUOUS_REF}`))).toBe(false);
        // The dev was NOT instructed to re-run on the ambiguous story (single dev call).
        const ambiguousDevCalls = calls.filter((c) => (c.opts.label ?? "").startsWith(`dev:${AMBIGUOUS_REF}`));
        expect(ambiguousDevCalls).toHaveLength(1);
        // The normal story DID reach its reviewer + gate (it completed).
        expect(calls.some((c) => (c.opts.label ?? "").startsWith(`rev:${NORMAL_REF}`))).toBe(true);
        expect(calls.some((c) => (c.opts.label ?? "").startsWith(`gate:${NORMAL_REF}`))).toBe(true);
    });
    it("emits an operator notification naming the ref and question when a story pauses — and none for a story that completes normally", async () => {
        ...
        const narratorLine = logs.find((l) => l.includes("NEEDS HUMAN") && l.includes(AMBIGUOUS_REF));
    ...
    it("the run is safe when NO notifier seam is injected — it still pauses and surfaces through the narrator", async () => {
        ...
        const agent = async (_prompt, agentOpts = {}) => {
            const label = agentOpts.label ?? "";
            if (agentOpts.schema)
                return { stdout: JSON.stringify(seamResult(label)) };
            if (label.startsWith(`dev:${AMBIGUOUS_REF}`)) {
        ...
        expect(paused).toBeDefined();
        expect(paused.question).toBe(QUESTION);
        expect(logs.some((l) => l.includes("NEEDS HUMAN") && l.includes(QUESTION))).toBe(true);
    });
});
