...
beforeEach(async () => {
    ...
    // git init (needed for runDevTerminalAction; also ensures .flow/ dir is usable)
...
describe("AC3b — defer class", () => {
    it("(i) gh() raises GhRecoverableError class=defer; (iii) manifest carries blocked_by=gh-defer; (iv) still in in-progress/", async () => {
        ...
        // (v) chat line shape
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=defer) — story ${STORY_REF} blocked. blocked_by stamped to gh-defer. Operator action: wait and re-run /flow:start`);
    });
});
// ---------------------------------------------------------------------------
...
describe("AC3c — needs-human class", () => {
    it("(i) gh() raises GhRecoverableError class=needs-human; (iii) blocked_by=gh-needs-human; (iv) still in in-progress/", async () => {
        ...
        await assertManifestInInProgress();
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=needs-human) — story ${STORY_REF} blocked. blocked_by stamped to gh-needs-human. Operator action: run \`gh auth login\` then re-run /flow:start`);
    });
});
// ---------------------------------------------------------------------------
// AC3d — retry class
...
describe("AC3d — retry class", () => {
    it("(i) gh() raises GhRecoverableError class=retry; (iii) blocked_by=gh-retry; (iv) still in in-progress/", async () => {
        ...
        await assertManifestInInProgress();
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=retry) — story ${STORY_REF} blocked. blocked_by stamped to gh-retry. Operator action: transient network error; re-run /flow:start (v2 will auto-retry)`);
    });
});
// ---------------------------------------------------------------------------
