...
describe("AC4(a): happy handoff + READY FOR MERGE", () => {
    it("full cycle: spawn-reviewer → done-ready-for-merge, completed: true, verbatim chatLog, manifest moved to done/", async () => {
        ...
        const doneManifestRaw = await fs.readFile(path.join(tmpRoot, ".flow", "state", "done", `${STORY_REF}.yaml`), "utf8");
...
describe("AC4(c): handoff grammar drift → done-blocked-handoff-grammar", () => {
    it("processReviewerTranscript is NOT called; manifest blocked_by: 'handoff-grammar'; verbatim AC3 line", async () => {
        ...
        expect(devResult.chatLog).toContain(`handoff grammar drift — story ${STORY_REF} blocked. expected verbatim phrase: "Handoff to reviewer — story ${STORY_REF} ready for review." Edit the manifest to clear blocked_by and re-run /flow:start.`);
...
describe("AC4 (4.3c): green-path two-story drain via processReviewerTranscript internal seam", () => {
    let twoStoryRoot;
    beforeEach(async () => {
        twoStoryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flow-ac4-4-3c-two-story-"));
    ...
    it("drives two stories through claim → dev → reviewer-ready (internal completeStory), then queue-drained", async () => {
        ...
        await expect(fs.stat(path.join(root, ".flow", "state", "in-progress", `${refA}.yaml`))).resolves.toBeDefined();
        await expect(fs.stat(path.join(root, ".flow", "state", "to-do", `${refA}.yaml`))).rejects.toThrow();
        ...
        await expect(fs.stat(path.join(root, ".flow", "state", "in-progress", `${refA}.yaml`))).rejects.toThrow(); // ENOENT — moved by processReviewerTranscript internally
        const doneManifestARaw = await fs.readFile(path.join(root, ".flow", "state", "done", `${refA}.yaml`), "utf8");
        ...
        await expect(fs.stat(path.join(root, ".flow", "state", "in-progress", `${refB}.yaml`))).resolves.toBeDefined();
        ...
        await expect(fs.stat(path.join(root, ".flow", "state", "in-progress", `${refB}.yaml`))).rejects.toThrow();
        const doneManifestBRaw = await fs.readFile(path.join(root, ".flow", "state", "done", `${refB}.yaml`), "utf8");
        ...
        expect(claimThird.chatLog[0]).toBe("queue drained — to-do/ and in-progress/ are both empty. Stop here, or run /flow:plan to add work.");
...
describe("AC4 (4.3c): blocked branches do NOT invoke completeStory", () => {
    let blockedRoot;
    beforeEach(async () => {
        blockedRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flow-ac4-4-3c-blocked-"));
    ...
    it("AC4(g): reviewer BLOCKED verdict — manifest stays in-progress, no completed field, done/ empty", async () => {
        ...
        const inProgressRaw = await fs.readFile(path.join(root, ".flow", "state", "in-progress", `${refA}.yaml`), "utf8");
