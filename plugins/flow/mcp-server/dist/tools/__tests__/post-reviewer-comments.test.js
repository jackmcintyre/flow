...
async function writeResultFile(data) {
    ...
    const sessDir = path.join(tmpRoot, ".flow", "state", "sessions", SESSION_ULID, sanitiseRefForPathSegment(STORY_REF));
...
describe("(4.12 AC5 b1) reviewer.verdict telemetry — normal POST-success path", () => {
    it("writes exactly one reviewer.verdict event with correct fields to tmpdir JSONL", async () => {
        ...
        try {
            ...
            await atomicWriteFile(path.join(telemetryTmpRoot, ".flow", "config.yaml"), "adapter: native\nadapter_config: {}\n");
