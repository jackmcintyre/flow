...
describe("(b) drift → done-blocked-handoff-grammar", () => {
    it("stamps blocked_by: 'handoff-grammar', AC3 chatLog line", async () => {
        ...
        expect(result.chatLog).toContain(`handoff grammar drift — story ${STORY_REF} blocked. expected verbatim phrase: "Handoff to reviewer — story ${STORY_REF} ready for review." Edit the manifest to clear blocked_by and re-run /flow:start.`);
        const onDisk = await readOnDiskManifest();
...
describe("(e) recoverable-error: class=defer → done-blocked-gh-defer (AC3b)", () => {
    it("stamps blocked_by: gh-defer, returns done-blocked-gh-defer with correct chat line", async () => {
        ...
        expect(result.chatLog).toContain(`gh recoverable error (class=defer) — story ${STORY_REF} blocked. blocked_by stamped to gh-defer. Operator action: wait and re-run /flow:start`);
        const onDisk = await readOnDiskManifest();
...
describe("(f) recoverable-error: class=retry → done-blocked-gh-retry (AC3d)", () => {
    it("stamps blocked_by: gh-retry, returns done-blocked-gh-retry with correct chat line", async () => {
        ...
        expect(result.chatLog).toContain(`gh recoverable error (class=retry) — story ${STORY_REF} blocked. blocked_by stamped to gh-retry. Operator action: transient network error; re-run /flow:start (v2 will auto-retry)`);
        const onDisk = await readOnDiskManifest();
...
describe("(g) recoverable-error: class=needs-human → done-blocked-gh-needs-human (AC3c)", () => {
    it("stamps blocked_by: gh-needs-human, returns done-blocked-gh-needs-human with correct chat line", async () => {
        ...
        expect(result.chatLog).toContain(`gh recoverable error (class=needs-human) — story ${STORY_REF} blocked. blocked_by stamped to gh-needs-human. Operator action: run \`gh auth login\` then re-run /flow:start`);
        const onDisk = await readOnDiskManifest();
