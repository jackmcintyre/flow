...
describe("readBacklogInventory (3) — withdrawn flag is surfaced correctly", () => {
    ...
    it("returns withdrawn:true for a manifest that has been flipped", async () => {
        ...
        const manifestPath = path.join(root, ".flow", "state", "to-do", "native:01HZABC0000000000000000001.yaml");
...
describe("readBacklogInventory (8) — Story 10.6 cutover: native reads ULID refs, no detection ambiguity", () => {
    it("reads native:<ULID> manifests identically to how a BMad repo reads bmad:<ref> manifests", async () => {
        ...
        const stateEntries = result.backlog_inventory.filter((e) => e.state !== "native-source-only");
    ...
    it("an explicit `adapter: native` config binds native even when a BMad stories tree also exists (no AmbiguousAdapterError)", async () => {
        ...
        await atomicWriteFile(path.join(root, ".flow", "native-stories", `${ulid}.md`), "# Pinned native story\n\n## Narrative\n\nAs a user, I want it.\n");
        ...
        await atomicWriteFile(path.join(bmadStories, "1-1-some-story.md"), "# Story 1.1: Some story\n\nStatus: ready-for-dev\n");
...
describe("readBacklogInventory (9) — includeSpecText + ref single-item fetch", () => {
    ...
    async function buildRepo() {
        ...
        await atomicWriteFile(path.join(root, ".flow", "native-stories", `${SPEC_ULID}.md`), SPEC_CONTENT);
        const manifest = {
        ...
        await atomicWriteFile(path.join(root, ".flow", "state", "to-do", `native:${SPEC_ULID}.yaml`), yamlStringify(manifest));
        // A native-source-only story (no manifest).
        await atomicWriteFile(path.join(root, ".flow", "native-stories", `${SOURCE_ONLY_ULID}.md`), SOURCE_ONLY_CONTENT);
        return root;
    }
...
describe("readBacklogInventory (10) — skips `<ref>.snapshot.yaml` baselines", () => {
    ...
    async function buildMidBuildRepo() {
        ...
        await atomicWriteFile(path.join(root, ".flow", "state", "in-progress", `${ref}.yaml`), yamlStringify(manifest, { lineWidth: 0 }));
        // The anti-tamper baseline written on claim — NOT a manifest (no `ref`).
        await atomicWriteFile(path.join(root, ".flow", "state", "in-progress", `${ref}.snapshot.yaml`), "source_hash: " + "a".repeat(64) + "\ntitle: Mid-build story\n");
        return root;
    }
    it("renders without crashing and the mid-build story appears exactly once", async () => {
        const root = await buildMidBuildRepo();
        const result = await readBacklogInventory({ targetRepoRoot: root });
        const matches = result.backlog_inventory.filter((e) => e.ref === `native:${MID_BUILD_ULID}`);
