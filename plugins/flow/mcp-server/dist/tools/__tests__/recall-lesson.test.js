...
const LESSON_DETAIL = "Always run the full test suite before opening a PR — deploy-without-test PRs caused 3 rollbacks in a row.";
...
describe("recallLesson (integration)", () => {
    ...
    it("(g-integration) full detail is returned — more text than the one-line trigger", async () => {
        ...
        expect(result.found).toBe(true);
        expect(result.lesson.detail.length).toBeGreaterThan(result.lesson.applies_when.length);
        expect(result.lesson.detail).toBe(LESSON_DETAIL);
    });
});
