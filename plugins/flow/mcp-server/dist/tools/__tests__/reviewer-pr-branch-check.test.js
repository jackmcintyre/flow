...
// ---------------------------------------------------------------------------
// Fixture builder (flow workspace files)
// ---------------------------------------------------------------------------
async function buildFlowFixture(repoRoot) {
    // .flow/config.yaml — native adapter
    await fs.mkdir(path.join(repoRoot, ".flow"), { recursive: true });
    await atomicWriteFile(path.join(repoRoot, ".flow", "config.yaml"), "adapter: native\nadapter_config: {}\n");
    // Native stories dir + spec file
    const storiesDir = path.join(repoRoot, ".flow", "native-stories");
...
let tmpRoot;
beforeEach(async () => {
    tmpRoot = mkdtempSync(path.join(os.tmpdir(), "flow-5-26-"));
...
describe("AC3(d): review-worktree is torn down after session completes", () => {
    it("review-worktree directory does not exist after runReviewerSession returns", async () => {
        ...
        const worktreePath = path.join(tmpRoot, ".flow", "state", "sessions", SESSION_ULID, "review-worktree");
