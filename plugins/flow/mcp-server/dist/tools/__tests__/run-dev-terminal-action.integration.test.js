...
import { ConventionalCommitTypeUnknownError, GitPushFailedError, GhPrCreateFailedError, NegativeCapabilityDeniedError, PrePrLeakDetectedError, RebaseConflictError, } from "../../errors.js";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
async function setupWorktreeRepo() {
...
describe("runDevTerminalAction — pre-PR leak gate (Story native:01KT47430Q4C73K5E3ZECBSE5R AC2)", () => {
    ...
    it("AC2: stops BEFORE pr-create and throws PrePrLeakDetectedError when the shared master is dirtied", async () => {
        ...
        try {
            ...
            expect(spy.mock.calls.some(([cmd]) => cmd === "gh"), "gh pr create must never be called when the leak gate fires").toBe(false);
            // push was NEVER called — the leaking branch never reaches origin.
            const gitPushCalled = spy.mock.calls.some(([cmd, args]) => cmd === "git" && Array.isArray(args) && args[2] === "push");
