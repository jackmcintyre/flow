...
export const BmadToNativeIngestInputSchema = z.object({
     ...
     * `/flow:ingest` passes its orchestration session ULID when available.
     */
    sessionUlid: z.string().min(1).optional(),
});
/**
 * Zod schema for a single enriched draft supplied over the tool transport. The
 * `/flow:ingest` skill (the LLM enricher) reads each BMad story and produces one
 ...
 * `.flow/native-stories/` for the provenance citation each ingested story
...
async function indexAlreadyIngested(storiesDir) {
    ...
    for (const name of entries) {
        ...
        for (const cited of parsed.cited_sources ?? []) {
            ...
            if (!byCitedPath.has(cited))
                byCitedPath.set(cited, abs);
        }
    }
    return byCitedPath;
}
 ...
 */
export async function bmadToNativeIngest(rawInput, enrich) {
 ...
 */
export async function bmadToNativeIngestTool(rawInput) {
    ...
    const enrich = (story) => {
        ...
        if (!draft) {
            ...
            throw new Error(`${NO_DRAFT_SUPPLIED}: no enriched draft was supplied for ${story.ref}. ` +
                `The /flow:ingest skill must draft a §3 enrichment for every BMad story ` +
                `it intends to seed, or leave the story for human fix-up.`);
        }
        return draft;
    };
    return bmadToNativeIngest({ targetRepoRoot: input.targetRepoRoot, sessionUlid: input.sessionUlid }, enrich);
}
