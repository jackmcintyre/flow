...
export async function createSmokeScratchRepo(opts) {
    ...
    const scratchRoot = await fs.mkdtemp(path.join(parentDir ?? os.tmpdir(), `flow-smoke-${label}-`));
    ...
    // Step 2: write minimal native-adapter .flow/config.yaml.
    // Non-canonical path (not under .flow/state/**, team/**, etc.) — no
    // mcpToolContext needed.
    await writeManagedFile({
        absPath: path.join(scratchRoot, ".flow", "config.yaml"),
        contents: "adapter: native\nstandards: {}\n",
        targetRepoRoot: scratchRoot,
    });
    // Step 3: copy standards template to .flow/standards.md.
    const standardsContents = await fs.readFile(standardsTemplatePath, "utf8");
    await writeManagedFile({
        absPath: path.join(scratchRoot, ".flow", "standards.md"),
