...
export function lensVerdictFilePath(targetRepoRoot, sessionUlid, ref, lens) {
    return path.join(targetRepoRoot, ".flow", "state", "sessions", sessionUlid, sanitiseRefForPathSegment(ref), `judge-${lens}.json`);
}
// ---------------------------------------------------------------------------
 ...
 * Exported for unit testing.
 */
export function resolveLensRoleBinding(hiredRoles) {
