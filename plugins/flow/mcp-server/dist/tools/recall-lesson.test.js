...
const LESSON_DETAIL = "Always run the full test suite before opening a PR — deploy-without-test PRs caused 3 rollbacks.";
...
function personaWithLessons(lessons) {
...
describe("recallLesson — archived store fallback (AC2)", () => {
    ...
    it("(AC2-e) use_count is incremented in the persona file on live lesson recall", async () => {
        const lessonWithCount = { ...FIXTURE_LESSON, use_count: 3 };
        ...
        const updatedPersona = await fs.readFile(path.join(tmpRoot, "team", "generalist-dev", "PERSONA.md"), "utf8");
    ...
    it("(AC2-f) use_count is incremented in the archive file on archived lesson recall", async () => {
        const lessonWithCount = { ...FIXTURE_LESSON, use_count: 1 };
        await makeEmptyPersonaFile();
        ...
        const updatedArchive = await fs.readFile(path.join(tmpRoot, "team", "generalist-dev", "_archived", `${LESSON_ID}.json`), "utf8");
        const parsed = JSON.parse(updatedArchive);
