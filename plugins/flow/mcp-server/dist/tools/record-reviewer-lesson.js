 ...
 */
import { MalformedStoryRetroPayloadError, ReviewerResultFileMissingError, } from "../errors.js";
import { atomicWriteFile } from "../lib/managed-fs.js";
import { readReviewerResultFile, reviewerResultFilePath, } from "../lib/read-reviewer-result-file.js";
 ...
 */
export async function recordReviewerLesson(opts) {
    ...
    // Step 2: Read the EXISTING per-ref reviewer-result.json. Null → no verdict
    // file to merge onto: runReviewerSession never ran for this ref/session.
    ...
    // Step 3: MERGE only the lesson field — never clobber the binding verdict,
    // acResults, riskTier, or any other field.
    const merged = {
        ...existing,
        lesson: parsed.data,
    };
    // Step 4: Write back to the SAME per-ref path (the shared derivation). The
    // parent directory already exists (runReviewerSession created it).
