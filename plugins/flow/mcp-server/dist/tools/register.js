...
export function registerAllTools(server) {
    ...
    server.registerTool({
        ...
        description: "Write a new native-adapter story file under <targetRepoRoot>/.flow/native-stories/<ULID>.md. " +
    ...
    server.registerTool({
        ...
        description: "Assemble the deterministic retro input bundle for the /flow:retro skill " +
    ...
    server.registerTool({
        ...
        description: "Mint a fresh session ULID for a /flow:start invocation. Pure — no IO. " +
    ...
    server.registerTool({
        ...
        description: "Enumerate claimable to-do manifests for the /flow:start claim-spawn loop. " +
    ...
    // .flow/config.yaml + .crew/standards.md. Used by the /flow:smoke skill
    // as the first checkpoint step (AC1).
    server.registerTool({
        name: "createSmokeScratchRepo",
        description: "Create a disposable smoke-harness scratch repo seeded with git init + empty commit + minimal .flow/config.yaml + .crew/standards.md. Used by the /flow:smoke skill as the first checkpoint step.",
        inputSchema: {
            type: "object",
            properties: {
                label: { type: "string" },
    ...
    server.registerTool({
        ...
        description: "Scan <targetRepoRoot>/.flow/state/in-progress/ for manifests whose claimed_by ULID " +
    ...
    server.registerTool({
        ...
        description: "Persist a structured agent.friction telemetry event when an agent compensates " +
