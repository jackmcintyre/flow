 ...
 * both "sprint-status.yaml" (no prefix) and ".flow/sprint-status.yaml" (with prefix).
 *
 ...
 */
export function isEnrichedStory(story) {
    ...
    if (isEnrichedStory(story)) {
        ...
        if (tasks.length === 0) {
            reasons.push({
                ...
                field: "tasks",
                detail: "Native story declares no tasks. Every native story must have ≥1 task in '## Tasks', each mapping to at least one AC via a trailing '(AC: <n>, …)' clause.",
            });
        }
