...
// Output dir is overridable (FLOW_BUNDLE_OUT_DIR) so the dist-shipping drift test
...
const OUT_DIR = process.env["FLOW_BUNDLE_OUT_DIR"]
  ? path.resolve(process.env["FLOW_BUNDLE_OUT_DIR"])
  : path.join(ROOT, "dist");
