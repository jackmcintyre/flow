/**
 * Typed error hierarchy for the flow plugin.
 *
...
/**
 * `.flow/config.yaml` exists but failed schema validation
...
export class UnknownAdapterError extends DomainError {
  ...
  constructor(opts: {
    ...
    super(
      `'.flow/config.yaml' declares adapter '${opts.configuredAdapterName}' at ${opts.configPath}, ` +
 ...
 * User must author `.flow/config.yaml` manually.
 */
  ...
  constructor(opts: { targetRepoRoot: string; registeredAdapters: string[] }) {
    super(
      `No registered adapter recognises ${opts.targetRepoRoot}. ` +
        ...
        `Author .flow/config.yaml manually following ` +
        `plugins/flow/example/.flow/config.yaml.`,
    );
  ...
  constructor(opts: { targetRepoRoot: string; matchingAdapters: string[] }) {
    super(
      `Multiple adapters recognise ${opts.targetRepoRoot}: ` +
        ...
        `Author .flow/config.yaml manually to pick one.`,
    );
    this.targetRepoRoot = opts.targetRepoRoot;
  ...
  constructor(opts: { path: string; reason: string; details?: Record<string, unknown> }) {
    super(
      `BMad story at '${opts.path}' is malformed: ${opts.reason}. ` +
        `See plugins/flow/docs/spikes/bmad-format.md for the expected shape. (Story 3.3)`,
    );
    this.path = opts.path;
    this.reason = opts.reason;
  ...
  constructor(opts: {
    ...
    super(
      `Cross-filesystem move refused for manifest '${opts.ref}': ` +
        ...
        `single filesystem, or align the .flow/state/ tree ` +
  ...
  constructor(opts: { sourcePath: string; zodMessage: string }) {
    super(
      `Catalogue role at ${opts.sourcePath} is malformed: ${opts.zodMessage}. ` +
        `See the canonical example in plugins/flow/catalogue/generalist-dev.md ` +
...
/**
 * An execution manifest at `<target-repo>/.flow/state/<state>/<ref>.yaml`
 ...
 * does not exist in `plugins/flow/catalogue/`. Distinct from
...
/**
 * A native-story file at `<target-repo>/.flow/native-stories/<ULID>.md`
...
/**
 * An execution manifest at `<target-repo>/.flow/state/in-progress/<ref>.yaml`
 ...
 * `/flow:plan` to discard the story (Story 3.6 flow).
 *
  ...
  constructor(opts: { ref: string; changedFields: readonly string[]; absPath: string }) {
    ...
    super(
      `Refusing: ${opts.ref} in in-progress/ has been hand-edited (fields: ${fieldList}). ` +
        ...
        `Wait for the story to land in done/ or blocked/, or discard it via /flow:plan.`,
    );
    this.ref = opts.ref;
 ...
 * `/flow:start` SKILL.md documents it as a failure mode and a test asserts
  ...
  constructor(opts: { ref: string }) {
    super(
      `Dev subagent for story ${opts.ref} terminated without the verbatim locked handoff phrase. ` +
        ...
        `Edit the manifest to clear blocked_by and re-run /flow:start.`,
    );
    this.ref = opts.ref;
  }
}
 ...
 */
export class RebaseConflictError extends DomainError {
  readonly reason: string;
  ...
  constructor(opts: {
    ...
    super(
      `pre-PR sync gate stopped: the story branch could not be rebased onto the ` +
        `latest origin/main because it genuinely conflicts with trunk work that ` +
        `landed first. The rebase was aborted (working tree left clean) and NO ` +
        `pull request was opened — opening one would have produced a PR that ` +
        `cannot merge. ` +
        (paths.length > 0
          ? `Conflicting paths: ${paths.join(", ")}. `
          : "") +
        `reason: ${opts.reason || "(none)"}. ` +
        `Resolve the conflict against the latest trunk and re-run. ` +
        `(Story native:01KT40THFTS10F9PT37KCW9PF4)`,
    );
    this.reason = opts.reason;
...
/**
 * `plugins/flow/permissions/gh-error-map.yaml` failed schema validation: the
 ...
 */
export class ReviewerResultFileMissingError extends DomainError {
  readonly path: string;
  readonly ref: string;
  readonly sessionUlid: string;

  constructor(opts: { path: string; ref: string; sessionUlid: string }) {
    super(
      `No reviewer-result.json at ${opts.path} for ref ${opts.ref} (session ${opts.sessionUlid}). ` +
        `recordReviewerLesson must be called AFTER runReviewerSession, which writes that file.`,
    );
    this.path = opts.path;
    this.ref = opts.ref;
 ...
 * always be present in `plugins/flow/docs/risk-tiering.md`.
 *
  ...
  constructor(opts: { sessionUlid: string; ref: string }) {
    super(
      `reviewer-result.json absent for session ${opts.sessionUlid} (story ${opts.ref}). ` +
        ...
        `Operator must inspect the reviewer transcript before re-running /flow:start. ` +
...
/**
 * `acceptProposal`'s id locator scanned every `.flow/retro-proposals/*.md`
  ...
  constructor(opts: { proposalId: string; filesScanned: number }) {
    super(
      `No retro proposal with id '${opts.proposalId}' found ` +
        ...
        `under .flow/retro-proposals/). Check the id against the proposal markdown file, ` +
        `or run /flow:retro to produce proposals first. (Story 6.4 AC1)`,
    );
 ...
 * (the author subagent / the `/flow:author` skill) surfaces `violations` back
...
export class RuleNotFoundError extends DomainError {
  ...
  constructor(opts: { targetRuleId: string; registryPath: string }) {
    super(
      `rule-retirement refused: no rule with id '${opts.targetRuleId}' found in ` +
...
export class RetirementWouldEmptyRegistryError extends DomainError {
  ...
  constructor(opts: { targetRuleId: string }) {
    super(
      `rule-retirement refused: retiring rule '${opts.targetRuleId}' would leave the ` +
