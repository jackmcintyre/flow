...
function telemetryEvent(failureClass: string): TelemetryEvent {
  return {
    type: "reviewer.verdict" as unknown as TelemetryEvent["type"],
    ts: "2026-05-31T10:00:00.000Z",
    session_id: "session-1",
    agent: "generalist-reviewer",
    data: { failure_class: failureClass } as unknown,
  } as unknown as TelemetryEvent;
}

/** Build a minimal DisciplineRule. */
function rule(id: string, targetClass: string): DisciplineRule {
  return {
    id,
    text: `Rule for ${targetClass}`,
    target_failure_class: targetClass,
    introduced_at: "2026-05-01T00:00:00.000Z",
    // No level set: these generic-rule fixtures represent rules without an
    // explicit level, which are still retirement candidates at zero fires.
    // Use an explicit level: "must" only in tests that exercise the
    // foundational-guardrail guard.
  };
}
