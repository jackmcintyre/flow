 ...
 *
 * Before the per-ref fix every story in a run run wrote to a single shared
...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flow-dev-outcome-"));
});
...
async function writeOutcome(
  ref: string,
  outcome: { prUrl: string; prNumber: number; branch: string; commitSha: string },
): Promise<string> {
...
describe("readDevOutcomeFile — absent / malformed", () => {
  ...
  it("throws DevOutcomeFileMalformedError on invalid JSON", async () => {
    ...
    await expect(
      readDevOutcomeFile(tmpRoot, SESSION_ULID, ref),
  ...
  it("throws DevOutcomeFileMalformedError when prNumber is missing", async () => {
    ...
    await fs.writeFile(
      filePath,
      JSON.stringify({ prUrl: "https://github.com/o/r/pull/9", branch: "b", commitSha: "c" }),
      "utf8",
    );
