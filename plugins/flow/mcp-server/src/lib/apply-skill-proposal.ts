 ...
 * `.md` extension. `.flow/skills/foo.md` → `foo`.
 */
 ...
 * resolved relative to the skill file's OWN directory's `.flow/skills/` root.
 * For `.flow/skills/foo.md` the archive is `.flow/skills/_archived/foo.md`.
 */
 ...
 */
function skillName(relPath: string): string {
  return path.basename(relPath, ".md");
}

function makeLessonToSkillHandler(deps: SkillHandlerDeps): ProposalApplyHandler {
  return {
