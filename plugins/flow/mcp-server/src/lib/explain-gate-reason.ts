 ...
 */
const GATE_REASON_EXPLANATIONS: Readonly<Record<string, string>> = {
  "low-risk-met-threshold":
    "Auto-merged: this is a low-risk change and the team's recent agreement with the reviewer met the trust threshold.",

  "low-risk-sub-threshold":
    "Paused for a human: this is a low-risk change, but the team's recent agreement with the reviewer fell below the trust threshold.",

  "low-risk-insufficient-data":
    "Paused for a human: this is a low-risk change, but there isn't yet enough verdict history to measure whether the reviewer can be trusted to auto-merge.",

  "low-risk-provisional-trust":
    "Auto-merged: this is a low-risk change merged on cold-start provisional trust while the agreement history is still building.",

  "medium-risk":
    "Paused for a human: this is a medium-risk change, which always waits for a human regardless of reviewer agreement.",

  "high-risk":
    "Paused for a human: this is a high-risk change, which always waits for a human regardless of reviewer agreement.",

  "no-tier-no-signal":
    "Paused for a human: the change has no risk tier on its manifest, so the gate has no signal to act on and defers to a human.",
 ...
 */
const UNKNOWN_REASON_EXPLANATION =
  "Paused for a human: the auto-merge gate returned an unrecognized reason, so the decision could not be explained automatically.";
 ...
 */
export function explainGateReason(reason: string): string {
