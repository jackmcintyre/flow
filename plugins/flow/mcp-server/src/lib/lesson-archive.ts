 ...
 */
export function archivedLessonsPath(
 ...
 * corruption recovery).
 */
export async function readArchivedLessons(
  targetRepoRoot: string,
  role: string,
): Promise<ArchivedLesson[]> {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed as ArchivedLesson[];
    }
    return [];
  } catch {
    return [];
 ...
 */
export async function appendArchivedLessons(
  targetRepoRoot: string,
  role: string,
  ...
  const newEntries: ArchivedLesson[] = lessons
