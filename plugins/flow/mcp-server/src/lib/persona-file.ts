 ...
 */
export function parsePersonaFile(raw: string, sourcePath: string): PersonaFile {
  ...
  if (knowledgeIdx <= promptIdx) {
    throw new PersonaFileMalformedError({
      personaPath: sourcePath,
      zodMessage:
        "required '##' sections are out of order — 'Knowledge' must appear after 'Prompt'",
    });
  }
...
function extractSections(
  body: string,
): Partial<Record<RequiredPersonaSection, string>> {
  ...
  const filtered: Partial<Record<RequiredPersonaSection, string>> = {};
