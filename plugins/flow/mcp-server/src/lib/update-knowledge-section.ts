...
import type { PersonaFrontmatter } from "../schemas/persona.js";
 ...
 */
export async function removeKnowledgeLessonsById(
  targetRepoRoot: string,
  role: string,
  idsToRemove: Set<string>,
): Promise<UpdatedPersonaResult> {
  ...
  const { newBody, removedCount } = removeBlocksById(
 ...
 */
export async function updateLessonUsageInPersona(
  targetRepoRoot: string,
  role: string,
  id: string,
  useCount: number,
  ...
  const { newBody, modifiedCount } = updateBlockUsage(
    parsed.sections["Knowledge"],
    id,
    useCount,
    lastUsedAt,
...
function removeBlocksById(
  ...
  idsToRemove: Set<string>,
): { newBody: string; removedCount: number } {
 ...
 */
function updateBlockUsage(
  knowledgeBody: string,
  id: string,
  useCount: number,
  lastUsedAt: string,
): { newBody: string; modifiedCount: number } {
  const lines = knowledgeBody.split("\n");
  const result: string[] = [];
  let modifiedCount = 0;
