...
const BASE_MANIFEST = {
  ...
  adapter: "native",
  source_path: ".flow/native-stories/01HZABC0000000000000000001.md",
...
describe("ready field (Story 9.1)", () => {
  ...
  const TODO_MANIFEST_WITHOUT_READY = {
    ...
    adapter: "native",
    source_path: ".flow/native-stories/01HZABC0000000000000000002.md",
...
describe("tasks / cited_sources / narrative_struct fields (Story 10.2)", () => {
  ...
  it("rejects a task with an empty ac_refs array (min(1))", () => {
    expect(() =>
      parseExecutionManifest(
        { ...BASE_MANIFEST, tasks: [{ text: "no refs", ac_refs: [] }] },
        { absPath: "/fake/path.yaml" },
      ),
    ).toThrow(MalformedExecutionManifestError);
  });
  ...
  it("rejects a narrative_struct missing a part (so_that)", () => {
    expect(() =>
      parseExecutionManifest(
        { ...BASE_MANIFEST, narrative_struct: { role: "x", want: "y" } },
        { absPath: "/fake/path.yaml" },
      ),
    ).toThrow(MalformedExecutionManifestError);
  });
  ...
  it("preserves the strict posture — an unknown key alongside the new fields is still rejected", () => {
    expect(() =>
      parseExecutionManifest(
        { ...NATIVE_10_2_MANIFEST, not_a_real_field: 1 },
        { absPath: "/fake/path.yaml" },
      ),
    ).toThrow(MalformedExecutionManifestError);
  });
