...
function expectFileRejected(input: unknown, msgFragment?: string | RegExp): void {
  try {
    ...
    if (!(err instanceof MalformedRetroProposalError)) {
      throw new Error(
        `expected MalformedRetroProposalError, got ${(err as Error).constructor.name}: ${String(err)}`,
      );
    }
...
function parseProposal(input: unknown): RetroProposal {
  const r = RetroProposalSchema.safeParse(input);
  if (!r.success) {
    throw new Error(
      `expected proposal to parse, got Zod issues: ${JSON.stringify(r.error.issues)}`,
    );
  }
  return r.data;
}
...
function fileWith(proposal: unknown): unknown {
  return {
    iso_timestamp: ISO,
    cycle_window: null,
    proposals: [proposal],
...
describe("RetroProposalSchema (AC2 — discriminated union surface)", () => {
  it("exposes exactly thirteen proposal types", () => {
...
describe("RuleProposalSchema (AC3)", () => {
  ...
  it("rejects an unknown recommended_promotion_level value", () => {
    expectFileRejected(
      fileWith({ ...VALID, recommended_promotion_level: "maybe" }),
    );
  });
...
describe("SkillCreateProposalSchema (AC4)", () => {
  ...
  it("rejects an absolute proposed_path (leading '/')", () => {
    expectFileRejected(
      fileWith({ ...VALID, proposed_path: "/etc/passwd" }),
    );
  });
...
describe("TeamChangeProposalSchema (AC5)", () => {
  ...
  it("rejects a non-kebab-cased target_role (Title Case)", () => {
    expectFileRejected(
      fileWith({ ...VALID, target_role: "SecurityReviewer" }),
    );
  });

  it("rejects a target_role with spaces", () => {
    expectFileRejected(
      fileWith({ ...VALID, target_role: "security reviewer" }),
    );
  });
...
describe("RuleRetirementProposalSchema (AC6)", () => {
  ...
  it("rejects a malformed target_rule_id (not a ULID)", () => {
    expectFileRejected(
      fileWith({ ...VALID, target_rule_id: "not-a-ulid" }),
    );
  });
  ...
  it("rejects an unknown recommended_action", () => {
    expectFileRejected(
      fileWith({ ...VALID, recommended_action: "delete" }),
    );
  });
});
...
describe("SkillReviseProposalSchema (AC6)", () => {
  ...
  it("rejects a traversal target_skill_path", () => {
    expectFileRejected(
      fileWith({ ...VALID, target_skill_path: "../escape.md" }),
    );
  });
});
...
describe("SkillSupersedeProposalSchema (AC6)", () => {
  const VALID = {
    ...
    superseded_skill_path: ".flow/skills/old-x.md",
    replacement: {
      proposed_path: ".flow/skills/new-x.md",
  ...
  it("happy path — parses a valid skill-supersede proposal", () => {
    ...
    if (p.type === "skill-supersede") {
      expect(p.replacement.proposed_path).toBe(".flow/skills/new-x.md");
    }
  });
...
describe("SkillRetireProposalSchema (AC6)", () => {
  const VALID_WITH_DATE = {
    ...
    target_skill_path: ".flow/skills/old-x.md",
  ...
  it("rejects a rule proposal carrying a skill-create's proposed_path", () => {
    ...
    expectFileRejected(
      fileWith({
        ...
        proposed_path: ".flow/skills/x.md",
      }),
    );
  });
...
describe("ProposalBase guards", () => {
  ...
  it("rejects a malformed id (lowercase)", () => {
    expectFileRejected(
      fileWith({ ...BASE_RULE_VALID, id: ULID.toLowerCase() }),
    );
  });
  ...
  it("rejects a non-UTC created_at (offset form)", () => {
    expectFileRejected(
      fileWith({ ...BASE_RULE_VALID, created_at: "2026-05-28T14:32:11+02:00" }),
    );
  });

  it("rejects a malformed created_at (not ISO)", () => {
    expectFileRejected(
      fileWith({ ...BASE_RULE_VALID, created_at: "2026-05-28" }),
    );
  });
});
    ...
    const file = parseRetroProposalFile({
      ...
      proposals: [
        ...
        },
        {
          ...
          created_at: ISO,
          rationale: "r3",
          target_skill_path: ".flow/skills/never-fires.md",
          last_invoked_at: null,
        },
      ],
    });
