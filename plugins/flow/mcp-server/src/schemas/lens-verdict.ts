...
export const PanelVerdictSchema = z
  .object({
    ...
    lenses: z
      ...
      .refine(
        (lenses) => new Set(lenses.map((l) => l.lens)).size === lenses.length,
        { message: "every lens verdict must be for a distinct lens — no lens repeated" },
      )
      .refine(
        (lenses) => LENS_NAMES.every((name) => lenses.some((l) => l.lens === name)),
        { message: "all five Tier-1 lenses must be present — no lens skipped" },
      ),
  })
  .strict();
