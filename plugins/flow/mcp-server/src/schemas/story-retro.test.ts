...
const VALID_PATTERN: Record<string, unknown> = {
  id: ULID,
  kind: "pattern",
...
const VALID_PITFALL: Record<string, unknown> = {
  id: ULID,
  kind: "pitfall",
...
describe("StructuredLessonSchema — happy-path round-trips", () => {
  ...
  it("parses a lesson with optional source_pr", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.source_pr).toBe(
        "https://github.com/jackmcintyre/crew/pull/286",
      );
    }
  });
...
describe("StructuredLessonSchema — required field rejections", () => {
  ...
  it("rejects a lesson with a malformed id (not a ULID)", () => {
    expect(
      StructuredLessonSchema.safeParse({ ...VALID_PATTERN, id: "not-a-ulid" }).success,
    ).toBe(false);
  });
  ...
  it("rejects a lesson with an unknown kind", () => {
    expect(
      StructuredLessonSchema.safeParse({ ...VALID_PATTERN, kind: "advice" }).success,
    ).toBe(false);
  });
  ...
  it("rejects a lesson with an empty applies_when", () => {
    expect(
      StructuredLessonSchema.safeParse({ ...VALID_PATTERN, applies_when: "" }).success,
    ).toBe(false);
  });
  ...
  it("rejects a lesson with a non-UTC learned_at (offset form)", () => {
    expect(
      StructuredLessonSchema.safeParse({
        ...VALID_PATTERN,
