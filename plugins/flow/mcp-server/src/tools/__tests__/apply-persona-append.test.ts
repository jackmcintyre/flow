...
function personaAppendOnlyRegistry(): ProposalApplyRegistry {
...
function personaAppendProposal(id: string, lesson: string): Record<string, unknown> {
  return {
    type: "persona-append",
    id,
    created_at: ISO,
...
async function seedPersona(root: string, role: string, content: string): Promise<string> {
...
describe("persona-append schema and registry (AC4)", () => {
  it("persona-append is present in RETRO_PROPOSAL_TYPES and the count is 10", () => {
...
describe("persona-append — missing persona file (AC3)", () => {
  ...
  it("PersonaFileNotFoundError names the role and path", async () => {
    ...
    const e = caught as PersonaFileNotFoundError;
...
describe("persona-append — confirmed apply (AC1)", () => {
  ...
  it("only modifies the target role's persona file (no other persona changed)", async () => {
    ...
    const otherBefore = await fs.readFile(
      path.join(tmpRoot, "team", otherRole, "PERSONA.md"),
      "utf8",
    );
