...
let scratch: string;
let root: string;
let storiesDir: string; // BMad source stories
let nativeDir: string; // .flow/native-stories

async function listNativeFiles(): Promise<string[]> {
  try {
...
async function seedBmadStory(filename: string, body: string): Promise<string> {
...
function bmadStoryBody(epic: number, story: number, title: string): string {
  return [
 ...
 */
const cleanEnricher: BmadEnricher = (story): EnrichedDraft => ({
  title: story.title,
...
beforeEach(async () => {
  scratch = await fs.mkdtemp(path.join(os.tmpdir(), "flow-bmad-ingest-"));
  root = path.join(scratch, "workspace");
  storiesDir = path.join(root, STORIES_REL);
  nativeDir = path.join(root, ".flow", "native-stories");
...
describe("bmadToNativeIngest AC1 — every input is accounted for", () => {
  it("enriches a signal-carrying story to a Tier-0-clearing native file, surfaces an un-enrichable one, drops nothing", async () => {
    ...
    const enrich: BmadEnricher = (story) => {
      if (story.ref === "bmad:1.1") return cleanEnricher(story);
      return {
        title: story.title,
    ...
    expect(
      report.written.length + report.needs_fix_up.length + report.skipped.length,
...
describe("bmadToNativeIngest AC2 — no active-adapter guard", () => {
  it("writes to .flow/native-stories/ with config.yaml still set to adapter: bmad", async () => {
    ...
    const cfg = await fs.readFile(path.join(root, ".flow", "config.yaml"), "utf8");
...
describe("bmadToNativeIngest AC3 — non-destructive + idempotent re-run", () => {
  it("re-running dedupes by provenance — the already-ingested story is skipped, not re-written with a new ULID", async () => {
    ...
    expect(second.skipped[0]!.existing_native_path).toContain(
      firstNativeRef.replace("native:", ""),
    );
    ...
    expect(
      second.written.length + second.needs_fix_up.length + second.skipped.length,
    ).toBe(second.input_count);
  });
...
describe("bmadToNativeIngest AC4 — Tier-0 gate is the sole arbiter", () => {
  it("rejects a hollow enrichment (plausible prose, no cited sources) at the gate, not the enricher — nothing written", async () => {
    ...
    const hollowEnricher: BmadEnricher = (story) => ({
      title: story.title,
      narrative: { role: "developer", want: "a thing", so_that: "value" },
      acceptance_criteria: [
        {
  ...
  it("rejects an enrichment whose task references a non-existent AC (T0-1), nothing written", async () => {
    ...
    const danglingEnricher: BmadEnricher = (story) => ({
      title: story.title,
      narrative: { role: "developer", want: "a thing", so_that: "value" },
      acceptance_criteria: [
        {
