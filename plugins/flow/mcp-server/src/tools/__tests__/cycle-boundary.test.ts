...
async function seedNativeWorkspace(root: string): Promise<void> {
  ...
  await atomicWriteFile(
    path.join(root, ".flow", "native-stories", "01KT484NY4HCBPBTT6VEY1Q0CS.md"),
    "# fixture story\n",
  );
}
...
function agentInvokeLine(ts: string, story: string): string {
...
async function seedTelemetry(fileName: string, lines: string[]): Promise<void> {
  await atomicWriteFile(
    path.join(tmpRoot, ".flow", "telemetry", fileName),
    lines.join("\n") + "\n",
  );
}

async function seedProposal(isoStamp: string): Promise<void> {
  await atomicWriteFile(
    path.join(tmpRoot, ".flow", "retro-proposals", `${isoStamp}.md`),
    "# a proposal\n",
  );
}
...
describe("AC2 — retro inputs are scoped to the open cycle's window", () => {
  it("returns only done manifests + telemetry from after the cycle opened", async () => {
    ...
    const inScopeStoryEvents = bundle.telemetrySummary.events.filter(
      (e) => e.type === "agent.invoke",
    );
...
describe("AC3 — the prior cycle's record is archived on the next open", () => {
  it("writes the prior cycle's manifests, proposals, and telemetry summary to .flow/cycle-archive/", async () => {
    ...
    const record = yamlParse(raw) as {
      ...
      retro_proposals: Array<{ iso_timestamp: string }>;
...
describe("AC4 — no cycle ever opened preserves the baseline", () => {
  ...
  it("an explicit cycleState: null forces the full-history baseline regardless of disk", async () => {
    ...
    expect(bundle.doneManifests.map((m) => m.ref)).toContain(
      "native:STORYONLY000000000000001",
    );
  });
});
