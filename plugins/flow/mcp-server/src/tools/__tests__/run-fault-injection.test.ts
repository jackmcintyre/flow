/**
 * Run fault-injection integration test — native:01KT19N3H7WZCF1SKQSWDGARF4.
 *
 * Proves the unattended run reports every story it picks up HONESTLY when one
 ...
 * zero, never two — and a run-level run reason, rather than throwing.
 *
 * This is ADDITIVE test infrastructure only. There is NO production-behaviour
 * change to the run: the harness runs the real `run.workflow.js` body
 * VERBATIM through the existing AsyncFunction runner, exactly as
 * run-observability-non-fatal.test.ts does. That means it runs the production
 * bounded `runWorker` concurrency pool
 ...
 * seam is therefore CAUGHT by `runWorker` and bucketed as
 ...
 * needs-human pause notification (run.workflow.js notifyHumanNeeded) can be
 ...
 *         non-empty reason, runReason set, run did not throw.
...
interface RunOpts {
  /** Stories the `claim:` seam hands out in order; an empty queue → queue-emptied. */
  queue?: QueuedStory[];
  /** Concurrency for the main run loop (the workflow's maxConcurrency arg). */
   ...
   */
  claimAfterQueue?: "queue-emptied" | "waiting-on-in-progress";
...
/**
 * Does `label` belong to story `ref`? Run seam labels for a story carry the ref
...
async function runRun(opts: RunOpts = {}): Promise<{
  ...
  const storyForClaimIdx = (idx: number): unknown => {
    const s = queue[idx];
    if (!s) return { next: opts.claimAfterQueue ?? "queue-emptied" };
    return {
      next: "spawn-dev",
      ref: s.ref,
      title: s.title,
  ...
  const seamResult = (label: string): unknown => {
    ...
    // fix/run-isolation-coordination-honesty: completeStory now runs as a run
    ...
    if (label.startsWith("progress-start:")) {
      ...
      return runPhaseStart({ ref: refOfLabel(label) ?? "x", phase: phase as any });
    }
  ...
  const agent = async (prompt: string, agentOpts: AgentCall["opts"] = {}) => {
    ...
    // runWorker's catch buckets it as worker-threw.
    if (
      opts.throwSeam &&
    ...
    if (label.startsWith("dev:")) {
      ...
      // INSTEAD of the handoff phrase (run.workflow.js NEEDS_HUMAN_MARKER).
  ...
  // (run.workflow.js notifyHumanNeeded). Capturing it lets AC4 assert the
...
const REF = "run-fi-story-1";
const REF_A = "run-fi-story-a";
const REF_B = "run-fi-story-b";
 ...
 * returned a run reason without throwing.
 *
 * Per run-result-and-exit-reasons.md, `merged`/`pausedForHuman` refs are a
...
function assertHonestyInvariant(
  ...
  // The run finished on its own — it did not throw and it set a run reason.
...
describe("run fault-injection — honest reporting under misbehaving seams (native:01KT19N3H7WZCF1SKQSWDGARF4)", () => {
  it("AC1: under each injected fault the run finishes itself and reports every story in exactly one outcome with a reason", async () => {
    ...
    const faults: Array<{ name: string; opts: RunOpts }> = [
    ...
    for (const { name, opts } of faults) {
      const { result, thrown } = await runRun(opts);
      ...
      try {
        ...
        // A genuine full run: the queue emptied (no fault aborts the loop).
  ...
  it("AC2: a garbled VERDICT relay buckets that story in blocked with the verdict-step reason, run keeps going", async () => {
    ...
    // processStory buckets A in `blocked` WITHOUT aborting (run.workflow.js
  ...
  it("AC3: under concurrency, one story's hard failure blocks ONLY it (worker-threw) — the sibling completes and the run never aborts", async () => {
    ...
    // seam() re-throws, processStory propagates, and runWorker's per-worker
    ...
    // blocked reason") — proving the runWorker backstop fired, not a path
    // inside processStory.
    const a = result.blocked.find((b: any) => b.ref === REF_A);
    expect(a, "story A should be blocked").toBeDefined();
    ...
    // And the run still emptied the queue.
    expect(result.runReason).toBe("queue-emptied");
    assertHonestyInvariant(result, thrown, [REF_A, REF_B]);
  });
  ...
  it("AC4: a story that genuinely needs a decision pauses for the human AND delivers a notify payload carrying the ref + verbatim question", async () => {
    ...
    const { result, thrown, notifications } = await runRun({
      queue,
    ...
    // verbatim question (run.workflow.js needs-human branch).
    const p = result.pausedForHuman.find((x: any) => x.ref === REF);
  ...
  it("AC5: a failing INFORMATIONAL status seam drops only its line — buckets + reasons are identical to a clean run", async () => {
    ...
    // The structured result — buckets AND run reason — is byte-identical: the
    // cosmetic status hiccup changed what happened to NO story.
    expect(degraded.result).toEqual(clean.result);
    expect(clean.result.runReason).toBe("queue-emptied");
  ...
  it("TERMINATION: a manifest wedged in in-progress/ with no live worker makes the run EXIT (stalled-in-progress), never spin forever", async () => {
    // fix/run-isolation-coordination-honesty — the non-termination fix.
    //
    ...
    // runReason 'stalled-in-progress' instead of spinning.
    //
    ...
    expect(thrown).toBeUndefined();
    expect(result.runReason).toBe("stalled-in-progress");
    expect(result.queueEmptied).toBe(false);
