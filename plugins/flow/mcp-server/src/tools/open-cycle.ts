...
// ---------------------------------------------------------------------------
// Archive shape (the YAML record written to .flow/cycle-archive/)
// ---------------------------------------------------------------------------

interface CycleArchiveRecord {
  prior_cycle_id: string;
  archived_at: string;
  done_manifest_refs: string[];
  retro_proposal_paths: string[];
 ...
 */
export async function openCycle(opts: OpenCycleOptions): Promise<OpenCycleResult> {
  ...
  let archivedPriorCycleId: string | null = null;
  if (priorState !== null) {
  ...
  const newState: CycleState = { cycle_id: cycleId, opened_at: openedAt };
...
async function archivePriorCycle(
  ...
  const archiveRecord: CycleArchiveRecord = {
...
async function gatherDoneManifestRefs(targetRepoRoot: string): Promise<string[]> {
...
async function gatherRetroProposalPaths(targetRepoRoot: string): Promise<string[]> {
...
async function countTelemetryLines(targetRepoRoot: string): Promise<number> {
