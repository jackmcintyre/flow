 ...
 */
export function routeLessonDurability(
  input: LessonRoutingInput,
  ...
  if (
    (lesson.kind === "pitfall" || lesson.kind === "tool-quirk") &&
    ...
    return {
      tier: "code",
      reason:
        "This failure has a stable mechanical shape and keeps recurring — a guard makes it impossible",
    };
  }
...
export async function writeRetroProposal(
...
): Promise<{ absPath: string; proposalCount: number; routedLessons: RoutedLesson[] }> {
  const {
    targetRepoRoot,
    isoTimestamp,
    proposals,
  ...
  const routedLessons: RoutedLesson[] = lessonRoutings.map((input) => ({
