...
describe("parseRiskTieringSpec — frontmatter delimiters", () => {
  it("rejects an empty file", () => {
    expect(() => parseRiskTieringSpec("", SOURCE_PATH, COPY_TARGET)).toThrow(
  ...
  it("rejects a whitespace-only file", () => {
    expect(() => parseRiskTieringSpec("   \n  \n", SOURCE_PATH, COPY_TARGET)).toThrow(
      /empty or whitespace-only/,
    );
  });
  ...
  it("rejects a file that does not start with ---", () => {
    expect(() =>
      parseRiskTieringSpec("version: 1.0.0\n", SOURCE_PATH, COPY_TARGET),
    ).toThrow(/missing YAML frontmatter opener/);
  });

  it("rejects a file with opening --- but no closing ---", () => {
    expect(() =>
      parseRiskTieringSpec("---\nversion: \"1.0.0\"\n", SOURCE_PATH, COPY_TARGET),
    ).toThrow(/missing YAML frontmatter closer/);
  });
...
describe("parseRiskTieringSpec — YAML errors", () => {
  it("rejects invalid YAML syntax", () => {
    const raw = "---\n: invalid: yaml: {\n---\n";
    expect(() => parseRiskTieringSpec(raw, SOURCE_PATH, COPY_TARGET)).toThrow(
      MalformedRiskTieringSpecError,
    );
  });
});
...
describe("parseRiskTieringSpec — round-trip", () => {
  ...
  it("error includes sourcePath and copyTarget", () => {
    let caught: MalformedRiskTieringSpecError | undefined;
    try {
      parseRiskTieringSpec("", SOURCE_PATH, COPY_TARGET);
    } catch (e) {
      caught = e as MalformedRiskTieringSpecError;
    }
