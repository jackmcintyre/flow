...
import { RiskTieringSpecSchema, type RiskTieringSpec } from "../schemas/risk-tiering-spec.js";
 ...
 */
export function parseRiskTieringSpec(
  raw: string,
  sourcePath: string,
  copyTarget: string,
): RiskTieringSpec {
  ...
  // Post-Zod invariant: duplicate rule id detection
  type TierName = "low" | "medium" | "high";
  const tierNames: TierName[] = ["low", "medium", "high"];
  const seen = new Map<string, { tier: TierName; index: number }>();
  ...
  for (const tier of tierNames) {
    ...
    for (const rule of rules) {
      ...
      if (
        thresholds?.min_lines_changed !== undefined &&
