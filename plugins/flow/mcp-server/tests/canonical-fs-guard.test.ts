...
const FS_WRITE_WHITELIST = new Set<string>([
  ...
  // Story 5.32: index.ts now writes the daemon's PID file to ~/.flow/mcp-daemon.pid
  ...
  // it lives outside any target-repo .flow/state/** path. Whitelisted because the
...
describe("writeManagedFile runtime guard (AC5c runtime)", () => {
  ...
  it("rejects canonical-state writes without an MCP tool context", async () => {
    ...
    tmpDirs.push(root);
    const target = path.join(root, ".flow", "state", "to-do", "bmad:1.yaml");
    ...
    try {
      ...
      expect(e.message).toContain(".flow/state/**");
  ...
  it("permits canonical writes when an MCP tool context is provided", async () => {
    ...
    tmpDirs.push(root);
    const target = path.join(root, ".flow", "state", "to-do", "bmad:2.yaml");
