 ...
 *   2. The script source contains the substring `$HOME/.claude/plugins/cache/flow/flow`
...
describe("dev-install.sh content-anchor (Story 1.11 AC6)", () => {
  ...
  it("script contains the cache path substring targeting the right location", () => {
    ...
    // The script uses $HOME/.claude/plugins/cache/flow/flow in its printed output
    // and variable assignments. This substring proves it targets the correct cache.
    expect(content).toContain("$HOME/.claude/plugins/cache/flow/flow");
  });
 ...
 * Returns { repoRoot, pluginsFlowDir, cacheParent }.
 */
function makeTempEnvironment(): {
  repoRoot: string;
  pluginsFlowDir: string;
  cacheParent: string;
  cleanup: () => Promise<void>;
} {
  const base = mkdtempSync(resolve(tmpdir(), "flow-dev-install-"));
  const repoRoot = resolve(base, "repo");
  const pluginsFlowDir = resolve(repoRoot, "plugins", "flow");
  const cacheParent = resolve(base, "home");

  // Create the source plugin tree.
  mkdirSync(resolve(pluginsFlowDir, ".claude-plugin"), { recursive: true });
  mkdirSync(resolve(pluginsFlowDir, "mcp-server", "dist"), { recursive: true });
  mkdirSync(resolve(pluginsFlowDir, "skills", "sentinel"), { recursive: true });

  writeFileSync(
    resolve(pluginsFlowDir, ".claude-plugin", "plugin.json"),
    JSON.stringify({ name: "flow", version: "0.0.0-test", description: "test" }),
  );
  writeFileSync(
    resolve(pluginsFlowDir, "mcp-server", "dist", "index.js"),
    "// test stub\n",
  );
  writeFileSync(
    resolve(pluginsFlowDir, "skills", "sentinel", "SKILL.md"),
    "# Sentinel skill\nOriginal content.\n",
  );
...
describe("dev-install.sh integration scenario (Story 1.11 AC7)", () => {
  ...
  it("AC7-1: first run creates a symlink from cache to the source dir", () => {
    ...
    const expectedReal = realpathSync(env.pluginsFlowDir);
  ...
  it("AC7-3: edit propagation — changes in source are immediately visible via cache (symlink)", () => {
    ...
    const skillPath = resolve(env.pluginsFlowDir, "skills", "sentinel", "SKILL.md");
  ...
  it("AC7 error path: exits 3 when dist/index.js is missing", () => {
    rmSync(resolve(env.pluginsFlowDir, "mcp-server", "dist", "index.js"));
  ...
  it("error path exit 2: exits 2 when run outside a git repo", () => {
    ...
    const pluginsFlowDir = resolve(base, "plugins", "flow");
    mkdirSync(resolve(pluginsFlowDir, ".claude-plugin"), { recursive: true });
    mkdirSync(resolve(pluginsFlowDir, "mcp-server", "dist"), { recursive: true });
    writeFileSync(
      resolve(pluginsFlowDir, ".claude-plugin", "plugin.json"),
  ...
  it("error path exit 2: exits 2 when plugin.json is missing", () => {
    ...
    rmSync(resolve(env.pluginsFlowDir, ".claude-plugin", "plugin.json"));
