...
async function seedConfig(root: string): Promise<void> {
  ...
  await fs.writeFile(path.join(root, ".flow", "config.yaml"), VALID_CONFIG_YAML, "utf8");
}
...
describe("getStatus", () => {
  it("AC4a — valid standards.md → standards.state=ok, render starts with 'flow v', 'standards: ok — '", async () => {
    ...
    expect(lines[0]).toBe(`flow v${report.pluginVersion}`);
  ...
  it("AC1/AC3 — render's first line matches the SEMVER_REGEX-anchored 'flow v…' shape and standards line starts with 'standards: ok — '", async () => {
    ...
    expect(lines[0]).toMatch(/^flow v\d+\.\d+\.\d+(?:-[\w.]+)?$/);
    expect(lines[3]!.startsWith("standards: ok — ")).toBe(true);
  });
  ...
  it("end-to-end via MCP — registerAllTools registers getStatus, ListTools includes it, CallTool returns the rendered text", async () => {
    ...
    // pre-existing `.flow/config.yaml`. Because validateActiveAdapter calls
