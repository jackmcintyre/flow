...
describe("NativeAdapter.detect()", () => {
  it("returns true for the fixture repo (has ULID .md files under .flow/native-stories/)", async () => {
  ...
  it("returns false when the directory exists but has no ULID .md files", async () => {
    ...
    try {
      await fs.mkdir(path.join(tmp, ".flow", "native-stories"), { recursive: true });
      // Write a non-ULID file — should not match.
      await fs.writeFile(path.join(tmp, ".flow", "native-stories", "README.md"), "# ignore\n");
      expect(await NativeAdapter.detect(tmp)).toBe(false);
    } finally {
      await fs.rm(tmp, { recursive: true, force: true });
    }
  ...
  it("returns true when exactly one ULID .md file is present", async () => {
    ...
    try {
      const dir = path.join(tmp, ".flow", "native-stories");
      await fs.mkdir(dir, { recursive: true });
      await fs.cp(
        path.join(FIXTURE_REPO, ".flow", "native-stories", "01JX9000000000000000000001.md"),
...
describe("NativeAdapter.listSourceStories()", () => {
  ...
  it("silently skips non-ULID files in the stories directory", async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(tmp, ".flow", "native-stories", "not-a-ulid.md"), "# ignore\n");
  ...
  it("Story 10.3 — a malformed ULID-named file is contained (warned + skipped), NOT a whole-scan abort", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tmp, ".flow", "native-stories", `${badUlid}.md`),
...
describe("NativeAdapter.resolveSourcePath()", () => {
  ...
  it("returns the absolute path for a valid native ref", () => {
    ...
    expect(p).toContain(path.join(".flow", "native-stories"));
  });
