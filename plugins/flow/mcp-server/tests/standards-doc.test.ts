...
describe("standards-example.md (shipped copy-target)", () => {
  ...
  it("AC4 (README install path): plugins/flow/README.md references standards-example.md as the copy-target", () => {
    const readmePath = path.resolve(HERE, "..", "..", "README.md");
    const readme = readFileSync(readmePath, "utf8");
    expect(readme).toContain("plugins/flow/docs/standards-example.md");
