...
async function createNativeTargetRepo(tmpDir: string): Promise<string> {
  ...
  const nativeStoriesDir = path.join(flowDir, "native-stories");
  ...
  await fs.writeFile(path.join(flowDir, "config.yaml"), config, "utf8");
...
describe("validatePlannerBacklog — AC1: missing-integration-AC", () => {
  ...
  it("does NOT write any file as a side effect", async () => {
    ...
    const flowEntries = await fs.readdir(path.join(tmpDir, ".flow"));
    expect(flowEntries.sort()).toEqual(["config.yaml", "native-stories"]);
