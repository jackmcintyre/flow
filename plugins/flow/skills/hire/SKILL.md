...
allowed_tools: [Read, Task, readPersona, instantiatePersona, analyzeTeamFit]
---
...
4. **Gather demand-driven fit analysis (fail-soft).** Call `analyzeTeamFit({ targetRepoRoot })` once. If the call succeeds, cache the result as `fitAnalysis`. If the call throws any error (e.g. a fresh repo with no backlog or no `.flow/config.yaml`), set `fitAnalysis = null` and continue — this is graceful degradation, not a failure. Never abort the hire flow because `analyzeTeamFit` errored.
5. **Spawn the hiring-manager subagent** via Claude Code's `Task` tool. Assemble the system prompt as `readCatalogue({ role: "hiring-manager" })`'s `Prompt` section verbatim followed by an `<initial-context>` block containing the serialised `RepoSignals`, `currentRoster` array (empty in fresh-hire mode), `<custom-roles>...JSON.stringify(customRoles) (array of CatalogueRole, possibly empty)...</custom-roles>`, and — when `fitAnalysis` is non-null — `<fit-analysis>...JSON.stringify(fitAnalysis)...</fit-analysis>` (omit the `<fit-analysis>` element entirely when `fitAnalysis` is null). The subagent's allowlist is enforced by `permissions/hiring-manager.yaml`.
6. **Pass the conversation through.** The skill is a thin orchestrator — the subagent owns the proposal grammar, approve/decline/amend handling, and re-entry actions per the catalogue prompt.
7. **Exit conditions.** Watch for the catalogue's terminal handoff signal — exactly `Handoff to planner — team hired, ready to plan` in fresh-hire mode, or the operator-typed `done` in re-entry mode. The verdict-grammar parser is not invoked here (this is not a reviewer skill).

# Failure modes

- **Adapter-resolution error from any allowlisted tool:** the core allowlisted tools (`readCatalogue`, `instantiatePersona`, `readPersona`, `lookupRoleByDomain`, `readRepoSignals`, `readCustomRole`, `heartbeat`) do not require adapter resolution; a `NoAdapterMatchedError` from these is a programming bug — surface the error in the reply for the operator to file, but do not abort the hire flow on its account. `analyzeTeamFit` and `readBacklogInventory` require adapter resolution and are expected to fail on a fresh repo — treat any error from either as `fitAnalysis = null` (graceful degradation, see step 4).
- **`analyzeTeamFit` errors on a fresh repo:** this is expected (no backlog yet); the skill sets `fitAnalysis = null` and the subagent falls back to surface-clue-only proposal. Not a failure.
