...
3. **Mint or reuse a session ULID** for this panel run (the per-lens verdict files are namespaced under it). Obtain the lens→role binding from the live hired roster by calling `resolveLensRoles({ targetRepoRoot })` — this is the only permitted source for the binding. The tool runs maximum bipartite matching over the hired team and returns `{ lensRoles }` with all five lenses assigned to five DISTINCT roles, preferring a specialist (e.g. test-specialist for Verifiability) when one is hired. Do NOT hand-enumerate a binding (e.g. Structure→architect, Verifiability→test-specialist…) — any hard-coded or operator-supplied binding is superseded by `resolveLensRoles`. If the tool throws `LensJudgeUnavailableError`, the roster is too small to staff the panel; surface the error verbatim and stop.
