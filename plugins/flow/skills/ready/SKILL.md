...
This is the minimal **intake cockpit** for the readiness brake (Epic 9). The drain only ever claims a backlog item once you have explicitly marked it **ready** — a freshly-scanned item sits in the backlog but is *not claimable* until you bless it here. `/flow:ready` lets you:
...
- **No `.flow/config.yaml` / no backlog:** if `listClaimableTodos` returns an empty `todos` list, the backlog has nothing to bless yet — run `/flow:scan` to project source stories into the backlog first.
