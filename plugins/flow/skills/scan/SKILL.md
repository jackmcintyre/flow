...
Projects your active planning tool's source stories into per-story execution manifests under `<target-repo>/.flow/state/to-do/<ref>.yaml`. Idempotent: re-running this skill after no source changes is a no-op. (Internally the skill invokes the `scan-sources` MCP tool, registered on the flow server.)
