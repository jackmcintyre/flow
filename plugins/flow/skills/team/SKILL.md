...
- **No `team/` directory:** the snapshot renders the empty-state block — `crew team — 0 role(s)` followed by a cross-reference to `/flow:hire` and `/flow:skip-hiring`. This is not an error; run one of those skills to create the team.
